    ...
    wx.config({
        ...
        wx.ready(function (){
            ...
            wx.onMenuShareTimeline({
                ...
                link: "<?=!empty($givecard_url)?"{$givecard_url}&member_card_id={$card_info['member_card_id']}&cardOpenid={$card_openid}":''?>", // 分享链接
                imgUrl: "<?php echo $card_info['logo_url'] ?>", // 分享图标
                success: function () {
                    var post_url = "<?=!empty($hang_card_url)?$hang_card_url:''?>";
                    ...
                    function(data){
                        window.location.href="<?=!empty($center_url)?$center_url:''?>";
                    }, "json");
                },
                cancel: function () {
...
$(function(){
    ...
    <?php if($card_info){ ?>
        $('.getcard').click(function(){
            var post_url = "<?=!empty($savegivecard_url)?$savegivecard_url:''?>";
            var post_cardId = <?php echo $card_info['member_card_id']; ?>;
            var openid = "<?php echo $card_openid; ?>";
            var module = "<?php echo $card_info['receive_module'] ?>";
            $.post(post_url, {
