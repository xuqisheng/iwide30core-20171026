...
<?php if(!empty($orders)){foreach($orders as $o){?>
	<div class="item">
        ...
        <div class="webkitbox justify h20 martop overflow">
        	<?php if($o['status']==3){ ?><div><a href="<?php echo Hotel_base::inst()->get_url("TO_COMMENT",array('oid'=>$o['id']))?>" class="btn_void xs color_888">评价</a></div> <?php }?>
        	<div>
		        <?php if($o['re_pay']){ ?>
				<a href="<?php echo site_url('wxpay/hotel_order').'?id='.$inter_id.'&orderid='.$o['orderid'];?>" class="btn_void xs color_888" style="margin-right:20px">立即支付</a>
		        <?php } ?>
				<a href="<?php echo Hotel_base::inst()->get_url("ORDERDETAIL",array('oid'=>$o['id']))?>" class="btn_void xs color_888">订单详情</a>
            </div>
        </div>
    </div>
<?php }}?>
