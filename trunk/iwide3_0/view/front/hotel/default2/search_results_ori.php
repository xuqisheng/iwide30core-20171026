...
<?php if(!empty($result)){ foreach($result as $r){?>
	<div onclick="go_hotel('<?php echo Hotel_base::inst()->get_url("INDEX",array("h"=>$r->hotel_id));?>')" class="webkitbox justify">
        ...
        <?php if(!empty($r->lowest)){?>
            <div class="qi"><span class="color_main">¥</span><span class="color_main h36" id="lowest_p_<?php echo $r->hotel_id;?>"><?php if ($inter_id=='a476756979')echo intval($r->lowest);else echo $r->lowest;?></span></div>
            <?php }else{?>
