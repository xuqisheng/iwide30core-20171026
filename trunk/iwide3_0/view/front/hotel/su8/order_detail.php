...
<header class="realtime_status">
    ...
    <ul class="cur_status">
    	<li class="iscured">
            ...
            <div class="cur_status_txt" style="color:#d21527;">提交订单</div>
...
<section class="all_message">
    ...
    <ul class="otherway webkit_box" >
<!--     	<li><a href="#" class="normal ui_color">打车前往</a></li> -->
        <li><a href="#" onclick="tonavigate(<?php echo $order['latitude'];?>,<?php echo $order['longitude'];?>,'<?php echo $order['hname'];?>','<?php echo $order['haddress'];?>')" class="normal">路线导航</a></li>
        <li><a href="<?php if (!empty($hotel['arounds'])){?><?php echo site_url('hotel/hotel/arounds').'?id='.$inter_id.'&h='.$hotel['hotel_id']; }else{?>http://cps.dianping.com/mm/weixin/home?showwxpaytitle=1&utm_source=card<?php }?>" class="normal">酒店周边</a></li>
        <li><a href="tel:<?php echo $order['htel'];?>" class="normal">联系酒店</a></li>
    </ul>
</section>
...
<section class="order_message">
    ...
    <ul>
    	<li><span>入住人</span><span><?php echo $order['name'];?></span></li>
    	<li><span>电话号码</span><span><?php echo $order['tel'];?></span></li>
    	<li><span>订单编号</span><span><?php echo $order['show_orderid'];?></span></li>
        <li><span>订单总价</span><span class="ui_price ui_color"><?php echo $order['ori_price'];?></span></li>
...
<div style="padding-top:15%">
    ...
    <?php if($can_cancel==1){?>
		...
		<a class="cancelbtn" href="<?php echo site_url('hotel/hotel/to_comment'); ?>?id=<?php echo $inter_id; ?>&oid=<?php echo $order['id']; ?>">评价</a>
		<?php }?>
		<?php if($re_pay==1){?>
		<div class="submit_btn" onclick="check_repay()">支付订单</div>
		<?php }else{?>
      	<a class="submit_btn" href="<?php echo site_url('hotel/hotel/index'); ?>?id=<?php echo $inter_id?>&h=<?php echo $order['hotel_id']?>">再次预订</a>
        <?php }?>
    </div>
</div>
</body>
...
function check_repay(){
	pageloading('请稍候');
	$.get('/index.php/hotel/check/check_repay?id=<?php echo $inter_id;?>',{
		oid:'<?php echo $order['orderid']?>'
	},function(data){
		$('.page_loading').remove();
		if(data.s==1){
			location.href="<?php echo site_url('wxpay/hotel_order').'?id='.$inter_id.'&orderid='.$order['orderid'];?>";
		}
		else{
			$.MsgBox.Alert('通知',data.errmsg);
		}
