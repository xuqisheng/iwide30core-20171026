...
<?php if(isset($middle_ads['ads']) && !empty($middle_ads['ads'])) foreach($middle_ads['ads'] as $fad){ foreach($fad as $fa){?>
	<a href="<?php echo $fa['ad_link'];?>" class="pad3 bg_fff bd h24 martop color_main" style="display:block"><em class="iconfont"><?php if(!strstr($fa['ad_img'],"http")) echo $fa['ad_img'];?></em><?php echo $fa['ad_title'];?></a>
<?php }}?>
		...
		$.post('/index.php/hotel/hotel/return_more_room?id=<?php echo $inter_id;?>',{
			...
			$.each(data.rooms,function(i,n){
				...
				if(n.state_info!=''){
					...
					temp+='<em class="iconfont">&#x33;</em></div>';
				}
				else{
				...
				if(n.state_info!=''){
					$.each(n.state_info,function(sk,sn){
						...
						temp+='<div class="h24 color_999">'+sn.des+'</div></div>';
						temp+='<div class="webkitright">';
