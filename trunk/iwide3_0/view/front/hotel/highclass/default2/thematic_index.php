...
<header class="headers" <?php if($hidden==1) echo 'style="display:none"';?>>
...
<form <?php if($hidden==1) echo 'style="display:none"';?> action="<?php echo Hotel_base::inst()->get_url('SRESULT')?>" onsubmit="to_search($(this))" method="post" id="index_search">
...
<div class="ui_none" <?php if($hidden==0) echo 'style="display:none"';?> >
    <div>
    	<p><?php echo $tips;?></p>
        <div class="webkitbox" style="-webkit-box-align:baseline; margin-top:10px">
        <?php if($hidden==1){?>
            <?php if(!empty($list)){?>
            <div style="text-align:right; min-width:100px; padding-right:5px;">为您推荐：</div>
            <div style="-webkit-box-flex:1;box-flex:1;">
                <?php foreach ($list as $re) {?>
                    <a style="display:block; padding-bottom:2px; text-align:left" href="<?php echo Hotel_base::inst()->get_url('THEMATIC_INDEX').'&tc_id='.$re['id']?>" class="color_link"><?php echo $re['act_name'];?></a>
                <?php }?>
            <?php }else{?>
        	   <a style="text-align:center; min-width:100px; padding-right:5px;" href="<?php echo $index_url;?>" class="color_link">逛逛订房</a>
            <?php }?>
               </div>
        <?php }?>
        </div>
...
<script>
    ...
    $('.checkin_time >span').html(<?php echo $minSelect; ?>);
	function hidesearch(){
		$('.address_layer').hide();
		$('#index_search').show();
	}
	...
	function search_hotel(keyword){
		...
		$.get('<?php echo Hotel_base::inst()->get_url('AJAX_HOTEL_SEARCH');?>',{
			keyword:keyword,
            tc_id:tc_id,
			city:$('#city').val()
		},function(data){
    ...
    $(function(){
        ...
        $('.act_select').change(function(){
            window.location.href = '<?php echo Hotel_base::inst()->get_url('THEMATIC_INDEX').'&tc_id=';?>'+ $(this).val();
        })
    });
