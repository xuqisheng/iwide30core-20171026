   ...
   <?php if($bonus_setting==1){?>
	...
	<div class="input_item">
       ...
       <?php if (!empty($point_consum_set['max_use'])){ $point_max_use=$point_consum_set['max_use'];?>
    	placeholder="可用<?php echo round($point_consum_set['max_use']);?>积分抵扣<?php echo round($point_consum_set['max_use']*$point_consum_rate); ?>元"
    	<?php }else{ $point_max_use=$member->bonus;?>
        placeholder="可用<?php echo round($member->bonus);?>积分抵扣<?php echo round($member->bonus*$point_consum_rate); ?>元"
        <?php }?> /></div>
    </div>
...
<div class="pad3">
	...
	<div class="color_888 h22">
		<span id="retain-policy" style="<?php echo empty($bookpolicy_condition['retain_time'][$pay_ways[0]->pay_type])?'display: none;':''; ?>">&radic;房间保留至<span class="color_main"><?php echo !empty($bookpolicy_condition['retain_time'][$pay_ways[0]->pay_type])?$bookpolicy_condition['retain_time'][$pay_ways[0]->pay_type]:''; ?></span>点！</span>
		<span id="delay-policy" style="<?php echo empty($bookpolicy_condition['delay_time'][$pay_ways[0]->pay_type])?'display: none;':''; ?>">&radic;延迟退房时间至<span class="color_main"><?php echo !empty($bookpolicy_condition['delay_time'][$pay_ways[0]->pay_type])?$bookpolicy_condition['delay_time'][$pay_ways[0]->pay_type]:''; ?></span>点</span>
	</div>
...
<div style="padding-top:15%">
    <div class="foot_fixed">
    	<div class="color_minor" style="padding:0 8px;">
        	...
        	<span id="total_price" class="y h36"><?php echo $total_price;?></span>
        	<?php }else{?>
        	<span id="total_point" class="h36"><?php echo $total_point;?> 积分</span>
        	<span id="total_price" class="y h36" style="display: none"><?php echo $total_price;?></span>
        	<?php }?>
        	<?php if (!isset($total_point)){?>
	...
	if(has_point_pay==1){
		$.post('<?php echo Hotel_base::inst()->get_url("RETURN_POINTPAY_SET");?>',{
			...
			if(data.can_exchange==1){
				...
				$('#total_point').html(data.point_need+' 积分');
			}else{
