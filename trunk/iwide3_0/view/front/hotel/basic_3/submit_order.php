...
<div class="form_list">
	<div class="item pay_list">
        ...
        <ul><?php if(!empty($pay_ways)) foreach($pay_ways as $k=>$pw){?>
            <li pay_type='<?php echo $pw->pay_type;?>' abled='1' class="pay_way <?php if($k==0){?>ischeck<?php }?>"><em class="ui_ico ui_ico13"></em><span><?php echo $pw->pay_name;?><?php if($pw->pay_type=='balance') { ?>(<span class="ui_price"><?php echo $member->balance;?></span>)<?php }?></span></li>
            <?php }?>
...
<div style="padding-top:15%">
    <div class="footfixed">
    	<div class="total">合计 <span id="total_price" class="ui_price"><?php echo $total_price;?></span>
			...
			if(data.cards!=''){
				if(data.vid==undefined||data.vid==0){
					...
					$.each(data.cards,function(i,n){
						tmp_id= n.ci_id.split('-')[0];
						temp+='<li onclick="choose_coupon(this)"  class="';
						if (tmp_id=='10100') temp+='hide ';
						if (choose_type!=''&& choose_type != tmp_id) temp+='gray ';
						...
						temp+='" code='+n.code+' amount="'+n.reduce_cost+'" max_use_num="'+n.hotel_max_use_num+'" card_type="'+n.ci_id+'" use_num_type="'+n.hotel_use_num_type+'" choose_type="'+tmp_id+'"><div class="checkbox"><em class="ui_ico ui_ico13"></em></div><div class="ui_vote"><p class="bordertop_img"></p><div class="vote_con">';
					...
					$.each(data.cards,function(i,n){
						tmp_id= n.ci_id.split('-')[0];
						temp+='<li onclick="choose_coupon_v(this)"  class="';
						if (tmp_id=='10100') temp+='hide ';
						if (choose_type!=''&& choose_type != tmp_id) temp+='gray ';
						if(coupons[n.code]!=undefined){
							temp+='ischeck ';
						...
						temp+='" code='+n.code+' amount="'+n.reduce_cost+'" card_type="'+n.ci_id+'" choose_type="'+tmp_id+'"><div class="checkbox"><em class="ui_ico ui_ico13"></em></div><div class="ui_vote"><p class="bordertop_img"></p><div class="vote_con">';
						temp+='<p class="ui_price ui_color">'+n.reduce_cost+'元</p>';
						temp+='<p><b>'+n.title+'</b></p>';
						temp+='<p class="ui_gray">'+n.brand_name;
						if(n.is_wxcard==1)temp+='--已添加到卡包';
               ...
               temp+='<li class="ischeck" ><div class="ui_vote" style="width:90%"><p class="bordertop_img"></p><div class="vote_con"><p style="text-align:center; font-size:12px;">暂无可用优惠券哦</p></div></div></li>';
			}
			$('#votelist').html(temp);
			$('.page_loading').remove();
		},'json');
...
function choose_disable(obj){
	choose_type = $(obj).attr('choose_type');
	$('#votelist li').addClass('gray');
	$('#votelist li[choose_type="'+choose_type+'"]').removeClass('gray');
}
function reset_choose(){
	if($('#votelist .ischeck').length<=0){
		choose_type='';
		$('#votelist li').removeClass('gray');
	}
}
