...
.headers .hotelname{font-size:2.8em}
</style>
...
<header class="headers"> 
    ...
    <div class="hotelname" style="width:90%;">
        ...
        <img src="/public/hotel/basic_3/images/1.png" style="display:none">
    </div>
    <div class="tag_zp" style="display:none"><img src="/public/hotel/basic_3/images/2_1.png"></div>
    <div class="addlike <?php if(!empty($collect_id)){?>islike<?php }?>" mid="<?php echo $collect_id;?>"></div>
</header>

<script>
console.log('debug');
console.log('<?php json_encode($hotel["icons"]); ?>');
<?php if(!empty($hotel['icons'])){?>
var imgurl='';
<?php foreach ($hotel['icons'] as $icon){?>
	imgurl+="<?php echo $icon;?>";
<?php }?>
if(imgurl!=''){
	if(imgurl.indexOf('qf311446498956')>=0){$('.hotelname img').show();}
	if(imgurl.indexOf('qf311448139446')>=0){$('.tag_zp').show();}
}
<?php }?>
</script>
            ...
            <?php if(!empty($r['state_info'])){ foreach($r['state_info'] as $si){?>
            	<div class="pay_way">
                    ...
                    <div class="item_right">
                        ...
                        <div onClick="pay(this,event)" room_id="<?php echo $r['room_info']['room_id'];?>" price_code="<?php echo $si['price_code'];?>" price_type="<?php echo $si['price_type'];?>" <?php if (isset($si['condition']['pre_pay'])&&$si['condition']['pre_pay']==1) echo 'class="pre_pay"><p>预付</p>';else echo 'class="now_pay" ><p>预订</p>'; ?>
                        </div>
                        <?php } else{?>
...
$(function(){
	...
	$('.tag_zp').click(function(){
		$.MsgBox.Alert('专票','该店可提供增值税专用发票');
	})
