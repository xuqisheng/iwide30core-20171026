...
<div class="form_list">
   ...
   <?php if(!empty($member->mem_id)&&!empty($point_consum_rate)){?>
	<div class="item">
    	<span>悦银抵用</span>
        <input max="<?php echo $member->bonus;?>" type="tel" id='bonus' name='bonus' placeholder="悦银抵房价(可用<?php echo $member->bonus;?>分/<?php echo $member->bonus*$point_consum_rate; ?>元)" />
    </div>
    <?php } ?>
   <?php if(empty($first_state['condition']['no_coupon'])) {?>
	<div class="item ui_btn_block usevote" >
    	<span>优惠券</span>
...
<div class="form_list">
	<div class="item pay_list">
        ...
        <ul><?php if(!empty($pay_ways)) foreach($pay_ways as $k=>$pw){?>
            <li pay_type='<?php echo $pw->pay_type;?>' class="pay_way <?php if($k==0){?>ischeck<?php }?>"><em class="ui_ico ui_ico13"></em><span><?php echo $pw->pay_name;?><?php if($pw->pay_type=='balance') { ?>(<?php echo $member->balance;?>元)<?php }?></span></li>
            <?php }?>
        </ul>
    </div>
</div>
