...
<style>
    ...
    .kill-layer .container .title {
        text-align: center;
        color: #222222;
        font-weight: normal;
        margin-bottom: 20px;
    }
    .kill-layer .container .content {
        color: #222222;
        font-size: 12px;
    }
    .kill-layer .container .content.tip-content {
        text-align: center;
        margin-bottom: 30px;
    }
    .kill-layer .container .qrcode-container {
        text-align: center;
    }
    .kill-layer .container .qrcode-container .qrcode {
    ...
    .kill-layer .container .qrcode-container .qrcode img {
        width: 100%;
        max-width: 100%;
    }
    .kill-layer .container .tips {
        margin-top: 20px;
        text-align: center;
        color: #e5503c;
        font-size: 12px;
    }
    .kill-layer .container .btn-group {
        text-align: center;
        margin-top: 20px;
        line-height: 1;
    }
    .kill-layer .container .btn-group .btn {
        display: inline-block;
        border-radius: 2.5px;
        padding: 6px 30px;
    ...
    .kill-layer .container .btn-group .btn.active {
        color: #ffffff;
        background: #ff9900;
    }
...
<div class="whiteblock webkitbox justify spring_bg2" style="margin-top:0">
...
<?php else: ?>
    <div class="whiteblock" <?php if(!$spec_product): ?> style="display:none" <?php endif; ?>>
...
<?php /**有秒杀**/ if( !empty($killsec) && isset($killsec['is_stock']) && $killsec['is_stock']==Soma_base::STATUS_TRUE ): ?>
    <div class="whiteblock" id="ks_stock_div" style="display: none;">
        <div class="justify webkitbox">
            ...
            <div><span class="color_main">剩余库存：</span><span class="color_888 fill2">0<?php //echo $ks_stock; ?>/1<?php //echo $ks_count; ?></span></div>
        </div>
    </div>
    <script>
...
<script>
    ...
    function get_in_line(){
        ...
        $.ajax({
            ...
            error: function() {
                $.MsgBox.Confirm('哎呦喂，被挤爆了，请稍后重试！',winreload,winreload,'好的','取消');
                return false;
            },
...
<div class="foot_fixed">
    <div class="bg_fff webkitbox bd_top">
        <a href="<?php echo Soma_const_url::inst()->get_pacakge_home_page(array('id'=>$inter_id)); ?>" class="img_link"><img src="<?php echo get_cdn_url('public/soma/images/spring/'); ?>/icon_red4.png"/></a>
        <a href="<?php echo Soma_const_url::inst()->get_soma_ucenter(array('id'=>$inter_id)); ?>" class="img_link"><img src="<?php echo get_cdn_url('public/soma/images/spring'); ?>/icon_red5.png"/></a>

        <!-- <a class="h24 bg_main bdradius txtclip" style="border: 1px solid transparent" href=""><?php echo $show_name;  ?>购买</a> -->
        <?php if( $is_expire ): ?>
            <div class="h24 bdradius bg_999" style="border: 1px solid transparent">已过期</div>
        <?php else: ?>
            ...
            <?php if( isset( $isTicket ) && !$isTicket ):?>
                ...
                <div class="h24 bdradius btn_void txtclip select_type" href="<?php echo Soma_const_url::inst ()->get_package_pay(array('pid'=>$_GET['pid'],'id'=>$inter_id));?>">
                    ¥<?php echo $package['price_package'];?><?php
            ...
            <?php else:?>
                <a class="h24 bdradius btn_void txtclip" href="<?php echo Soma_const_url::inst()->get_url('*/*/ticket_select_time',array('id'=>$inter_id,'pid'=>$_GET['pid'],'tkid'=>$ticketId,'bType'=>$bType)); ?>">
                    <?php if($show_y_flag):?>¥<?php endif; ?><span><?php echo $package['price_package'];?></span>
                    <?php
                    if($package['type'] == $packageModel::PRODUCT_TYPE_BALANCE) {
        ...
        <?php /**有秒杀**/elseif( isset($killsec) && !empty($killsec)): ?>
            <a class="h24 bg_main bdradius txtclip" style="border: 1px solid transparent" onclick='get_in_line();'>¥<?php echo $killsec['killsec_price'];?><?php
                if($package['type'] == $packageModel::PRODUCT_TYPE_BALANCE) {
                    echo $show_name,'购买';
                } elseif($package['type'] == $packageModel::PRODUCT_TYPE_POINT) {
                    echo "积分购买";
...
<div class="kill-layer" id="qrcode" style="display:none">
    <div class="container">
        <p class="content f24">您已订阅成功，我们将在活动开始前10分钟内通知您，为保证能收到提醒信息，请扫码关注公众号</p>
...
<style>
    body,html{background:#f3f4f8 !important}
</style>
</body>
<script>
