...
<?php if( $multi_hotel || $multi_city ): ?>
    <?php if( $is_show_navigation == Soma_base::STATUS_TRUE ):?>
        <div class="tab_menus bg_fff border_bottom pad3">
            <a href="<?php echo $url;?>">
                <div class="menu_item cur webkitbox">
                	<em class="iconfont">&#xe622;</em>
                    <span>热销商品<br><tt class="h20">十万人的选择</tt></span>
                </div>
            </a>
            <div class="menu_item webkitbox bd_left" id="nearby" is_true="">
            	<em class="iconfont">&#xe623;</em>
                <span>在附近<br><tt class="h20">好东西在附近</tt></span>
            </div>
            <div class="s_select webkitbox bd_left">
            	<em class="iconfont">&#xe606;</em>
                <span>定位<br><tt class="h20">身边有好礼</tt></span>
            </div>
        </div>
    <?php endif; ?>
<?php endif; ?>
    ...
    <div class="tp_list" id="tp_list">
    	...
    	<a href="<?php echo Soma_const_url::inst()->get_package_detail(array('pid'=>$v['product_id'],'id'=>$inter_id) );?>" class="item bg_fff">
            <div class="img">
                <img src="<?php echo get_cdn_url('public/soma/images/default.jpg'); ?>" data-original="<?php echo $v['face_img'];?>" class="lazy"/>
                ...
                <?php if($v['type'] == $packageModel::PRODUCT_TYPE_BALANCE): ?> 
                    <div class="j_label color_main h24">会员</div>
                <?php endif; ?>
...
<script> 
    ...
    function fillProductContent(data,lat,lng){
        ...
        for(var item in data){
			...
			var txt1 = '';
			var txt2 = '<?php if($v['can_gift']== $packageModel::CAN_T):
					    ?><span class="btn_void xs color_888">可赠好友</span><?php 
                    endif;
                    if($v['can_reserve']== $packageModel::CAN_F):
                        ?><span class="btn_void xs color_888">不需预约</span><?php
                    endif;
                        ?>';
			var txt3 ='';
            if(data[item].killsec != undefined){ 
				txt1 = '秒杀';
				txt3 = data[item].killsec.killsec_price ;
            }else if(data[item].groupon != undefined){
				txt1 = '拼团';
				txt3 = data[item].groupon.group_price +'|'+data[item].groupon.group_count +'人团';
            }else if(data[item].auto_rule != undefined){
				txt1 = '满减';
				txt3 = data[item].price_package;
            }else if(false){
				txt1 = '会员';
				txt3 = data[item].price_package ;
			}else{
            ...
            '<div class="j_label color_main h24">'+txt1+'</div>'+
			'</div>' +
			'<p>' + data[item].name + '</p>' +
			'<div class="item_foot webkitbox justify pad3">'+
			'<div class="h20">'+txt2+'</div>'+
			'<div class="color_main h34">';
				...
				success: function(data){
					...
					if(data!=undefined&&data.status==1){
						...
						for(var n in  data.data){
							...
							str +='</div><p>'+data.data[n].name+'</p>';
							str +='<div class="item_foot webkitbox justify pad3"><div class="h20">';
							if(data.data[n].can_gift=='<?php echo $packageModel::CAN_T ?>')
							str +='<span class="btn_void xs color_888">可赠好友</span>';
							if(data.data[n].can_reserve=='<?php echo $packageModel::CAN_F ?>')
							str +='<span class="btn_void xs color_888">不需预约</span>';
							str +='</div><div class="color_main h34">';
                            if(show_y_flag)
                            {
                                str +='<tt class="h20 y">';
                            }
                            ...
                            str +='</tt>';
							if(data.data[n].killsec!=undefined)//有秒杀 
							str += data.data[n].killsec.killsec_price;
							else if(data.data[n].groupon!=undefined)//有拼团 
							str += data.data[n].groupon.group_price+' | '+n.groupon.group_count+'人团';
							else
							str +=data.data[n].price_package;
							str +='</div></div></a>';
						}
						$('#tp_list').append(str);
						pageIndex++;
