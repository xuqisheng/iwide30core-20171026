...
<script>
    ...
    <div id="goodsDetail">
        <?php if(!empty($effective_rule) && !empty($saler_info) && !in_array($this->inter_id, $idistributInterId)):?>
            <?php
                 $fanName = '亲爱的用户：';
                 if($fan['nickname']){
                    $fanName = '亲爱的'.$fan['nickname'].'：';
                 }
                 if($saler_info['saler_name']){
        ...
        <div class="tips box">
            ...
            <?php if($effective_rule['reward_type'] == Reward_rule_model::REWARD_TYPE_PERCENT): ?>
                订单
            <?php endif; ?>
            <span class="haoma"><?php echo $effective_rule['reward_rate']; ?></span>
            <?php if($effective_rule['reward_type'] == Reward_rule_model::REWARD_TYPE_PERCENT): ?>
                <span class="percent">%</span><?php else: ?>元
            <?php endif; ?>
                红包奖励，
            <?php if($saler_info['saler_type'] == 'FANS'): ?>
                隔天发至您的微信钱包
                <?php else: ?>由酒店发放
            <?php endif; ?>
            </div>
    ...
    <div class="detail">
        ...
        <div class="number box f24">
            <?php if($package['show_sales_cnt'] == 1):?>
        ...
        <div class="service box">
            ...
            <?php $hasOne = false; $count = 0; $limit = 4;?>
            ...
            <?php if($refund && $package['can_refund'] != $packageModel::CAN_REFUND_STATUS_FAIL && $package['type'] != $packageModel::PRODUCT_TYPE_BALANCE && $package['type'] != $packageModel::PRODUCT_TYPE_POINT): ?>
                <?php $hasOne = true; $count +=1 ;?>
                <div class="item flex1" style="display: <?php echo ($count > $limit ? 'none' : 'block') ?>">
    ...
    <footer>
        <div class="clearfix">
            <div class="cell fl box">
                <a href="<?php echo Soma_const_url::inst()->get_url('*/*/index',array('id'=>$inter_id,'openid'=>$this->openid, 'tkid' => $ticketId)); ?>" class="flex1">
            ...
            <input type="hidden" name="select_type" value="<?php echo Soma_const_url::inst ()->get_package_pay(array('pid'=>$_GET['pid'],'id'=>$inter_id,'bType'=>$bType));?>" />
            <?php if($killsec['killsec_time'] < date('Y-m-d H:i:s', time())){ ?>
                <a id="killsec_btn" href="javascript:void(0)" class="fl ta-c f32 btn"><?php echo '立即秒杀'; ?></a>
                <script>
                    startFlag = true;
                </script>
            <?php }
    ...
    function countDown () {
        ...
        if(showDay == undefined && showHour == undefined && showMin == '00' && startSec == '00'){
            ...
            <?php if(isset($killsec) && !empty($killsec)):?>
                ...
                $('#killsec_btn').html("<?php echo '立即秒杀';?>");
                startFlag = true;
                history.go(0);
            <?php endif;?>
        }
