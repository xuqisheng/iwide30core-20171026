...
<div class="box">
    ...
    <div class="selsct">
        ...
        <span>
            <input type="text" placeholder="请输入短信验证码" style="width:9em;" id="verifyMsg"/>
            <tt id="sent_code">发送验证码</tt>
        </span>
    </div>
    <div class="selsct">
...
<script>
    ...
    function submit(){
        ...
        var verifyMsg = $('#verifyMsg').val();
        var department= $('#department').val();
        var rcode     = $('#rcode').val();
        if(name == undefined  || name == ''){
            $.MsgBox.Alert('请输入姓名');return false;
        }
        if(idnum == undefined  || !testval.identity.test(idnum)){
            $.MsgBox.Alert('请输入正确身份证');return false;
        }
        if(cellphone == undefined ||!testval._phon.test(cellphone)){
            $.MsgBox.Alert('请输入正确手机号码');return false;
        }
        if(verifyMsg == undefined){
            $.MsgBox.Alert('请输入短信验证码');return false;
        }
        if(department == undefined || department == ''){
            $.MsgBox.Alert('请输入部门');return false;
        }
        if($('#hotel').val() == ''){
            $.MsgBox.Alert('请选择酒店');return false;
        }
        if(sub){
            sub = false;
            $.post(
    ...
    var cutdown = function(){
        _sent = true;
        $('#sent_code').html(time+'s').addClass('disable');
        _time = window.setInterval(function(){
            time--;
            $('#sent_code').html(time+'s');
            if(time<=0){
                window.clearInterval(_time);
                $('#sent_code').html('重新发送').removeClass('disable');
                _sent =false;
                time = 60;
            }
        },1000);
    ...
    $('#sent_code').click(function(){
        ...
        $.ajax({
            type: 'POST',
            dataType:'Json',
            url: '<?php echo site_url('distribute/distribute/send_verify_sms')?>',
            data: {
                '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
