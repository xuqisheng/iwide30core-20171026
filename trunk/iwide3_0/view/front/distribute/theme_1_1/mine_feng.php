...
<script src="<?php echo base_url('public/distribute/theme_1_1/scripts/toShow.js')?>"></script> 
<title>全员营销</title>
<style>
...
<div class="ui_btn_list ui_border">
<?php $inter_id_default_array = array('a466585224');
    ...
    <?php $inter_id_taopiao_array = array('a468914243','a426755343','a465353793','a469617014','a475115706','a466585224','a476935671','a466585224','a474876699','a459234737','a479437128','a472114616','a476843906','a478659714');
...
if($inter_id_in_taopiao) :?>
    ...
    <?php endif;?>
    <?php $inter_id_haibao_array = array('a452223043','a445223616','a449675133','a450939254','a457946152','a463105262','a464177542','a455510007','a468209719');
	...
	function cutdown(str,_i){
		...
		if(_i==undefined){
			...
			_time=window.setInterval(function(){
				...
				$(tmp_this+' tt').html('正在'+str+'二维码海报,请稍候('+_count+'s)');
				if( isFinish()  && bgimg.complete){
					if (!isdown) creat(bgimg,erweima,'.pullhaibao'<?php if($inter_id == 'a457946152'|| $inter_id=='a468209719' || $inter_id  == 'a464177542'){echo ',true';}?>);
				}
				if (_count>180){
					_count=0;
					isdown=false;
...
<?php if(count($top_msg) > 0 && !empty($top_msg[0])): ?>
var popMsg = <?php echo json_encode($top_msg)?>;
function initMsgWin(){
	$('.flex .fle_btn').unbind('click');
    if(popMsg.length > 0){
		$('.flex .fle_btn').html('下一条');
		$('.flex .fle_btn').on('click',function(){
			var msg = popMsg.pop();
			$('.flex .title').html(msg.title);
			$('.flex .fle_con').html(msg.title);
			$('.flex .fle_href').attr('href','<?php echo site_url('distribute/dis_v1/msg_det')?>?id=<?php echo $inter_id?>&mid='+msg.nid);
			initMsgWin();
		});
    }else{
    	$('.flex .fle_btn').html('我知道了');
		$('.flex .fle_btn').on('click',function(){
			$('.flex').fadeOut();
			$('.flex').remove();
		});
...
window.onload=function(){
	var msg = popMsg.pop();
    toShows(msg.title, msg.sub_title,'<?php echo site_url('distribute/dis_v1/msg_det')?>?id=<?php echo $inter_id?>&mid='+msg.nid);
    initMsgWin();
}
<?php endif;?>
</script>