			...
			<section class="content">
              ...
              <!-- 
                ...
                <style>
                	#search_form .form-group{margin:.3em auto;}
							...
							<div class="box-body">
								<div class="panel panel-default">
								  <div class="panel-heading">新增<?php echo $this->uri->segment(3) == 'jfk_admins' ? '金房卡' : '酒店'?>授权</div>
					...
					<div class="panel panel-default">
  <div class="panel-heading"><?php echo $this->uri->segment(3) == 'jfk_admins' ? '金房卡' : '酒店'?>授权列表</div>
  <div class="panel-body">
...
$(document).ready(function() {
		...
		$.post($('#search_form').attr('action'),$('#search_form').serialize(),function(data){
			...
			if(data.count > 0){
				$.each(data.res,function(k,v){
					...
					tmpStr += '<td><a class="btn btn-default btn-sm" role="auth" rid="' + v.qrcode_id + '"><?php echo $this->uri->segment(3) == 'jfk_admins' ? '金房卡' : '酒店'?>授权</a></td>';
					tmpStr += '</tr>';
				});
			}else{
			...
			$('a[role=auth]').on('click',function(){
				...
				if(confirm('确认授权分销号为 ' + tds[0].innerText + ' 的 ' + tds[1].innerText + ' 发放福利吗?')){
					$.post('<?php echo site_url("distribute/welfare/new_auth")?>',{'admin':_this.attr('rid'),'<?php echo $this->security->get_csrf_token_name();?>':'<?php echo $this->security->get_csrf_hash();?>','typ':<?php echo $this->uri->segment(3) == 'jfk_admins' ? 2 : 1;?>},function(data){
						alert(data.errmsg);
						location.reload();
					},'json');
				}
	...
	$('a[name=status]').on('click',function(e){
		...
		if(confirm('确认修改分销号为 ' + tds[0].innerText + ' 的 ' + tds[1].innerText + ' 授权状态吗?')){
			$.post('<?php echo site_url("distribute/welfare/schange")?>',{'admin':_this.attr('sid'),'status':_this.attr('rs'),'<?php echo $this->security->get_csrf_token_name();?>':'<?php echo $this->security->get_csrf_hash();?>','typ':<?php echo $this->uri->segment(3) == 'jfk_admins' ? 2 : 1;?>},function(data){
				alert(data.errmsg);
				if(data.errcode == 'ok'){
					location.reload();
				}
