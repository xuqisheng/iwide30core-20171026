...
<link rel="stylesheet" href="<?php echo base_url(FD_PUBLIC). '/'. $tpl ?>/plugins/iCheck/all.css">
              ...
              <!-- 
                ...
                <div class="box-body">
                	<form action="">
						...
						<div class="form-group">
							<input type="radio" class="minimal" name="s_type" value="1"<?php if (isset($sub_config['typ']) && $sub_config['typ'] == 1):?> checked<?php endif;?> />&nbsp;关闭互动增粉
							<input type="radio" class="minimal" name="s_type" value="2"<?php if (isset($sub_config['typ']) && $sub_config['typ'] == 2):?> checked<?php endif;?> />&nbsp;开启互动增粉
						</div>
						<div class="form-group">
				...
				<div class="box-body">
					...
					<div class="form-group">
						...
						<div class="form-group">
							...
							<input type="radio" class="minimal" name="protection_status" value="CLOSED"  <?php if (isset($protection_config->status) && $protection_config->status == 'CLOSED'):?> checked<?php endif;?> />&nbsp;不启用
							<input type="radio" class="minimal" name="protection_status" value="OPEN" <?php if (isset($protection_config->status) && $protection_config->status == 'OPEN'):?> checked<?php endif;?> />&nbsp;启用
						</div>
						
						<div class="form-group">
						<a href="javascript:;" onclick="save_protection()" class="btn btn-primary">保存</a>
						</div>
					</fieldset>
				</div></div>
				  <div class="box">
				<div class="box-body">
					...
					<div class="form-group">
							...
							<select id="distribute_time" name='distribute_time' class="form-control" style="display:inline;width:inherit">
								<?php for($i=1;$i<=12;$i++){?>
						...
						<div class="form-group">
							...
							<input type="radio" class="minimal" name="distribute_status" value="0"  <?php if (isset($distribute_config_data['distribute_status']) && $distribute_config_data['distribute_status'] == 0):?> checked<?php endif;?> />&nbsp;不启用
							<input type="radio" class="minimal" name="distribute_status" value="1" <?php if (isset($distribute_config_data['distribute_status']) && $distribute_config_data['distribute_status'] == 1):?> checked<?php endif;?> />&nbsp;启用
						</div>
              ...
              <div class="box">
				<div class="box-body">
					<fieldset>
						<div class="form-group">
							<label>商城激励类型</label>
							<input type="radio" class="minimal" name="m_type" value="0"<?php if (isset($mall_config['type']) && $mall_config['type'] == 0):?> checked<?php endif;?> />&nbsp;订单金额百分比
							<input type="radio" class="minimal" name="m_type" value="1"<?php if (isset($mall_config['type']) && $mall_config['type'] == 1):?> checked<?php endif;?> />&nbsp;每个订单激励固定金额
						</div>
						<div class="form-group">
							<label>商城激励金额</label>
...
function save_protection(){
	if($('#protection_val').val() == '' && $('input[name="protection_status"]:checked').val() == 'OPEN') return alert('请输入有效时间');
	$.post("<?php echo site_url('distribute/distribute/save_protection_config')?>",{'<?php echo $this->security->get_csrf_token_name();?>':'<?php echo $this->security->get_csrf_hash();?>','time':$('#protection_val').val(),'status':$('input[name=protection_status]:checked').val()},function(datas){
		if(datas.errmsg == 'ok'){
			alert('保存成功');
		}else{
			alert('保存失败');
