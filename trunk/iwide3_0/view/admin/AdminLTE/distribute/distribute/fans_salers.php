          ...
          <div class="row">
            <div class="col-xs-12">
              ...
              <!-- 
                ...
                <div class="row">
                	<div class="col-sm-12">
                	<?php echo form_open('distribute/dis_ext/salers',array('class'=>'form-inline'))?>
                	...
                	<label>激活日期</label><input type="text" name="begin_time" class="form-control input-sm form_datetime" placeholder="" aria-controls="data-grid" value="<?php if(!empty($posts['begin_time'])):echo $posts['begin_time'];endif;?>">
                	<label>至</label><input type="text" name="end_time" class="form-control input-sm form_datetime" placeholder="" aria-controls="data-grid" value="<?php if(!empty($posts['end_time'])):echo $posts['end_time'];endif;?>">
                  ...
                  <table id="data-grid" class="table table-bordered table-striped table-condensed">
                    <thead>
                    	<tr role="row">
                    		...
                    		<th class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="昵称: activate to sort column ascending">昵称</th>
                    		<th class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="关注时间: activate to sort column ascending">关注时间</th>
                    		<th class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="激活时间: activate to sort column ascending">激活时间</th>
                    		<th class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="已核定绩效笔数: activate to sort column ascending">已核定绩效笔数</th>
                    		<th class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="已核定绩效金额: activate to sort column ascending">已核定绩效金额</th>
                    <tfoot></tfoot>
                    <tbody><?php foreach ($res as $saler):?>
                    	<tr>
                    		<td><?=$saler['fans_key']?></td>
                    		<td><?=$saler['nickname']?></td>
                    		<td><?=$saler['subscribe_time']?></td>
                    		<td><?=$saler['actv_time']?></td>
                    		<td><?php echo empty($grades[$saler['fans_key']]) ? 0 : $grades[$saler['fans_key']]->counts?></td>
                    		<td><?php echo empty($grades[$saler['fans_key']]) ? 0 : $grades[$saler['fans_key']]->grade_totals?></td>
                    	</tr><?php endforeach;?>
                    </tbody>
                  </table>
                  <div class="row">
