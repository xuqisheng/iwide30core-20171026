		...
		<div class="box-body">	
		  ...
		  <table id="data-grid" class="table table-bordered table-striped table-condensed dataTable" role="grid" aria-describedby="data-grid_info">
			<thead>
			  <tr role="row" class="odd" style="background-color:#f9f9f9;">
				...
				<div style="text-indent:1rem">今日实时（最新更新时间<?php echo date('Y-m-d H:i:s');?>）<span style="float:right;cursor:pointer;color:#0000FF" onClick="location.href=''"><!--历史数据--></span></div>
				</th>
			  </tr>
			</thead>
			<tbody>
			  <tr role="row" style="background:#FFF">
				...
				<td><?php echo $all_saler[0]['count'];?>
				<br>人员总数
				</td>
				<td><?php echo $all_fans_log[0]['count'];?>
				<br>粉丝总数
				</td>
				<td><?php echo $today_fans_log[0]['count'];?>
				<br>今日新增
				</td>
				<td><?php echo $today_fans_log_2[0]['count'];?>
				<br>关注状态
				</td>
				<td><?php echo $today_fans_log_1[0]['count'];?>
				<br>取消关注
				</td>
			  </tr>
			</tbody>
		  ...
		  <table id="data-grid1" class="table table-bordered table-striped table-condensed dataTable" role="grid" aria-describedby="data-grid_info">
			<thead>
			  <tr role="row" class="odd" style="background-color:#f9f9f9;">
				...
				<div style="text-indent:1rem">数据图表</div>
				</th>
			  </tr>
			</thead>
			<tbody>
		  ...
		  <table id="data-grid1" class="table table-bordered table-striped table-condensed dataTable" role="grid" aria-describedby="data-grid_info">
			<thead>
			  <tr role="row" class="odd" style="background-color:#f9f9f9;">
				<th width="5%" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="">
				<div style="text-indent:1rem">分销员性别</div>
				</th>
			  </tr>
			</thead>
			<tbody>
			  <tr role="row" style="background:#FFF">
				<td>
				<div id="sex_area" style="height:100%;width:100%"></div>
				</td>
			  </tr>
			</tbody>
		  </table>
		  ...
		  <table id="data-grid1" class="table table-bordered table-striped table-condensed dataTable" role="grid" aria-describedby="data-grid_info">
			<thead>
			  <tr role="row" class="odd" style="background-color:#f9f9f9;">
				<th width="5%" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="">
				<div style="text-indent:1rem">成交比例</div>
				</th>
			  </tr>
			</thead>
			<tbody>
			  <tr role="row" style="background:#FFF">
				<td>
				<div id="sex_order" style="height:100%;width:100%"></div>
				</td>
			  </tr>
			</tbody>
		  </table>`
		  ...
		  <br>
		  <table id="data-grid2" class="table table-bordered table-striped table-condensed dataTable" role="grid" aria-describedby="data-grid_info" style="display:none">
			<thead>
			  <tr role="row" class="odd" style="background-color:#f9f9f9;">
				<th width="5%" colspan="9" aria-controls="data-grid" rowspan="1" aria-label="">
				<div style="text-indent:1rem">热销商品排行（按成交订单量）<span style="float:right;cursor:pointer;color:#0000FF" onClick="location.href='old';"></span></div>
				</th>
			  </tr>
			</thead>
			<tbody>
			  ...
			  <tr role="row" style="background:#FFF">
				...
				<td>
				<?php echo $v['all_grade_amount'];?>
				</td>
				<td>
				<?php 
				if($v['all_grade_amount']>$v['all_order_amount']){
					echo intval($v['all_order_amount']*10000/$v['all_grade_amount'])/100;
				}
				else {
					echo intval($v['all_grade_amount']*10000/$v['all_order_amount'])/100;
				} 
				?> %
				</td>
				<td>
		  ...
		  <table id="data-grid2" class="table table-bordered table-striped table-condensed dataTable" role="grid" aria-describedby="data-grid_info" style="display:none">
			...
			<tbody>
			  ...
			  <tr role="row" style="background:#FFF">
				...
				<?php 
				if($v['all_grade_amount']>$v['all_order_amount']){
					echo intval($v['all_order_amount']*100/$v['all_grade_amount']);
				}
				else {
					echo intval($v['all_grade_amount']*100/$v['all_order_amount']);
				} 
				?> %
				</td>
				<td>
...
require(
	...
	function (ec) {
		// 基于准备好的dom，初始化echarts图表
		var myChart = ec.init(document.getElementById('sex_area')); 		
		var option = {
			title : {
				text: '分销员性别',
				subtext: 'iwide',
			...
			series : [
				{
					...
					radius : '55%',
					center: ['50%', '60%'],
					data:[{"value":"<?php echo $staff_order_sex[1][0]['count'];?>","name":"男"},{"value":"<?php echo $staff_order_sex[2][0]['count'];?>","name":"女"}]				}
			]
		};					   
		myChart.setOption(option); 
	}
...
require(
	...
	function (ec) {
		// 基于准备好的dom，初始化echarts图表
		var myChart = ec.init(document.getElementById('sex_order')); 		
		var option = {
			title : {
				text: '成交比例',
				subtext: 'iwide',
			...
			series : [
				{
					...
					radius : '55%',
					center: ['50%', '60%'],
					data:[{"value":"<?php echo $fans_log_sex[1][0]['count'];?>","name":"男"},{"value":"<?php echo $fans_log_sex[1][0]['count'];?>","name":"女"}]				}
			]
		};					   
		myChart.setOption(option); 
	}
