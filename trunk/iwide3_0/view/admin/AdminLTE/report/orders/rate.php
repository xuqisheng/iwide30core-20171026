		  ...
		  <table id="data-grid" class="table table-bordered table-striped table-condensed dataTable" role="grid" aria-describedby="data-grid_info">
			<thead>
			  <tr role="row" class="odd" style="background-color:#f9f9f9;">
				<th width="5%" colspan="8" aria-controls="data-grid" rowspan="1" aria-label="">
			...
			<tbody>
			  <tr role="row" style="background:#FFF">
				<td onClick="location.href='/index.php/report/financeexp/index'">
				<?php echo $order_count_all['count'];?>
				<br>订单总数
				</td>
				<td onClick="location.href='/index.php/report/financeexp/index'"><?php echo $order_count_notcheck['count'];?>
				<br>未审订单
				</td>
				<td onClick="location.href='/index.php/report/financeexp/index'"><?php echo $order_count_check['count'];?>
				<br>已审订单
				</td>
				<td onClick="location.href='/index.php/report/financeexp/index'"><?php echo $order_count_canel['count'];?>
				<br>取消订单
				</td>
				<td onClick="location.href='/index.php/report/financeexp/index'"><?php echo $order_count_else['count'];?>
				<br>异常订单
				</td>
				<td><?php if($order_count_all['count']>0){echo intval((($order_count_check['count']+$order_count_notcheck['count'])/$order_count_all['count'])*100);} else {echo '0';} ?> %
				<br>预订率
				</td>
		  ...
		  <table id="data-grid2" class="table table-bordered table-striped table-condensed dataTable" role="grid" aria-describedby="data-grid_info">
			<thead>
			  <tr role="row" class="odd" style="background-color:#f9f9f9;">
				...
				<div style="text-indent:1rem">分店列表（最新更新时间<?php echo date('Y-m-d H:i:s');?>）<span style="float:right;cursor:pointer;color:#0000FF" onClick="location.href='old';"></span></div>
				</th>
			  </tr>
			</thead>
			<tbody>
			  ...
			  <?php foreach($order_hotels as $v){ ?>
			  <tr role="row" style="background:#FFF">
				<td>
				<a href="/index.php/report/orders/rate?entity_id=<?php echo $v['hotel_id'];?>"><?php echo $v['name'];?></a>				
				</td>
				<td>
				<?php echo $v['hotel_id_data']['order_count_all']['count'];?>
				</td>
				<td>
				<?php if($v['hotel_id_data']['order_count_roomnums']['sum']){echo $v['hotel_id_data']['order_count_roomnums']['sum'];} else {echo '0';} ?>
				</td>
				<td>
				<?php echo $v['hotel_id_data']['order_count_notcheck']['count'];?>
				</td>
				<td>
				<?php echo $v['hotel_id_data']['order_count_check']['count'];?>
				</td>
				<td>
				<?php echo $v['hotel_id_data']['order_count_canel']['count'];?>
				</td>
				<td>
				<a href="/index.php/report/orders/hotel?select_hotel=<?php echo $v['hotel_id'];?>"><?php 
				if($v['hotel_id_data']['order_count_all']['count']>0){
					echo intval(($v['hotel_id_data']['order_count_notcheck']['count']+$v['hotel_id_data']['order_count_check']['count'])*100/$v['hotel_id_data']['order_count_all']['count']);
				}
				else {echo '0';}
				?>
				%</a>
				...
				<td>
				<?php echo $v['hotel_id_data']['order_sum_paid']['sum']?$v['hotel_id_data']['order_sum_paid']['sum']:0;?>
				</td>
				<td>
				<?php echo $v['hotel_id_data']['order_sum_all']['sum']?$v['hotel_id_data']['order_sum_all']['sum']:0;?>
				</td>
			  </tr>
			  <?php } ?>
			</tbody>
