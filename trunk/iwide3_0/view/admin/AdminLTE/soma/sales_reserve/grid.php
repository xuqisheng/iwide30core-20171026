...
//print_r($fields_config);die;
      ...
      foreach ($fields_config as $k=> $v): 
         ...
         if(in_array($k, array('qty','grand_total','comfirmed_status','reviewed_status'))) {
             $search_label= '';
         } else {
...
// $buttions.= '<button type="button" class="btn btn-default btn-sm" id="grid-btn-extra-0"><i class="fa fa-hand-pointer-o"></i>&nbsp;确认</button>';
// $buttions.= '<button type="button" class="btn btn-default btn-sm" id="grid-btn-extra-1"><i class="fa fa-hand-pointer-o"></i>&nbsp;审核</button>';

$buttions.= '<button type="button" class="btn btn-default btn-sm disabled" id="grid-btn-edit"><i class="fa fa-list-alt"></i>&nbsp;订单明细</button>';
...
var url_extra= [
//'http://iwide.cn/',
// '<?php echo EA_const_url::inst()->get_url("*/*/edit"); ?>' + '?op=comfirm',
// '<?php echo EA_const_url::inst()->get_url("*/*/edit"); ?>' + '?op=review',
];
...
if( count($result['data'])<$num){
    ...
    $gridjs= VIEWPATH. $tpl .DS . $m. DS. 'sales_reserve/gridjs.php';
    ...
    $gridjs= VIEWPATH. $tpl .DS . $m. DS. 'sales_reserve/gridjs_ajax.php';
    if( file_exists($gridjs) )
        require_once $gridjs;
    else 
