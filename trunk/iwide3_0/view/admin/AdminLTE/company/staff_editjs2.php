...
$('#data-grid tbody').on('click', 'tr', function(){
	...
	if(selected.length==1){
        ...
        $('#grid-btn-pass').addClass('disabled').unbind();	//先清除原有绑定事件
        $('#grid-btn-pass').removeClass('disabled').bind('click', selected, function(ev){
        if(confirm('所选全部审核通过吗？') ){
        window.location= url_delete+ '?ids='+ ev.data;
        }
        });
		$('#grid-btn-edit').addClass('bg-green');
		$('#grid-btn-del').addClass('bg-red');
        $('#grid-btn-pass').addClass('bg-red');
        ...
        $('#grid-btn-pass').attr('title', '批量').attr('data-placement','top').tooltip('show');
        ...
        $('#grid-btn-pass').addClass('disabled').removeClass('bg-red').unbind();
        $('#grid-btn-pass').tooltip('hide');
		$('button.single-btn').each(function(i,e){$(this).addClass('disabled').removeClass('bg-green');});
	}
});
