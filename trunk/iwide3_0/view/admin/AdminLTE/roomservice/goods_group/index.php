...
<div style="color:#92a0ae;">
    <div class="over_x">
        <div class="content-wrapper" style="min-width:1130px;" >
            <div class="banner bg_fff p_0_20">分组列表</div>
            <div class="contents">
                <div class="hottel_name ">
                        <?php echo $this->session->show_put_msg(); ?>
                    <div class="input_txt">
                        <form id="" action="<?php echo site_url('roomservice/goods_group/index')?>" class="form-inline" method="get">
                        关键词：<input type="text" name="wd" class="form-control input-sm" aria-controls="data-grid" value="<?php echo empty($posts['wd']) ? '' : $posts['wd']?>">
                        <input type="submit" class="btn " value="检索"/>&nbsp;&nbsp;&nbsp;
                         <a href="<?php echo site_url('roomservice/goods_group/add')?>" class="btn btn-sm bg-green">新增分组</a>
                        </form>
                    </div>
                </div>
                    ...
                    <table id="coupons_table" class="table-striped table-condensed dataTable no-footer" style="width:100%;">
                       ...
                       ?>
                          <tr class=" form_con">
                            ...
                            <td><a  class="color_72afd2" href="<?php echo site_url('roomservice/goods_group/edit?ids='.$v['group_id'])?>">编辑</a>
                            <?php if($v['is_delete']==0){?>
                               <span class="color_72afd2" style="cursor:pointer" onclick="del_group('<?php echo $v['group_id']?>','<?php echo $v['shop_id']?>')">删除</span>
                               <?php }?>
                            </td>
                            </tr>
                            <?php }}?>
...
// 	  modal.find('.modal-body input').val(recipient)
    ...
    function del_group(group_id,shop_id){
        if(!confirm("确定?")){
            return false;
        }
        $.post('<?php echo site_url('roomservice/goods_group/del_group')?>',{
            'group_id':group_id,
            'shop_id':shop_id,
    ...
    $(document).ready(function() {
        ...
        $('#output').click(function(){
            ...
            //url = "<?php echo site_url('roomservice/qrcodelist/output_qrcode')?>"+"?s="+tmp;
            //alert(url);return false;
            location.href="<?php echo site_url('roomservice/qrcodelist/output_qrcode')?>"+"?s="+tmp;
        });
    });
</script>
</body>
