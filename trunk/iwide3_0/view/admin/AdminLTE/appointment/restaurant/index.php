        ...
        <div class="content-wrapper" style="min-width:1130px;" >
            ...
            <div class="contents">
                <div class="hottel_name ">
                    ...
                    <div class="input_txt">
                        <form id="" action="<?php echo site_url('appointment/restaurant/index');?>" class="form-inline" method="get">
                        关键词：<input type="text" name="wd" class="form-control input-sm" placeholder="店铺名" aria-controls="data-grid" value="<?php echo empty($param['wd']) ? '' : $param['wd']?>">
                        <input type="submit" class="btn bg_ff9900 color_fff" value="检索"/>&nbsp;&nbsp;&nbsp;
                         <a href="<?php echo site_url('appointment/restaurant/add')?>" class="btn btn-sm bg-green">新增店铺</a>
                        </form>
                    </div>
                </div>
                        ...
                        <thead class="bg_f8f9fb form_thead">
                            ...
                            <?php
                             ...
                             <tr class=" form_con">
                                ...
                                <td><?php echo $value['take_num'] ? $value['take_num'] : '--';?></td>
                                <td><?php echo $value['offer_num'] ? $value['offer_num'] : '--' ;?></td>
                                <td>
                                    <?php
                                        if ($value['book_style'] == 1 || $value['book_style'] == 2) {
                                            ?>
                                            <span class="red pointer" onClick="change('<?php echo $value['dining_room_id'];?>','off','1')">停止取号</span>

                                            <?php
                                             }else {
                                            ?>
                                            <span class="blue pointer" onClick="change('<?php echo $value['dining_room_id'];?>','on','1')">开始取号</span>
                                ...
                                <td>
                                <?php
                                    if ($value['book_style'] == 1 || $value['book_style'] == 3) {
                                    ?>
                                    <span class="red pointer" onClick="change('<?php echo $value['dining_room_id'];?>','off','2')">停止预约</span>
                                    <?php
                                     }else {
                                    ?>
                                    <span class="blue pointer" onClick="change('<?php echo $value['dining_room_id'];?>','on','2')">开始预约</span>
                                <?php
                                }
                                ?>
                                </td>
                                <td><a class="blue" href="<?php echo site_url('appointment/restaurant/edit?ids='.$value['dining_room_id']);?>">编辑</a></td>
                            </tr>
                             <?php
                                    }
                                }
...
// 	  modal.find('.modal-body input').val(recipient)
    ...
    function change(id,status,type)
    {
        if(!confirm("确定更改")){
            return false;
        }
        $.post('<?php echo site_url('/appointment/restaurant/change_status')?>',{
            'dining_room_id':id,
            'book_style':status,
            'type':type,
