                ...
                <div  class="l_content clearfix">
                    ...
                    <table class="l_content_list">
                        <thead>
                            ...
                            <th width="8%">用户评分</th>
                            <th width="8%">中奖礼品</th>
                            <th width="8%">绩效发放状态</th>
                            <th width="8%">礼品发放状态</th>
                        </thead>
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for(var item in _json){
                ...
                _html += "<td>" + _json[item]["master_dept"] + "</td>"//所属部门
                _html += "<td>" + _json[item]["member_card_no"] + "</td>"//打赏金额
                _html += "<td>" + _json[item]["pay_money"] + "</td>"//打赏金额
                _html += "<td>" + _json[item]["score"] + "</td>"//用户评分
                _html += "<td>" + _json[item]["reward_name"] + "</td>"//中奖礼品
                _html += "<td>" + _json[item]["send_status"] + "</td>"//发放状态
                _html += "<td>" + _json[item]["is_send"] + "</td>"//礼品发放状态
                //***内容end***
                _html += "</tr>"
            }
...
$("#Exportreport").on("click",function(){
    //导出报表
     window.location.href="<?php echo site_url('tips/reward/extdata')?>?is_send="+$("#j_state").val()+"&hotel_id="+$("#j_hotel").val()/*+'&start_time='+$("#datepicker").val()+'&end_time='+ $("#datepicker2").val()+'&wd='+$("#j_wd").val()*/;
})
</script>
</body>
</html>
