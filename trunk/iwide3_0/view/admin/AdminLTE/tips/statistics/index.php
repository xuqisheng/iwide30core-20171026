                ...
                <div  class="l_content clearfix">
                    ...
                    <table class="l_content_list">
                        <thead>
                            ...
                            <th width="14%">所属门店</th>
                            <th width="10%">打赏总数</th>
                            <th width="13%">打赏总金额</th>
                            <th width="10%">总评分</th>
                            <th width="13%">总金额排名</th>
                            <th width="10%">已发金额</th>
                            <th width="10%">未发金额</th>
                        </thead>
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for(var item in _json){
                ...
                _html += "<td>" + _json[item]["sum_count"] + "</td>"//打赏总数
                _html += "<td>" + _json[item]["GRADE_TOTAL"] + "</td>"//打赏总金额
                _html += "<td>" + _json[item]["sum_score"] + "</td>"//总评分
                _html += "<td>" + _json[item]["rank"] + "</td>"//总金额排名
                _html += "<td>" + _json[item]["DELIVER"] + "</td>"//已发金额
                _html += "<td>" + _json[item]["UNDELIVER"] + "</td>"//未发金额
                //***内容end***
                _html += "</tr>"
            }
...
$("#Exportreport").on("click",function(){
    //导出报表
    window.location.href="<?php echo site_url('tips/statistics/extdata')?>?hotel_id="+$("#j_hotel").val()+'&start_time='+$("#datepicker").val()+'&end_time='+ $("#datepicker2").val()+'&wd='+$("#j_wd").val();
})
</script>
</body>
</html>
