...
<?php if(isset($id)){?>
<?php echo form_open('ticket/shop/edit?ids='.$id,array('id'=>'tosave','class'=>'form-horizontal','enctype'=>'multipart/form-data','onsubmit'=>'return sub();' ))?>
<?php }else{?>
<?php echo form_open('ticket/shop/add',array('id'=>'tosave','class'=>'form-horizontal','enctype'=>'multipart/form-data','onsubmit'=>'return sub();' ))?>
<?php }?>
<div class="whitetable">
    ...
    <div class="bd_left list_layout">
        ...
        <div>
            <div>景点星级</div>
            <div class="input flexgrow"><input name="spots_level" value="<?php echo isset($posts['spots_level'])?$posts['spots_level']:''?>"></div>
        </div>
        -->
    </div>
</div>
<div class="whitetable">
    <div>
        <span style="border-color:#4caf50">营业信息</span>
    </div>
    <div class="bd_left list_layout">
        <div>
            <div>售卖类型</div>
            <div class="flex flexcenter" style="max-width: 800px;!important">
                <?php foreach($sale_type as $sk=>$sv){?>
                    <label class="check"><input name="sale_type" type="radio" value="<?php echo $sk?>" <?php if(!empty($posts['sale_type'])&&$posts['sale_type']==$sk ||(!isset($id) && $sk==4)){echo "checked='checked'";}?> onclick="radiochange($(this).val())" /><span class="diyradio"><tt></tt></span><?php echo $sv?></label>
                <?php }?>
            </div>
        </div>
        <div>
    ...
    <div class="bd_left list_layout">
        <div style="padding-bottom:0" required>
            ...
            <div class="flexgrow" style="max-width:100%">
                ...
                <span style="color: red"> 注意：编辑商品时若修改时段名称，更改后请重新设置商品管理下的消费时段</span>
            </div>
...
<div class="whitetable">
    <div>
        <span style="border-color:#3f51b5">消费配置</span>
    </div>
    <div class="bd_left list_layout">
        <div>
            <div>消费限制</div>
            <div class="input ">
                可提前 <input name="advance_book_num" style="width: 80px;" value="<?php echo isset($posts['advance_book_num'])?$posts['advance_book_num']:''?>"> 天预订
            </div>
        </div>
    </div>
</div>
...
<div class="whitetable">
    <div>
        <span style="border-color:#3f51b5">店铺图片</span>
    </div>
    <div class="bd_left list_layout">
        <div required>
            <div>店铺图片 *</div>
            <div>
                <input type="hidden" required name="shop_image" id="el_shop_img" value='<?php if(!empty($posts['shop_image'])) echo $posts['shop_image'];?>'>
                <div id="shop_img" class="addimgs trim">
                    <?php if(!empty($posts['shop_image'])){?>
                        <div class="addimg candelete"><del></del><div><img src="<?php echo isset($posts['shop_image']) ? $posts['shop_image'] : '';?>"/></div></div>
                    <?php }?>
                </div>
                <div id="shop_img_add"></div>
            </div>
            <div class="layoutfoot">建议上传尺寸600*336像素的图片 图片大小必须 &lt; 300KB;</div>
        </div>
        <div>
            <div class="flex_aligntop">店铺简介</div>
            <div class="flexgrow input"><textarea name="shop_profiles" maxlength=""><?php echo !empty($posts['shop_profiles']) ? $posts['shop_profiles'] : '';?></textarea></div>
        </div>
    </div>
</div>
...
<div class="whitetable">
    <div>
        <span style="border-color:#3f51b5">其他信息</span>
    </div>
    <div class="bd_left list_layout">
        <div>
            <div>即时确认</div>
            <div class="flex flexcenter">
                <label class="check"><input name="instant_confirm" type="radio" value="1"  <?php echo  isset($posts['instant_confirm']) && $posts['instant_confirm'] == 1 || empty($posts['instant_confirm']) ? 'checked' :'';?>/><span class="diyradio"><tt></tt></span>是</label>
                <label class="check"><input name="instant_confirm" type="radio" value="2"  <?php echo  isset($posts['instant_confirm']) && $posts['instant_confirm'] == 2 ? 'checked' :'';?>/><span class="diyradio"><tt></tt></span>否</label>
            </div>
        </div>
        <div>
            <div>逾期订单</div>
            <div class="flex flexcenter">
                <label class="check"><input name="outdate_order" type="radio" value="1"  <?php echo  isset($posts['outdate_order']) && $posts['outdate_order'] == 1 || empty($posts['outdate_order']) ? 'checked' :'';?>/><span class="diyradio"><tt></tt></span>不处理</label>
                <label class="check"><input name="outdate_order" type="radio" value="2"  <?php echo  isset($posts['outdate_order']) && $posts['outdate_order'] == 2 ? 'checked' :'';?>/><span class="diyradio"><tt></tt></span>自动核销 <em style="color: #999">(次日00:00 自动核销前一日到期订单)</em></label>
            </div>
        </div>
    </div>
</div>
...
<div class="whitetable">
    <div>
        <span style="border-color:#3f51b5">分享设置</span>
    </div>
    <div class="bd_left list_layout">
        <div>
            <div>分享配图 </div>
            ...
            <div class="layoutfoot">建议上传尺寸200*200像素的图片 图片大小必须 &lt; 300KB;</div>
        </div>
        <div>
            <div>分享标题</div>
            <div class="input flexgrow"><input name="share_title" value="<?php echo isset($posts['share_title'])?$posts['share_title']:''?>"></div>
        </div>
        <div>
            <div class="flex_aligntop">分享内容</div>
            <div class="flexgrow input"><textarea name="share_spec" maxlength=""><?php echo !empty($posts['share_spec']) ? $posts['share_spec'] : '';?></textarea></div>
        </div>
    </div>
</div>
...
<div class="whitetable">
    <div>
        <span style="border-color:#ebe814">优惠策略</span>
    </div>
    <div class="bd_left list_layout">
        <div>
            <div>模板消息通知</div>
