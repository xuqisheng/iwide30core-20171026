    ...
    <div class="over_x" style="background:#ecf0f5;">
        ...
        <form class="head_cont contents_list bg_fff" method="get" action="" name='form'>
                  ...
                  <?php  if($hotel){foreach ($hotel as $hotel_arr){
                            if(isset($entity_id)){
                                if(in_array($hotel_arr['hotel_id'],$entity_id)){   ?>
                                    <option value="<?php echo $hotel_arr['hotel_id'];?>"><?php echo $hotel_arr['name'];?></option>
                  <?php
                                 }
                            }else{
                  ?>
          ...
          <div class="j_head">
            <div  class="w_307">
              <span>关键字</span>
              <span><input type="text" placeholder="输入关键字搜索" name="keyword" required/></span>
            </div>
            <div class="w_307">
              <span>订单号</span>
              <span><input type="text" placeholder="输入订单号搜索" name="orderid" required/></span>
            </div>
            <div>
              <span>总评分</span>
              <span>
                <select name="score" class="w_90" required>
                  ...
                  <option value="1~1.9"> 1~1.9分</option>
                  <option value="2~2.9"> 2~2.9 分</option>
                  <option value="3~3.9"> 3~3.9分</option>
                  <option value="4~4.9"> 4~4.9 分</option>
          ...
          <div style="min-width:1240px;">
            <div class="contents_list bg_fff" style="font-size:13px;">
              <div class="p_r_30 classification b_b_1">
                <div class="add_active">所有评论(<?php echo $comment['count']['all'];?>)</div>
                <div val="<?php echo $comment['count']['feedback'];?>">回复(<?php echo $comment['count']['feedback'];?>)</div>
                <div val="<?php echo $comment['count']['nfeedback'];?>">未回复(<?php echo $comment['count']['nfeedback'];?>)</div>
              </div>
            ...
            <div id="searchresult">
              ...
              ?>
                  <div class="border_1 b_box m_b_10 feedbackrow" <?php if(isset($feedback[$arr['comment_id']])){echo 'status="feedback"';}else{echo 'status="nofeedback"';}?>>
...
//    console.log(array);
    var _html = '';
    var _html_score='';
    $.get('<?php echo site_url('hotel/comment/get_comment')?>',{
        data:array
    },function(data){
        var allcomment = 0;
        var replycomment = 0;
        ...
        $.each(data.comment.list,function(k,v){
              ...
              if(data.feedback[v.comment_id]!=undefined){
                replycomment++;
                _html += '<div class="border_1 b_box m_b_10 feedbackrow" status="feedback">';
              }else{
                _html += '<div class="border_1 b_box m_b_10 feedbackrow" status="nofeedback">';
                noreplycomment++;
              }
              allcomment++;
          ...
          if($(this).hasClass('color_2d87e2')){
              ...
              $('.confirms').click(function(){
                if($('.textarea_con').val()!=''){
                    ...
                    var comment_id = _this.attr('value');
                    ...
                    $.get('<?php echo site_url('hotel/comment/hotel_new_feedback')?>',{
                      ...
                      function(data){
                        if(data.status==1){
                          $('.fixed_box').fadeOut();
                          _this.parents('.b_box').find('.comment').html('回复：'+$('.textarea_con').val());
                          _this.parents('.b_box').find('.comment').show();
                          _this.parents('.b_box').attr('status','feedback');
                          var nowindex = $('.classification').find('.add_active').index();
                          console.log(nowindex);
                          if(nowindex == 2){
                            _this.parents('.b_box').hide();
                          }
