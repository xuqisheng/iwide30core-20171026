					...
					<div class="con_right">
						<?php if(!$model->has_data()){?>
							<div class="hottel_name ">
								...
								<div class="input_txt">
									<select style="width:450px;" name="hotel_id" id="hotel">
										<?php if(!empty($fields_config['hotel_id']['select'])){?>
											<?php foreach ($fields_config['hotel_id']['select'] as $k => $v){?>
												<option value="<?php echo $k;?>" <?php if (!empty($hotel_id)&&$k==$hotel_id){?>selected<?php }?>><?php echo $v;?></option>
											<?php }?>	
										<?php }?>
									</select>
									<?php if(count($fields_config['hotel_id']['select'])>10){?>
						...
						<?php }else if (!empty($hotel_name)){?>
							<div class="hottel_name ">
							<div class="required">酒店</div>
							<div class="input_txt"><span><?php echo $hotel_name;?></span></div>
						</div>
						<?php }?>
						<div class="hottel_name ">
						...
						<div class="hottel_name ">
							<div class="required">门市价</div>
							<div class="input_txt"><input required placeholder="门市价" type="number" name="oprice" value="<?php if($model->has_data()) echo $model->m_get('oprice');?>"/></div>
						</div>
						<div class="address">
