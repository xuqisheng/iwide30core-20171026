...
<input type='hidden' name='rule_id' value='<?php echo $list['rule_id'];?>' />
	...
	.li_con >span:nth-of-type(2){padding-left:10px;}
      ...
      <div class="row r_cont" style="padding:2% 3%;">
      	...
      	<div class="li_con">
        	<span>会员等级：</span>
        	<span><?php if (empty($list['extra_condition']['level'])){?>全部会员等级
                  <?php }else{if (!empty($member_levels)){ foreach ($member_levels as $level=>$name){ if (!empty($list['extra_condition']['level'])&&in_array($level, $list['extra_condition']['level'])){ echo $name.'&nbsp;'; } }} }?></span>
        </div>
        <div class="li_con">
        	<span>酒店房型：</span>
      	...
      	<div class="li_con">
        	<span>支付方式：</span>
        	<span><?php if (empty($list['extra_condition']['paytype'])){?>全部支付方式
                  <?php }else{if (!empty($pay_ways)){foreach ($pay_ways as $k=>$p){if (!empty($list['extra_condition']['paytype'])&&in_array($p->pay_type, $list['extra_condition']['paytype'])){ echo $p->pay_name.'&nbsp;';} } } }?></span>
        </div>
      	<div class="li_con">
        	<span>积分规则：</span>
