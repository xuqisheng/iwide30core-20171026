...
<script type="text/javascript">
    <?php
    $sort_index= $module->field_index_in_grid($default_sort['field'],10);
    $sort_direct = $default_sort['sort'];
    $num= (config_item('grid_static_num'))? config_item('grid_static_num'): 500;
    $buttions='<div class="credit-export actives" id="subbtn" data-action="'.EA_const_url::inst()->get_url('*/memberexport/export').'">导出</div>';
...
$(function () {
    Wind.use("artDialog",function (){
        $(document).on('click','.credit-export',function () {
            var div1=$("<div/>",{id:'expot'}),div2=$("<div/>",{class:'expot'}).appendTo(div1),space='&nbsp;';
            $("<span/>").html('记录时间：').appendTo(div2),$("<input/>",{type:'text',name:'start_time',autocomplete:'off',class:'form-control input-sm date-export1'}).appendTo(div2),$("<span/>").html('至').appendTo(div2),$("<input/>",{type:'text',name:'end_time',autocomplete:'off',class:'form-control input-sm date-export1'}).appendTo(div2);
            ...
            art.dialog({
                ...
                ok:function (){
                    ...
                    window.location= url_action+'?tp=6&bt='+start_time+'&et='+end_time;
                },
                cancel:true,
                follow:onthis,
                okVal:'导出',
