...
if(isset($result['data'])&&!empty($result['data'])){
    foreach($result['data'] as $key=>$arr){
        if($arr[5]=='--'){
            $result['data'][$key][5]='部分酒店';
        }
    }
}
...
//$buttions.= '<button type="button" class="btn btn-sm disabled" style="margin-left:5px" id="grid-btn-edit2"><i class="fa fa-edit"></i>&nbsp;协议价</button>';
//$buttions.= '<button type="button" class="btn btn-sm disabled"  style="margin-left:5px" id="grid-btn-edit3"><i class="fa fa-edit"></i>&nbsp;成员</button>';
$buttions.= '<button id="qrcode_btn" data-target="#myModal" data-toggle="modal" type="button" class="btn btn-sm disabled single-btn" style="margin-left:5px"><i class="fa fa-qrcode"></i>&nbsp;二维码</button>';
$buttions.= '<button id="club_staff"  data-toggle="modal" type="button" class="btn btn-sm disabled single-btn" style="margin-left:5px"><i class="fa fa-edit"></i>&nbsp;成员</button>';
...
var url_add= '<?php echo EA_const_url::inst()->get_url("club/club_list/club_edit"); ?>';			//跟button对应
var url_edit= '<?php echo EA_const_url::inst()->get_url("club/club_list/club_edit"); ?>';		//跟button对应
var url_delete= '<?php echo EA_const_url::inst()->get_url("*/*/delete"); ?>';	//跟button对应
var url_ajax= '<?php echo EA_const_url::inst()->get_url("*/*/*"); ?>';

<?php /* 有更多的按钮，URL在此定义，与上面button顺序匹配 */ ?>
var url_extra= [
    '<?php echo EA_const_url::inst()->get_url("*/Club/qrcode_front"); ?>'
];
<?php /* 有更多的按钮，URL在此定义，与上面button顺序匹配 */ ?>
...
//        alert('123');
        var base_url= '<?php echo EA_const_url::inst()->get_url("*/club/qrcode_front"); ?>?ids=';
//        alert(base_url+selected);
        if( isNaN(selected) ){
            $('#myModal div[class="modal-body"]').html('<diiv class="alert alert-danger">请单独选择一个选项</div>');
        } else {
            var img= '<img id="qrcode-img" src="'+ base_url+ selected+ '" />';
//            console.log(img);
            $('#myModal div[class="modal-body"]').html(img);
        }
    });


    $('#club_staff').click(function(){

        if( isNaN(selected) ){
            $('#myModal div[class="modal-body"]').html('<diiv class="alert alert-danger">请单独选择一个选项</div>');
        } else {
            var base_url= '<?php echo EA_const_url::inst()->get_url("*/club_customer/index"); ?>?ids='+selected;
