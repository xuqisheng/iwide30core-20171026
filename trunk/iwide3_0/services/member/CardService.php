...
class CardService extends MemberBaseService
{
    ...
    //卡券详细页面
    public function receive($inter_id = '',$openid = '',$ec_code = '', $url_group = array(), $is_restful = false){
        $this->getBase();
        $this->assign_data = $url_group;
        $assign_data = $this->assign_data;
        ...
        $_card_info = $this->doCurlPostRequest(INTER_PATH_URL."membercard/get_info_by_code",$post_data);
        MYLOG::w("membercard_getinfo | ".@json_encode(array('id'=>$inter_id,'openid'=>$openid,'post_data'=>$post_data,'data'=>$_card_info)),'iapi/membervip/debug-log','receive');
    ...
    //卡券详细页面
    public function cardinfo($inter_id = '',$openid = '',$member_card_id = 0,$url_group = array(),$is_restful = false){
        ...
        $assign_data['js_share_config']['link'] = EA_const_url::inst()->get_url('membervip/card/receive',array('id'=>$inter_id,'sf'=>$encrypt));
    ...
    public function codeuseoff($inter_id = '',$openid = ''){
        ...
        if(!empty($scanqr_auth) && $scanqr_auth['status'] == 1){
            ...
            //增加以下jsapi
            $base_api_list = "'scanQRCode', 'closeWindow'";
            $data= array(
                'message'=> '点击页面，开始核销',
    ...
    public function check_useoff($inter_id = '',$openid = '',$coupon_code = '',$url_group = array()){
        ...
        if(is_ajax_request()){
            ...
            if(!empty($member_card) && $member_card['is_use'] == 't' && $member_card['is_useoff'] == 't'){
                ...
                $this->res_data['data'] = array('cardcenter_url' => $this->assign_data['cardcenter_url']);
                return $this->res_data;
            }
            $this->res_data['status'] = 3;
            $this->res_data['msg_lvl'] = 1;
