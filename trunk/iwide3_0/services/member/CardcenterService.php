 ...
 */
class CardcenterService extends MemberBaseService
{
    ...
    /**
     * 优惠券列表
     * @param string $show_inter_id 授权微信酒店集团ID
     * @param string $show_openid 授权微信用户ID
     * @param string $inter_id 微信酒店集团ID
     * @param string $template 前端模版名称
     * @param array $url_group 链接组
     * @return array
     */
    public function index($show_inter_id = '',$show_openid = '',$inter_id = '', $template = '',$url_group = array()){

        //start show
        $post_center_url = PMS_PATH_URL."member/center";
        $post_center_data =  array(
            'inter_id'=>$show_inter_id,
            'openid' =>$show_openid,
        );
        //请求用户登录(默认)会员卡信息
        ...
        $assign_data['inter_id'] = $show_inter_id;
        $assign_data['f_sign'] = $this->encrypt($show_inter_id,$show_openid);
    ...
    //加密链接参数
    public function encrypt($show_indter_id = '', $show_open_id = '', $inter_id = '',$open_id = ''){
        //加密参数
        $this->getCI()->load->helper('qfglobal');
        $public = $this->getCI()->Publics_model->get_public_by_id ($inter_id);
        $key = $public['app_secret'];
        $ec_data = $show_indter_id.$show_open_id;
        $sign = kecrypt($ec_data,$key);
        return base64_encode($inter_id.'***'.$open_id.'***'.$sign);
    }
    ...
    private function parse_card_data($card_data = array(),$key = 'usableCardLists',$url_group = array()){
        ...
        if(!empty($card_data[$key])){
            foreach ($card_data[$key] as &$v){
                ...
                $v['expire_time_quantum'] = isset($v['expire_time']) ? date('Y.m.d', $v['use_time_start']) . ' - ' . date('Y.m.d', $v['expire_time']) : '';
            }
     ...
     * @return array
     */
    public function pcardinfo($show_inter_id = '',$show_openid = '', $member_card_id = 0, $url_group = array()){
        $this->getBase();
        $this->assign_data = $url_group;
        $data = $this->assign_data;
        $my_card = false;
        ...
        if(!empty($member_card_id)){
            ...
            if(empty($card_info)){
                ...
                $card_info['expire_time_quantum'] = isset($card_info['expire_time']) ? date('Y.m.d', $card_info['use_time_start']) . ' - ' . date('Y.m.d', $card_info['expire_time']) : '';
                ...
                if($member_mode == 2){
                    ...
                    $fens_user = $this->getCI()->mem->get_user_info($show_inter_id,$show_openid,'member_info_id',$extra);
                    if(!empty($fens_user)){
        ...
        if(in_array($show_inter_id,$auth_inter)){
            $auth_gift = true;
        }

        $data['auth_gift'] = $auth_gift;

        $data['authentication_give'] = 0;
        if($auth_gift === true && !empty($card_info) && $card_info['is_can_give_friend'] == 't' && $card_info['is_giving'] == 'f'){
            $data['authentication_give'] = 1;
        }

        $this->getCI()->load->model('wx/access_token_model');
        $data['signpackage'] = $this->getCI()->access_token_model->getSignPackage($show_inter_id);
        $data['public'] = $this->getCI()->Publics_model->get_public_by_id($show_inter_id);
        $data['inter_id'] = $show_inter_id;
    ...
    //领取卡券
    public function addcard($show_inter_id = '', $show_openid = '', $card_rule_id = ''){
        $this->getBase();
        //获取领取卡券的信息
        $post_card = array(
            'inter_id'=>$show_inter_id,
        ...
        $add_card_data = array(
            ...
            'module'=>'vip',
            'uu_code'=>$show_openid.'gaze'.uniqid()
        );
        ...
        $this->res_data['data'] = (is_array($add_card_result['data'])) ? $add_card_result['data'] : $add_card_result;
        return $this->res_data;
    }
    ...
    public function hang_card($inter_id = '', $openid = '', $member_card_id = 0){
        ...
        $this->res_data['data'] = (is_array($result['data'])) ? $result['data'] : $result;
        return $this->res_data;
    }

    //转赠优惠券挂起
    public function gift_card($show_inter_id = '',$show_openid = '', $member_card_id = 0, $module = '',$card_code = ''){
        //卡券转赠挂起
    ...
    //转赠优惠券挂起
    public function receive_card($inter_id = '', $openid = '', $ec_code = '', $url_group = array()){
        $this->getBase();
        if(empty($ec_code)) {
            $this->res_data['status'] = 3;
            $this->res_data['msg_lvl'] = 1;
        ...
        $result = $this->parse_curl_msg($result);
        $this->res_data = $url_group;
        if(!empty($result) && is_array($result)) $this->res_data = array_merge($this->res_data,$result);
        $this->res_data['status'] = $result['code'] == '1000' ? 1 : 3;
        $this->res_data['data'] = (is_array($result['data'])) ? $result['data'] : $result;
        return $this->res_data;
    }
    ...
    public function savegivecard($inter_id = '',$openid = '', $from_openid = '',$card_id = 0,$cardModule = 'vip'){
        ...
        $this->res_data['data'] = (is_array($give_info['data'])) ? $give_info['data'] : $give_info;
        return $this->res_data;
    }

    //卡券详细页面
    public function receive($inter_id = '',$openid = '',$ec_code = '', $url_group = array(), $extra = array(), $is_restful = false){
        $this->getBase();
        $this->assign_data = $url_group;
        ...
        if(empty($ec_code)) {
            ...
            $this->res_data['jump'] = 1;
            if(!empty($extra['org_domain'])){
                $this->res_data['redirect_uri'] = "{$extra['org_domain']}/membervip/center?id={$inter_id}";
            }else{
        ...
        MYLOG::w("get_user_info | ".@json_encode(array('id'=>$inter_id,'openid'=>$openid,'data'=>$user)),'iapi/membervip/debug-log','receive');
        ...
        $_card_info = $this->doCurlPostRequest(INTER_PATH_URL."membercard/get_info_by_code",$post_data);
        MYLOG::w("membercard_getinfo | ".@json_encode(array('id'=>$inter_id,'openid'=>$openid,'data'=>$_card_info)),'iapi/membervip/debug-log','receive');
        ...
        $assign_data['can_received'] = 0;
        if(!empty($card_info['is_can_give_friend']) && $card_info['is_can_give_friend']=='t' && $card_info['is_giving']=='t' && $user['member_info_id'] != $gift_mem_info['member_info_id']){
            $assign_data['can_received'] = 1;
        }
    ...
    //卡券详细页面
    public function cardinfo($show_inter_id = '',$show_openid = '', $inter_id = '',$member_card_id = 0,$url_group = array(),$extra = array(),$is_restful = false){
        $this->getBase();
        $this->assign_data = $url_group;
        $assign_data = $this->assign_data;
        if(empty($member_card_id)){ //参数为空
            ...
            $this->res_data['jump'] = 1;
            if(!empty($extra['share_domain'])){
                $this->res_data['redirect_uri'] = "{$extra['share_domain']}/membervip/cardcenter?id={$inter_id}&f={$extra['segments']}";
            }else{
                $this->res_data['redirect_uri'] = site_url('membervip/card').'?id='.$show_inter_id;
            }
            $this->res_data['data'] = array();
            return $this->res_data;
        }
        ...
        $this->getCI()->load->model('membervip/front/Member_model','mem');
        $user = $this->getCI()->mem->get_user_info($show_inter_id,$show_openid,'member_info_id,open_id,member_mode,is_login');
        if(empty($user)){ //找不到会员
            $this->res_data['status'] = 3;
        ...
        if($user['member_mode'] == 2){
            ...
            $fens_user = $this->getCI()->mem->get_user_info($show_inter_id,$show_openid,'member_info_id',$_extra);
            if(!empty($fens_user)){
                $my_member_info_ids[] = $fens_user['member_info_id'];
            }
        }
        ...
        $key = base64_encode($show_inter_id);

        $encrypt = encrypt($ec_data,$key);
        $encrypt = base64_encode($encrypt);
        MYLOG::w("card_encrypt | ".@json_encode(array($key,$ec_data,$encrypt)),'membervip/debug-log');
        if(!empty($extra['share_domain'])){
            $assign_data['js_share_config']['link'] = "{$extra['share_domain']}/membervip/cardcenter/before_receive?id={$inter_id}&f={$extra['segments']}&sf={$encrypt}";
        }else{
            $assign_data['js_share_config']['link'] = \EA_const_url::inst()->get_url('membervip/card/receive',array('id'=>$show_inter_id,'sf'=>$encrypt));
        }
        ...
        $assign_data['user'] = $user;
        $assign_data['inter_id'] = $show_inter_id;
        $assign_data['openid'] = $show_openid;
        $assign_data['my_card'] = $my_card;
        $assign_data['auth_useoff'] = $auth_useoff;
        $this->getCI()->load->model('wx/access_token_model');
        $assign_data['public'] = $this->getCI()->Publics_model->get_public_by_id($show_inter_id);
    ...
    public function card_callback($inter_id = '',$openid = '',$code = ''){
        ...
        $this->res_data['data'] = (is_array($useoff_result['data'])) ? $useoff_result['data'] : $useoff_result;
        return $this->res_data;
    }

    public function passwduseoff($show_inter_id = '',$show_openid = '',$member_card_id = 0, $passwd = ''){
        $this->getBase();
        if(!$passwd){
            $this->res_data['status'] = 3;
            $this->res_data['msg_lvl'] = 1;
        ...
        if($useone['code'] == 1000){
            ...
            $this->res_data['data'] = (is_array($useoff['data'])) ? $useoff['data'] : $useoff;
            return $this->res_data;
        }else{
            $this->res_data = $useone;
            $this->res_data['status'] = $useone['code'] == '1000' ? 1 : 3;
            $this->res_data['data'] = (is_array($useone['data'])) ? $useone['data'] : $useone;
            return $this->res_data;
        }
    }

    public function getpackage($show_inter_id = '',$show_openid = '',$package_id = 0, $frequency = 0,$card_rule_id = 0){
        $this->getBase();
        //获取领取礼包的信息
        $post_card = array(
            'inter_id'=>$show_inter_id,
        ...
        $this->res_data['data'] = (is_array($package['data'])) ? $package['data'] : $package;
        return $this->res_data;
    }
