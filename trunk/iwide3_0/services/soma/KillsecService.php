...
//
     ...
     */
    public function get_openid_subscribe_actid(Array $actids, $inter_id, $openid)
    {
        $this->getCI()->load->model('soma/Activity_killsec_notice_model', 'killSecModel');
        $killSecModel = $this->getCI()->killSecModel;
        $result = $this->getCI()->soma_db_conn_read->select('notice_id,act_id,inter_id,openid')
                ->where('inter_id', $inter_id)
                ->where('openid', $openid)
                ->where_in('act_id', $actids)
                ->get($killSecModel->table_name($inter_id))
                ->result_array();
