...
/**
*	会员卡卡券/var/www/iwide30core/iwide3_0/controllers/front/membervip/Card.php
*	@author  Frandon
...
//	    $this->check_user_login();
    ...
    public function gift_card(){
        ...
        $module = !empty($this->args['module'])?floatval($this->args['module']):'';
        $card_code = !empty($this->args['card_code'])?floatval($this->args['card_code']):'';
        $data = array();
        if(!$this->is_restful()){
    ...
    public function receive_card(){
        $ec_code = !empty($this->args['ec_code'])?floatval($this->args['ec_code']):0;
        $data = array();
        if(!$this->is_restful()){
            $data = CardService::getInstance()->receive_card($this->inter_id,$this->openid,$ec_code);
        }
        echo json_encode($data);
    }
    ...
    public function receive(){
        $ec_code = !empty($this->args['sf'])?floatval($this->args['sf']):'';
        $data['data'] = array();
        if(!$this->is_restful()){
            $data = CardService::getInstance()->receive($this->inter_id,$this->openid,$ec_code,$this->url_group);
	...
	public function cardinfo(){
        ...
        $data['data'] = array();
        if(!$this->is_restful()){
            $data = CardService::getInstance()->cardinfo($this->inter_id,$this->openid,$member_card_id,$this->url_group);
        }
	...
	public function card_callback(){
        $code = !empty($this->args['code'])?floatval($this->args['code']):'';
        $data = array();
        if(!$this->is_restful()){
	...
	*/
	public function passwduseoff(){
        $member_card_id = !empty($this->args['member_card_id'])?floatval($this->args['member_card_id']):'';
        $passwd = !empty($this->args['passwd'])?floatval($this->args['passwd']):'';
        $data = array();
        if(!$this->is_restful()){
