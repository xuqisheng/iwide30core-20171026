...
*/
    ...
    public function package_booking()
    {
        ...
        if($this->isNewTheme()){ // 新主题
            $this->headerDatas['title'] = '预订';
            $this->_view("package_booking", []);
            return;
        }
...
// var_dump( $item );die;
    ...
    public function package_usage()
    {
        ...
        if($this->isNewTheme()){ // 新主题
            $this->headerDatas['title'] = '验劵';
            $this->_view("package_usage", []);
            return;
        }
	...
	public function get_shipping_order_id_by_ajax() {
		...
		if($order && $order->m_get('grand_total') > 0) {
	    	...
	    	if( isset( $result['status'] ) && $result['status'] == Soma_base::STATUS_TRUE ){
	    	    ...
	    	    $op_res['message'] = $result['message'];
            }
	    }
	    if (date('Y-m-d', time()) < '2017-09-08') {
            $ConsumerOrderModel->_write_log('shippingresultttttt'.json_encode($op_res));
        }
