...
class Hotel extends MY_Front_Hotel {
	...
	function __construct() {
		...
		if ($this->input->get ( 'theme' )) {
			$this->session->set_userdata ( $this->inter_id . 'skin', $this->input->get ( 'theme' ) );
		}
	}
	function search() {
		...
		//可预订的开始日期
		$date_check = $this->Order_model->date_validate ( date('Ymd'), date('Ymd',time()+86400), $this->inter_id,0,$config_data);
	...
	function sresult() {
        ...
        $is_near = $this->input->get ( 'nearby', TRUE );
		...
		$this->load->model ( 'hotel/Hotel_model' );
		$this->load->model ( 'hotel/Order_model' );
		$date_check = $this->Order_model->date_validate ( $startdate, $enddate, $this->inter_id,0,$config_data);
		$data ['startdate'] = $date_check [0];
		$data ['enddate'] = $date_check [1];
		$data ['pre_sp_date']=$date_check [2];
	...
	function index() {
		...
		$hotel_id = $this->Hotel_model->get_a_hotel_id ( $this->inter_id, $this->input->get ( 'h' ), true );
		...
		$this->load->model ( 'hotel/Order_model' );
		$date_check = $this->Order_model->date_validate ( $startdate, $enddate,$this->inter_id,$hotel_id,$config_data);
		$data ['startdate'] = $date_check [0];
		$data ['enddate'] = $date_check [1];
		$data['pre_sp_date'] = $date_check [2];
		...
		//专题活动处理 add by ping
		$tc_id = $this->input->get ( 'tc_id', TRUE );
		if($tc_id>0){
			$data ['tc_id'] = $tc_id;
            ...
            if (!empty($r['state_info'])){
                ...
                foreach ($r['state_info'] as $state_key=>$state){
                    ...
                    if (empty($state['condition']['no_coupon'])&&empty($state['coupon_condition']['no_coupon'])){
                        ...
                        $cardlist = $this->Coupon_model->select_coupon($params,$mycards);
	...
	function return_more_room() {
        ...
        $member_level =$this->input->post ( 'mem_level' );
	...
	function bookroom() {
        ...
        $package_info = json_decode ( $this->input->post ( 'package_info' ), TRUE );
        ...
        if (!empty($data ['first_state']['goods_info']['items'])){
            if (!empty($data ['first_state']['is_packages'])&&empty($package_info)&&$data ['first_state']['goods_info']['sale_way']!=2){
                ...
                $package_check = $this->Goods_order_model->check_order_package($this->inter_id,$data ['first_state']['goods_info'],$package_info,array('startdate'=>$data ['startdate'],'enddate'=>$data ['enddate'],'roomnums'=>1));
...
// 		) );
        ...
        if (empty($data ['first_state']['condition']['no_coupon'])&&empty($data ['first_state']['coupon_condition']['no_coupon'])){
            ...
            $cardlist = $this->Coupon_model->select_coupon($coupon_condition);   //自动选择优惠券
    ...
    //            $data['total_price'] = $data['total_price'] - $select_coupon['select_coupon_favour'];
        ...
        $data['paytype_icon'] =  $this->Hotel_model->bigger_payways_icon();
...
MYLOG::w('bonus_exchange+'.'room_info_bonus_consume_code：'.$bonus_consume_code,"bonus_exchange");
		...
		if (!empty($first_state['goods_info']['items'])){
    		if (empty($package_info)&&$first_state['goods_info']['sale_way']!=2){
    		    ...
    		    $this->load->model('hotel/goods/Goods_order_model');
                $package_check = $this->Goods_order_model->check_order_package($this->inter_id,$first_state['goods_info'],$package_info,array('startdate'=>$startdate,'enddate'=>$enddate,'roomnums'=>$order_data['roomnums']));
    		    if ($package_check['s']==0){
    		        unset($package_check['data']);
    		        echo json_encode ( $package_check );
    		        exit ();
    		    }else{
...
// 		) );
		...
		if(empty($order_data ['link_saler'])){
			...
			$true_saler = $this->Idistribute_model->get_protection_saler($this->openid,$this->inter_id);
			if(!empty($true_saler)){
			...
			if ($pre_pay != 1) {
				...
				if ($msg ['s'] == 0) {
					...
					if (!empty($info['has_paid'])||!empty($msg['has_paid'])){
						...
						$this->Order_model->update_order_status ( $this->inter_id, $info ['orderid'], $status, $this->openid, true );
					    ...
					    $this->Order_model->update_order_status ( $this->inter_id, $info ['orderid'], 1, $this->openid, FALSE );
					}
				}
			} else {
	...
	function add_hotel_collection() {
		...
		$mark_title = $this->input->get ( 'hname' );
		$data = array (
			'mark_name' => $hotel_id,
			'inter_id' => $this->inter_id,
			'openid' => $this->openid,
		...
		$this->load->model ( 'hotel/Hotel_model' );
		echo $this->Hotel_model->add_front_mark ( $data );
	}
	...
	function orderdetail() {
		...
		if (!empty($orderid)){
		    ...
		    $order_condition['oid'] = intval ( $this->input->get ( 'oid' ) );
    		$oid=$order_condition['oid'];
		}
		$this->load->model ( 'hotel/Order_model' );
		$order_condition ['package_condition'] = array('get_goods_info'=>2,'syn_status'=>1);
		$list = $this->Order_model->get_main_order ( $this->inter_id, $order_condition );
		if ($list) {
			$list = $list [0];
			$flag = 1;
			$comment = 0;
	...
	function hotel_photo() {
        ...
        $skin_config=$this->get_skin_config($module_view['skin_name'], 'hotel/hotel_photo');
        $module_view=array(
            'module_view'=>$module_view
        );
	...
	function return_usable_coupon() {
        ...
        $pay_favour = $this->input->post ( 'pay_favour' );
        ...
        if(isset($config_data['MORE_ROOM_NO_FAVOUR']) && $config_data['MORE_ROOM_NO_FAVOUR']==1 && $params ['product_num'] > 1){
...
//                保存上传完之后的URL
    ...
    public function photo_list(){
        ...
        $data['inter_id'] = $this->inter_id;
        $hotel_id = $this->input->get('h');
        $data['gallery'] = $this->Gallery_model->get_gallery ( $this->inter_id, array('hotel_id'=>$hotel_id) );
        $this->display('hotel/bigger/hotel_photo',$data);
    }
