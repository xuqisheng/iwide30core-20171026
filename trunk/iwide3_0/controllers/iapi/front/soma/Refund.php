...
class Refund extends MY_Front_Soma_Iapi
{
    ...
    public function get_index()
    {
        ...
        $data['item_detail'] = [
            'price_package' => $order_detail['items'][0]['price_package'],
            'qty' => $order_detail['items'][0]['qty'],
            'face_img' => $order_detail['items'][0]['face_img'],
            'hotel_name' => $order_detail['items'][0]['hotel_name']
        ];
        $data['tip'] = '小提示：退款成功后，使用的积分、储值将自动返还至您的账户，购买获得的积分将被扣除';
    ...
    public function post_apply()
    {
        ...
        if ($result && $billing_result) {
            ...
            //显示申请退款成功后的页面
            $this->json(BaseConst::OPER_STATUS_SUCCESS, '');
            return;
        } else {
            $this->json(BaseConst::OPER_STATUS_FAIL_TOAST, '提交失败');
        }
