...
//        $this->load->model('soma/Consumer_shipping_model');
    ...
    public function batch_post()
    {
        ...
        //查找适用数据
        $select ='shipping_id,order_id,consumer_id,openid,inter_id,address,distributor,tracking_no,status';

        $list = $model->get_apply_list_byIds( array_keys( $shippingIds ), $inter_id, $select);
        if (empty($list)) {
            $tmp = array();
            foreach($shippingIds as $k=>$v){
                $tmp[] = array(
                    $pk => $k,
                    'message' => '已发货',
                );
            }
            $this->out_put_msg(AdminConst::OPER_STATUS_FAIL_TOAST, '全部发货失败', $tmp);
        }
