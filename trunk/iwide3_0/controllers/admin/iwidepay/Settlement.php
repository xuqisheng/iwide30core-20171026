...
class Settlement extends MY_Admin
{
    ...
    public function transfer_accounts()
    {
        ...
        $select = 'sr.id,sr.amount,sr.status,sr.is_company,sr.bank,sr.bank_card_no,sr.add_time,sr.update_time,sr.remark';
        ...
        if ($total > 0)
        {
            $status = array(0=>'待转账',1=>'成功',2=>'失败',3=>'处理中');
        ...
        $return = array(
            ...
            'param'     => $filter,
            'url'       => $url,
            'publics'   => $this->get_publics(),
            'hotels'    => $this->get_hotels($inter_id),
        );
    ...
    protected function ext_report($filter)
    {
        ...
        $headArr = array('转账时间','所属公众号','所属门店','转账金额','返回状态时间','转账状态','备注');
