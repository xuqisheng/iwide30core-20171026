...
class IwidepayApi extends MY_Controller
{
    ...
    public function check_account()
    {
        ...
        if ($status_name == 'send' || $status_name == 're_send')
        {
            ...
            {
                $res = $this->Iwidepay_merchant_model->update_check_account(array('id' => $check_record['id']),$check);
            }
            else
            {
                $res = $this->Iwidepay_merchant_model->insert_check_account($check);
            }
        }
        ...
        else
        {
            ...
            ajax_return(self::EN_ERROR,'发起验证失败',$ajax_data);
        }
    }
    ...
    {
        $param = request();
        $order_no = !empty($param['order_no']) ? addslashes(trim($param['order_no'])) : '';
        if (empty($order_no))
        {
            ajax_return(self::PARAM_ERROR,'参数错误');
        }
        ...
        $select = 'o.id,o.inter_id,o.hotel_id,o.module,o.order_no,o.order_status,o.transfer_status,o.orig_amount,o.trans_amt,o.is_dist,o.add_time';
        $orders = $this->iwidepay_order_model->get_orders($select,array('order_no' => $order_no),1,10);
        if (!empty($orders))
        {
            $this->load->model('iwidepay/Iwidepay_refund_model' );
            $module = $this->module_arr;
            ...
            {
                //未分账 1待定、2待分
                if ($value['transfer_status'] == 1 || $value['transfer_status'] == 2)
                {
                    $value['refund_amount'] = $value['trans_amt'];//可退金额即是交易金额
                }
                //已分账 查询退款表
                else
                {
                    $where_arr = array('refund_status' => 1,'module' => $value['module'],'orig_order_no' => $value['order_no']);
                    $refund_amount = $this->Iwidepay_refund_model->get_refund_amt($where_arr);

                    $value['refund_amount'] = $value['trans_amt'] - $refund_amount;
                }
    ...
    public function send_refund()
    {
        ...
        $this->load->model('iwidepay/iwidepay_order_model' );
        $order = $this->iwidepay_order_model->get_one('*',array('id' => $order_id,'order_no' => $order_no));
        if (empty($order))
        {
        ...
        //判断退款金额 未分账
        if ($order['transfer_status'] == 1 || $order['transfer_status'] == 2)
        {
            $refund_amount = $order['trans_amt'];
        }
        //已分账
        else
        {
            $this->load->model('iwidepay/Iwidepay_refund_model' );
            $where_arr = array('refund_status' => 1,'module' => $order['module'],'orig_order_no' => $order['order_no']);
            $refund = $this->Iwidepay_refund_model->get_refund_amt($where_arr);
            $refund_amount = $order['trans_amt'] - $refund;
        }
    ...
    public function single_send()
    {
        ...
        if ($status == 'send')
        {
            ...
            if (true == $res)
            {
                ajax_return(self::SUCCESS,'发起转账成功');
            }
        }
        else
        {
        ...
        $ajax_data['remark'] = '';
        $this->load->model('iwidepay/iwidepay_sum_record_model' );
        $sum = $this->iwidepay_sum_record_model->get_one('remark', array('id' => $id));
        if (!empty($sum))
        {
            $ajax_data['remark'] = $sum['remark'];
        }

        ajax_return(self::EN_ERROR,'发起转账失败',$ajax_data);
    }
