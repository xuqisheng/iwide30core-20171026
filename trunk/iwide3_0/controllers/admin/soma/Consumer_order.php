	        ...
	        if( $result ){
	        	foreach ($result as $k => $v) {
		            ...
		            if( $SalesOrderModel ) {
		                ...
		                $v['price_package'] = (($v['grand_total'] / $SalesOrderModel->m_get('row_qty')) / $v['use_cnt']) * $v['consumer_qty'];
		                $v['price_package'] = round($v['price_package'], 2);
		            } else {
		                $v['grand_total'] = '';
		            }
		...
		if( !$consumer_code && !$name && !$order_id && !$remark ){
	        ...
	        if( $result ){
	        	...
	        	foreach ($result as $k => $v) {
					...
					if($SalesOrderModel) {
						$da['price_package'] = $SalesOrderModel->m_get('real_grand_total');
						// 优化核销价格＝订单实付总额／购买数量／分时次数＊核销数量 同导出
		                if($v['can_split_use'] == Soma_base::STATUS_FALSE) {
		                	$v['use_cnt'] = 1;
		                } else {
