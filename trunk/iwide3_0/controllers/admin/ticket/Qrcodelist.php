...
class Qrcodelist extends MY_Admin_Ticket {
	...
	public function index(){
        ...
        $base_url = site_url('ticket/qrcodelist/index/');
        //获取公众号下的酒店
        ...
        $sale_type = array(1=>'客房内',2=>'堂食',3=>'外卖',4=>'预约核销');
        //分页
	...
	public function add()
	{
        ...
        if($post){//add数据
           ...
           // $filter['sale_type'] = isset($post['sale_type'])?$post['sale_type']:'';
            ...
            //$url= EA_const_url::inst()->get_front_url($this->inter_id, 'ticket/ticket/index', array('id'=> $this->inter_id,'hotel_id'=>$filter['hotel_id'], 'shop_id'=>$filter['shop_id'],'type_id'=>$filter['type_id']));
            $url = $this->get_host() . "index.php/qrcodeurl/jump/run_ticket_book?id=".$this->inter_id.'&hid='.$filter['hotel_id'].'&sid='.$filter['shop_id'].'&tid='.$filter['type_id'].'&type=din';
            //生成二维码 并上传到服务器
...
// 下载文件
    ...
    public function ajax_get_shop_info(){
        ...
        if(!empty($shop)){
            ...
            $time_range = $shop['time_range'];
            $range_time = '';
            if (!empty($time_range))
            {
                $time_range_time = array();
                $time_range = json_decode($time_range,true);
                if (!empty($time_range))
                {
                    foreach ($time_range as $value)
                    {
                        $time_range_time[] = $value['start_time'].'-'.$value['end_time'];
                    }
                }
                if (!empty($time_range_time))
                {
                    $range_time = implode(',',$time_range_time);
                }
            }

            $shop['shop_time'] = $tmp.$range_time;
            $return['errcode'] = 0;
            $return['msg'] = 'ok';
            $return['data'] = $shop;
            echo json_encode($return);
...
//导入权限
        ...
        //查询interId下的店铺
        $this->load->model('roomservice/roomservice_shop_model');
        $shop_info = $this->roomservice_shop_model->get_list(array('inter_id'=>$this->inter_id,'hotel_id'=>$entity_id,'is_delete'=>0,'sale_type'=>4));
        $shop_list = array();
        if($shop_info){
...
//                         }
                    ...
                    $url= EA_const_url::inst()->get_front_url($this->inter_id, 'ticket/book/goods_list', array('id'=> $this->inter_id,'hotel_id'=>$new_data[$row]['hotel_id'], 'shop_id'=>$new_data[$row]['shop_id'],'type_id'=>$new_data[$row]['type_id']));
                    $url = $this->get_host() . "index.php/qrcodeurl/jump/run_ticket_book?id=".$this->inter_id.'&hid='.$new_data[$row]['hotel_id'].'&sid='.$new_data[$row]['shop_id'].'&tid='.$new_data[$row]['type_id'].'&type=din';
                    //生成二维码 并上传到服务器
                    $path = 'qrcode/roomservice/qrcodelist/'.date('Ymd');
