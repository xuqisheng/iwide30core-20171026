...
// 		$data ['order_params'] = $model->order_find_des ();
		$data ['order_params'] = array('{BOOK_USER}'=>'预约姓名','{BOOK_PHONE}'=>'手机号','{GOODS_INFO}'=>'商品名称','{BOOK_DATE}'=>'预约时间'
                                ,'{PAY_MONEY}'=>'支付金额','{ORDER_SN}'=>'订单号','{STATUS}'=>'订单状态'
        );
		...
		if (! empty ( $data ['tid'] )) {
			...
			if (! empty ( $list )) {
				...
				redirect ( site_url ( 'ticket/tmmsg/index' ) );
			}
		} else {
			$msg_list = $model->get_temp_msg_list ( $this->inter_id, self::ENUM_ORDER_TMG_TYPE );
			$types = $model->get_temp_msg_types ( 'ticket' );
