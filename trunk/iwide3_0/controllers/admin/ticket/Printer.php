...
class Printer extends MY_Admin_Ticket {
	...
	public function index(){
        ...
        $base_url = site_url('/ticket/printer/index/');
        $first_url = site_url('/ticket/printer/index/').$search_url;
        $suffix = $search_url;
     ...
     * 导出打印机
     */
    public function ext_print_info()
    {
        $sql = "
            SELECT ht.`name`,rs.shop_name,pi.printer_no FROM iwide_roomservice_printer_info as pi
            LEFT JOIN iwide_roomservice_shop as rs ON rs.shop_id = pi.shop_id
            LEFT JOIN iwide_hotels as ht ON ht.hotel_id = pi.hotel_id AND pi.inter_id = ht.inter_id
            WHERE source = 1
        ";

        $data_info = $this->db->query($sql)->result_array();

        if (!empty($data_info))
        {
            $this->excel($data_info);
        }
    }
    ...
    public function excel($list)
    {
        ...
        $confs = array('name'=>'酒店名称','shop_name'=>'店铺名称','printer_no'=>'打印机编号');
        $data = "";
