...
// 				if (isset ( $list ['use_condition'] ['member_level'] ) && $list ['type'] == 'member') {
	...
	public function room_types() {
		...
		$this->load->model ( 'hotel/Hotel_model' );
		echo json_encode ( $this->Hotel_model->get_hotel_rooms ( $this->inter_id, $hotel_id, 1 ) );
	}
	...
	public function code_edit() {
		$this->label_action = '编辑价格代码／新增价格代码';
		$this->_init_breadcrumb ( $this->label_action );
		$data = $this->common_data;
	...
	public function quick_save_set() {
		...
		$this->load->model('common/Pms_model');
		$pms_set=$this->Pms_model->get_hotel_pms_set($this->inter_id,0);
		if (empty($pms_set) && (empty($data ['price']) || $data ['price'] <0.01)){
		    echo 0;exit;
		}
			...
			foreach ( $rooms as $r ) {
				foreach ( $day_arr as $day ) {
					...
					if (! $check) {
						...
						unset($data ['inter_id']);
						unset($data ['edittime']);
						$ident='room_state'.'#'.$this->inter_id.'_'.$hotel_id.'_'.$r.'_'.'-1';
						...
						$ident='room_state'.'#'.$this->inter_id.'_'.$hotel_id.'_'.$r.'_'.'-2';
						$edit_log_idents[]=$ident;
						$edit_log_datas[]=$update_diff;
					}
				}
			}
