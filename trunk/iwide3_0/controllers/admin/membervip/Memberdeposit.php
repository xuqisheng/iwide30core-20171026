...
*	@
*/
class Memberdeposit extends MY_Admin_Api
{

    protected $admin_info = '';
    protected $label_module = '会员中心4.0';
    protected $label_controller = '会员储值';
    protected $label_action = '储值列表';
    ...
    public function index(){
        ...
        $params['bl.inter_id'] = $admin_profile['inter_id'];
        ...
        $params['table_name'] = 'balance_log';
        $params['alias'] = "bl";
        $params['join'] = [
            ['table'=>'member_info m','on'=>'m.member_info_id = bl.member_info_id','type'=>'left']
        ];
        $select = ['bl.*','m.nickname','m.name','m.membership_number'];
        $params['sort_field'] = 'bl.createtime';
        $params['sort_direct'] = 'desc';

        //排序字段
        $order_columns = ['bl.balance_log_id','bl.order_id','m.nickname','m.name','m.membership_number','bl.amount','bl.note','bl.createtime'];
        ...
        $counts = $this->pum->_shard_db()->select('COUNT(balance_log_id) as count')
        ...
        if(is_ajax_request()){
            ...
            //HTML输出
            if( !$this->label_action ) $this->label_action= '储值列表';
