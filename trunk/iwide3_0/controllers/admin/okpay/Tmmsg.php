...
// 		$data ['order_params'] = $model->order_find_des ();
		$data ['order_params'] = array('{PAY_MONEY}'=>'已支付金额','{ORDER_SN}'=>'订单号','{HOTEL}'=>'酒店名称','{PAY_TIME}'=>'支付时间');
		...
		if (! empty ( $data ['tid'] )) {
			...
			if (! empty ( $list )) {
				...
				redirect ( site_url ( 'okpay/tmmsg/index' ) );
			}
		} else {
			$msg_list = $model->get_temp_msg_list ( $this->inter_id, self::ENUM_ORDER_TMG_TYPE );
			$types = $model->get_temp_msg_types ( 'okpay' );
