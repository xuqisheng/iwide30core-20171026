...
class Orders extends MY_Admin_Roomservice
{
    ...
    public $username = '';
    public $dada_host_test = 'http://newopen.qa.imdada.cn'; //测试的域名
    public $dada_host_work = 'http://newopen.imdada.cn'; //正式域名
    ...
    public function index()
    {
        ...
        $shops = $this->db->where(array('inter_id'=>$this->inter_id,'is_delete'=>0,'sale_type'=>3))->get('roomservice_shop')->result_array();
        $shoplist = array();
        if($shops){
        ...
        $base_url = site_url('/take-away/orders/index/');
        ...
        if(!empty($order)){
            ...
            if($status == $orderModel::OS_CONFIRMED){//需要确认
                ...
                if($res){
                    ...
                    //如果是外卖 调用推送配置
                    if ($order['type'] == 3 && $order['shipping_type'] == 2) {
                        //达达配送外卖的话调用
                    ...
                    else if ($order['type'] == 3 && $order['shipping_type'] == 3) {
                        //蜂鸟配送外卖的话调用
                        $dada_res = $this->sendOrderToEle($order);
                        if ($dada_res['errcode'] == 0) {
                            //成功
                            //更改蜂鸟订单状态
                            //$this->update_ele_status($order,90);

                            $note .= " | 推送蜂鸟成功";
                            $return['msg'] .= " | 推送蜂鸟：成功";
                        } else {
                            $note .= " | 推送蜂鸟失败：" . $dada_res['msg'];
                            $return['msg'] .= " | 推送蜂鸟：失败，" . $dada_res['msg'];
                        }
                    }
                ...
                //更新订单状态
                if ($order['order_status'] == $orderModel::OS_SHPPING)
                {
                    $return['errcode'] = 0;
                    $return['msg'] = '订单已配送中';
                }
                ...
                {
                    $this->db->update('roomservice_orders', array('order_status' => $orderModel::OS_SHPPING), array('inter_id' => $this->inter_id, 'order_id' => $order_id,'order_status'=>$orderModel::OS_CONFIRMED));
        ...
        if (($_orders['pay_status'] == 1 && in_array($_orders['pay_way'],array(1,2))) && in_array($_orders['order_status'],array(0,5,10)))
        {
            ...
            if (($orders_item['goods_num'] >= $quantity) && ($orders_item['refund_num'] < $orders_item['goods_num']))
            {
                ...
                if(1 == $_orders['pay_way'])//微信支付
                {
                    ...
                    $refund_result = $this->iwidepay_model->refund($iwidepay_refund,$_orders['order_sn']);
                    if(isset($refund_result['status']) && isset($refund_result['message']) &&$refund_result['status']==2 && $refund_result['message']=='empty') {
                    ...
                    if (is_array($refund_result) && "SUCCESS" == $refund_result['return_code'] && "SUCCESS" == $refund_result['result_code'])
                    {
                        ...
                        $refund_data['refund_sn'] = isset($refund_result['out_refund_no'])?$refund_result['out_refund_no']:'';//就是 refund_sn
    ...
    protected function sendOrderToDada($order = array())
    {
        ...
        $dadaInfo = $this->roomservice_dada_model->get(array('inter_id' => $this->inter_id, 'hotel_id' => $order['hotel_id'], 'status' => 1, 'type' => 1));
        if (empty($dadaInfo)) {
            $return['msg'] = '没有达达配送信息';
            return $return;
        }
     ...
     * @date : 2017-6-13
     */
    protected function sendOrderToEle($order)
    {
        $return = array('errcode' => 1, 'msg' => '');
        //获取蜂鸟配置信息
        $this->load->model('roomservice/roomservice_dada_model');
        $eleInfo = $this->roomservice_dada_model->get(array('status' => 1, 'type' => 2));
        if (empty($eleInfo)) {
            $return['msg'] = '没有蜂鸟配送信息';
            return $return;
        }
        $res = $this->handleOrderEle($order, $eleInfo);
        return $res;
    }
    ...
    private function handleOrderData($order, $dadaInfo, $type = 1)
    {
        ...
        //判断环境
        if (!empty($dadaInfo['api_status']) && $dadaInfo['api_status'] == 2) {
            $this->dada_host = $this->dada_host_work;
            $callback_url = 'http://dc.jinfangka.cn/';
        } else {
            $this->dada_host = $this->dada_host_test;
            $callback_url = 'http://dingfang.liyewl.com/';
        }
        ...
        $data = array(
            ...
            //'callback'=>'index.php/dadareturn/dada_rtn/'.$this->inter_id,//正式
            //'callback'=>'http://true.iwide.cn/index.php/dadareturn/dada_rtn/'.$this->inter_id,//预发布
            'callback' => $callback_url . 'index.php/dadareturn/dada_rtn/' . $this->inter_id,//测试用
        );
     ...
     * @param $eleInfo 配置信息
     */
    protected function handleOrderEle($order, $eleInfo)
    {
        //获取酒店信息
        $this->load->model('hotel/hotel_model');
        $hotel = $this->hotel_model->get_hotel_detail($order['inter_id'], $order['hotel_id']);

        //获取店铺信息
        $shop = $this->db->where(array('shop_id'=>$order['shop_id'],'inter_id'=>$order['inter_id']))->get('roomservice_shop')->row_array();
        //获取商品信息
        $this->load->model('roomservice/roomservice_orders_item_model');
        $order_item = array(
            'order_id' => $order['order_id'],
        ...
        $item_orders = $this->roomservice_orders_item_model->get_order_item($order_item);
        $items_json = array();
        if (!empty($item_orders)) {
            foreach ($item_orders as $value) {
                $items_json[] = array(
                    'item_name' => $value['goods_name'],
                    'item_quantity' => $value['goods_num'],
                    'item_price' => $value['goods_price'],
                    'item_actual_price' => $value['goods_price'],
                    'is_need_package' => 0,
                    'is_agent_purchase' => 0
                );
            }
        ...
        $this->load->library('Dada/EleOpenapi', $eleInfo, 'EleOpenapi');
        $notify_url = $this->EleOpenapi->getCallbackUrl($eleInfo['callback_status']) . 'index.php/eleCallback/notify_url?id='.$this->inter_id;

        $shop_name = !empty($shop) ? $shop['shop_name'] : '';
        //组装数据
        $order_send = array(
            'transport_info' => array(
                'transport_name' => $hotel['name'] .$shop_name,
                'transport_address' => $hotel['address'],
                'transport_longitude' => $hotel['longitude'],
                'transport_latitude' => $hotel['latitude'],
                'position_source' => 2,
                'transport_tel' => $hotel['tel'],
                'transport_remark' => ''
            ),
            'receiver_info' => array(
                'receiver_name' => $order['consignee'],
                'receiver_primary_phone' => $order['phone'],
                'receiver_second_phone' => $order['phone'],
                'receiver_address' => $order['address'],
            ...
            'order_type' => 1, //订单类型 1: 蜂鸟配送
            'order_total_amount' => $order['row_total'],//总订单金额
            'order_actual_amount' => $order['sub_total'],//支付金额
            'order_weight' => 0.01,
            'is_invoiced' => 0,
            'invoice' => '',
            ...
            'goods_count' => 1,
            'is_agent_payment' => 0,//是否需要ele代收 0:否 1:是
            //'require_receive_time' => 1800 * 1000  //注意这是毫秒数
        );

        MYLOG::w('推送蜂鸟请求' . json_encode($order_send), 'take-away/ele');

        $reqEle = $this->EleOpenapi->sendOrder($order_send, '');
        $reqRes = json_decode($reqEle, true);
        //接受成功
        if ($reqRes['code'] == 200) {
            $update_data = array(
                'ele_status' => 90,
                'shipping_type' => 3,//推送订单成功
            );

            $where = array('order_id' => $order['order_id']);
            $update = $this->db->update('roomservice_orders', $update_data, $where);
            ...
            $data['ele_order_status'] = 90;
            $data['ele_update_time'] = time();
            $data['rec_content'] = $reqEle;
            $data['add_time'] = time();

            $this->db->insert('ele_log', $data);
        } else {
            $return['errcode'] = 2;
            $return['msg'] = $reqRes['msg'];
        }
        return $return;
    }
     ...
     * @date : 2017-6-14
     */
    public function eleCancelOrder()
    {
        $id = $this->input->post("oid", true);
        $this->load->helper('appointment');
        //获取订单信息
        ...
        MYLOG::w('取消蜂鸟订单' . json_encode($order), 'take-away/ele');

        $reqEle = $this->EleOpenapi->cancelOrder($order,'');
        $reqRes = json_decode($reqEle, true);
        MYLOG::w('取消蜂鸟订单' . $reqEle, 'take-away/ele');
        //取消成功
        if ($reqRes['code'] == 200)
        {
            //插入推送日志
            ...
            $data['inter_id'] = $order['inter_id'];
            $data['order_sn'] = $order['order_sn'];
            $data['ele_order_status'] = 4;
            $data['ele_update_time'] = time();
            $data['rec_content'] = $reqEle;
            $data['add_time'] = time();
            $data['cancel_from'] = 2;
            $data['cancel_from'] = '商家主动取消';

            $this->db->insert('ele_log', $data);
            ajax_return(1, '取消蜂鸟配送成功');
        }
        else
        {
            ajax_return(0, $reqRes['msg']);
        }
    }
    ...
    //更改蜂鸟配送订单状态
    protected function update_ele_status($order, $ele_status)
    {
        $update = array(
            'ele_status' => $ele_status,
            'shipping_type' => 3,//更改酒店配送 => 蜂鸟配送
        );

        if ($update['ele_status'] == 4)
        {
            $data = array();
            $data['cancel_reason'] = '商家主动取消';
            $data['cancel_from'] = 2;
            $this->db->update('ele_log', $data, array('inter_id' => $order['inter_id'], 'order_sn' => $order['order_sn']));
        }
        $this->db->update('roomservice_orders', $update, array('order_id' => $order['order_id']));
        return $this->db->affected_rows();
    }
    ...
    protected function query_ele()
    {
        ...
        $reqEle = $this->EleOpenapi->queryOrder($order,'');
        $reqRes = json_decode($reqEle, true);
    ...
    public function send_ele()
    {
        ...
        ajax_return($res['errcode'],$res['msg']);
    }
