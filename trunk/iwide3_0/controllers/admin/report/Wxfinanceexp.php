...
class Wxfinanceexp extends MY_Admin {
	...
	{
		return 'report/Wxfinanceexp_model';
	}
...
//            11=>'系统取消',
        ...
        $inter_id_filter = $inter_id_filter." and t1.istatus in (".$istatus_csv.")";   //特定支付状态
		...
		if ($orderid) {
			$inter_id_filter = $inter_id_filter." and (t1.orderid='".$orderid."' or t1.webs_orderid like '%".$orderid."%')";
...
//		}
		...
		if ($hotel_name) {
			$inter_id_filter = $inter_id_filter." and t1.hotel_name='".$hotel_name."'";
		...
		if (strlen($status)>0) {
			$inter_id_filter = $inter_id_filter." and t1.istatus=".$status;
			$condition['istatus'] = $status;
		}
		...
		$sum_price = $this->db->query("select SUM(t3.iprice) as iprice from (".$sql.") as t3 where 1")->result_array();
