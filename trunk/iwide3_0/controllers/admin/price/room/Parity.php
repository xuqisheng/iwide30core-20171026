...
class Parity extends MY_Admin {
	...
	private $verifcode = array(
		...
		'a452839067'=>'xedScEkA4iyZBfOQ',//广州三英温泉度假酒店
		);
	...
	public function showList(){
		...
		*/
		...
		foreach ($result_array_sort as $ch => $vh) {
			...
			// }
			$outinfo = $this->db->query("SELECT * FROM ".$this->db->dbprefix(self::TAB_PWR)." WHERE inter_id='{$vh['inter_id']}' AND hotel_id='{$vh['hotel_id']}' AND adddate='2016-12-05'");
			...
			// }else{
			// MYLOG::w('errno:'.$iwiderooms['result_code'].',error:'.$iwiderooms['errmsg'],'parity');
			// continue;
			// }
			...
			if(0){
				...
				foreach ($iwideroomsarr as $ke => $val) {
					...
					foreach ($mins as $k => $v) {
						...
						foreach ($this->ctriprooms as $kc => $vc) {
							if($vc['nkey']==$v['nkey']){
								foreach ($this->timerooms as $vt) {
									if(!empty($vc['remark'])&&mb_strpos($vc['remark'],$vt)!==false){
										$iszd = true;
										$remarks = $vc['remark'];
										break;
									}
						...
						if(mb_strpos($val['price_name'], '积分')===false&&$this->analysisRoom($v['roomname'],$val['roomname'])){						
							...
							$ok = 1;
							unset($nomins[$k]);
							// MYLOG::w($vh['name'].'=>携程：“'.$v['roomname'].'”，iwide：“'.$val['roomname'].'”，匹配成功','parity');
						}
					}
					if($ok==0){
						...
						// MYLOG::w($vh['name'].'=>携程：“'.$v['roomname'].'”，无匹配','parity');
					}else{
						$isfor = 1;
				...
				if($isfor==0){
					MYLOG::w('该酒店无一匹配，name:'.$vh['name'].'，inter_id:'.$vh['inter_id'].'，hotel_id:'.$vh['hotel_id'],'parity');
				}
	...
	private function getCtripOriginal($nkey){
		...
		// 	}
		// }
		MYLOG::w('获取携程初始房型属性失败，roomname:'.$roomname,'parity');
	}
			...
			if($k=='room'){
				foreach ($v as $key => $value) {
					...
					if(!empty($value['bed'])&&mb_strpos($value['bed'],'/')!==false){
						...
						$prices[] = array('roomname'=>$value['roomname'],'bed'=>$bed,'price'=>ltrim($value['price'],'¥'),'breakfast'=>$value['breakfast'],'nkey'=>$value['nkey'],'remark'=>isset($value['remark'])?$value['remark']:'');
					}
				}
			}
		}
		...
		if($ctripname&&$iwidename){
			...
			if($this->currcname==$this->curriname){
				...
				if(isset($cp['first'])&&isset($iwp['first'])&&isset($cp['refirst'])&&isset($iwp['refirst'])){
					...
					similar_text($this->currcname, $this->curriname,$perc);
					if($perc>=80){
						if($this->str_resort_contrast($this->currcname,$this->curriname)){
							$status = true;
						}else{
							$status = false;
						}
	...
	private function splitRoomName($roomname){
		...
		foreach ($this->roomkeys as $k=>$val) {
			foreach ($val as $rk=>$rkeys) {
				if($k=='second'){
					$key1 = mb_substr($rkeys,0,1,'utf-8');
	...
	public function str_resort_contrast($str1,$str2){
		...
		$restr1 = implode('', $arr1);
		$restr2 = implode('', $arr2);
		if(strtoupper($restr1)==strtoupper($restr2)){
			return true;
		}else{
			return false;
		}
	...
	public function getRoomStateTest(){
		...
		// var_dump($rooms);
		$status = $this->analysisRoom('豪华单人房大床无早','豪华三人房无早');
		var_dump($status);
	}
