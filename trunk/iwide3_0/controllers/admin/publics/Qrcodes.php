...
class Qrcodes extends MY_Admin {
	...
	public function qrcode_list() {
		...
		$keyword = empty($this->uri->segment(4)) ? 'QUERY_ALL' : $this->uri->segment(4);
		...
		$config['base_url']          = site_url("publics/qrcodes/qrcode_list/".$keyword.'/');
		$config['total_rows']        = $this->qrcode_model->get_qrcodes_list($inter_id,null,null,$keyword == 'QUERY_ALL' ? '' : $keyword)->num_rows();
...
// 		$config['use_global_url_suffix'] = TRUE;
		...
		$query      = $this->qrcode_model->get_qrcodes_list($inter_id,$config['per_page'],$page,$keyword == 'QUERY_ALL' ? '' : $keyword);
		$this->load->model ( 'hotel/hotel_model' );
		$hotels = $this->hotel_model->get_hotel_hash ( array('inter_id'=>$inter_id) );
		$view_params= array(
				'pagination' => $this->pagination->create_links(),
				'hotels'     => $hotels,
				'k'     => $keyword == 'QUERY_ALL' ? '' : $keyword,
				'res'        => $query->result()
		);
