...
// 		$data ['order_params'] = $model->order_find_des ();
		$data ['order_params'] = array('{PAY_MONEY}'=>'支付金额','{ORDER_SN}'=>'订单号','{GOODS_INFO}'=>'商品名称','{STATUS}'=>'订单状态');
		...
		if (! empty ( $data ['tid'] )) {
			...
			if (! empty ( $list )) {
				...
				redirect ( site_url ( 'roomsesrvice/tmmsg/index' ) );
			}
		} else {
			$msg_list = $model->get_temp_msg_list ( $this->inter_id, self::ENUM_ORDER_TMG_TYPE );
			$types = $model->get_temp_msg_types ( 'roomservice' );
			...
			if(empty($types)){
				redirect ( site_url ( 'roomservice/tmmsg/index' ) );
				exit;
			}
			$data['types']=$types;
