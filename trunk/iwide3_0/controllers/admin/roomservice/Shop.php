...
class Shop extends MY_Admin_Roomservice {
	...
	public function index(){
       ...
       // $filter['hotel_id'] = empty($this->hotel_id)?'':$this->hotel_id;
        ...
        $base_url = site_url('/roomservice/shop/index/');
        $first_url = site_url('/roomservice/shop/index/').$search_url;
        $suffix = $search_url;
	...
	public function add()
	{
       ...
       // $submit = addslashes($this->input->post('submit'));
        if($post){//add数据
            ...
            if(isset($post['verify_info']) && is_array($post['verify_info'])){//核对信息
                $filter['verify_info'] = implode(',',$post['verify_info']);
            }else{
                $filter['verify_info'] = '';
            }
    ...
    public function edit()
    {
       ...
       // $submit = addslashes($this->input->post('submit'));
        ...
        if(!empty($res)){
            ...
            //核对信息
            if(!empty($res['verify_info'])){
                $res['verify_info'] = explode(',',$res['verify_info']);
            }else{
                $res['verify_info'] = array();
            }
