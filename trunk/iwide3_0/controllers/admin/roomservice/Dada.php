...
class Dada extends MY_Admin_Roomservice {
	...
	public function index(){
       ...
       // $filter['hotel_id'] = empty($this->hotel_id)?'':$this->hotel_id;
        ...
        $base_url = site_url('/roomservice/dada/index/');
        $first_url = site_url('/roomservice/dada/index/').$search_url;
        $suffix = $search_url;
        //分页
	...
	public function add()
	{
       ...
       // $submit = addslashes($this->input->post('submit'));
        if($post){//add数据
            if(empty($post['app_key']) || empty($post['app_secret'])||empty($post['source_id'])||empty($post['shop_no']) ){
            ...
            $filter['app_key'] = !empty($post['app_key'])?$post['app_key']:'';
            $filter['app_secret'] = empty($post['app_secret'])?'':$post['app_secret'];
            $filter['source_id'] = empty($post['source_id'])?'':$post['source_id'];
    ...
    public function edit()
    {
        ...
        if($post&&$id){//update数据
            ...
            if(!empty($post['app_key'])){
                $filter['app_key'] = $post['app_key'];
            }
            if(!empty($post['app_secret'])){
                $filter['app_secret'] = $post['app_secret'];
            }
            if(!empty($post['source_id'])){
                $filter['source_id'] = $post['source_id'];
            }
