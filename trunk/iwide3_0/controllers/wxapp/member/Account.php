 ...
 *
 */
class Account extends MY_Front_Wxapp
{
	...
	public function register()
	{
		...
		$this->out_put_msg ( 1, '',$data,'member/register/register');
	...
	public function registersave()
	{
		...
		if($this->inter_id == "a421641095"){
			...
			if($this->user_session("code") != $data['smspic'] && $data['smspic']==""){

				//$this->session->set_userdata('message', "图片验证码错误！");
				$this->out_put_msg ( 2, "图片验证码错误！");
				exit;

				//redirect('member/account/register');exit;

			}
			if($this->user_session("sms") != $data['sms']){

				//$this->session->set_userdata('message', "短信验证码有误！");
				$this->out_put_msg ( 2, "短信验证码有误！");
				exit;
		...
		if($result) {
			...
			if(isset($result['IsError'])){
				...
				$this->out_put_msg ( 2, $result['errmsg']);
				exit;
				//redirect('member/account/register');exit;
			...
			$this->out_put_msg ( 1, '注册成功');
			//$this->session->set_userdata('message', $result['errmsg']);
		} else {
			//$this->session->set_userdata('message', "注册失败！");
			$this->out_put_msg ( 1, "注册失败！");
			//redirect('member/account/register');exit;
		}
...
// 	    	}
	...
	public function resetpasswordsave(){
		...
		//$new_data['sms'] = $data['sms'];
		//telephone=13560428194&pic_verify=1474&sms=305376&custom5=123456
		$result = $this->imember->updatePassWordin( $openid, $data, $this->inter_id, 0);
		if($result['code']==0) {
			//$this->session->set_userdata('message', $result['errmsg']);
			$this->out_put_msg ( 1, $result['errmsg']);
			//redirect('member/center');
		} else {
			//$this->session->set_userdata('message', $result['errmsg']);
			$this->out_put_msg ( 2, $result['errmsg']);
...
// 		$cardid = 'pcbScjqtEK34RJRq6AUXQKxVUM8c';
	...
	public function seMemberCheck(){
		...
		//http://super8.iwide.cn/index.php/member/center/sendsms?name=&telephone=18102535483&password=&picCode=8697
		//图片验证
		$picCode = $this->input->post('picCode');
		//$picCode = $this->user_session("code");
		if(!empty($picCode)){
			if($this->user_session("code") && ($picCode != $this->user_session("code"))){
				$result = array(
					'status'   => 3,
					'msg'    => '验证码不正确'
				);
				$this->out_put_msg ( 2, '验证码不正确' );
	...
	public function seMemberCheckForResetPassword(){
		$this->load->library('Baseapi/Subaapi_webservice',array('testModel'=>true));
		$suba = new Subaapi_webservice(false);
		$phone = $this->input->post('tel');
		//http://super8.iwide.cn/index.php/member/center/sendsms?name=&telephone=18102535483&password=&picCode=8697
