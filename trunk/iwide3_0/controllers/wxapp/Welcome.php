...
// 		$this->Member_model->consum_point ( 'a449675133', 'ORDERID', 'OPENID', 'POINTUSEDAMOUNT');
	...
	function insert_unionid(){
		...
		foreach($data as $key => $d){
			...
			if($error_num >= 10){
				MYLOG::w("error num more than 10 times","temp_update_unionid","_th_{$inter_id}_{$th_num}");
			...
			if(isset($result['errcode']) && $result['errcode'] == 40003){
				...
				MYLOG::w("openid : {$openid} | 40003","temp_update_unionid","_th_{$inter_id}_{$th_num}");
	...
	function update_unionid(){
		...
		foreach($data as $key => $d){
			...
			if(isset($result['errcode']) && $result['errcode'] == 40003){
				...
				MYLOG::w("openid : {$openid} | 40003","update:temp_update_unionid","_th_{$inter_id}_{$th_num}");
			...
			//存在就更新
			if(isset($temp_data_ext[$result['unionid']])){
				...
				MYLOG::w("openid : {$openid} | OK ","update:temp_update_unionid","_th_{$inter_id}_{$th_num}");
