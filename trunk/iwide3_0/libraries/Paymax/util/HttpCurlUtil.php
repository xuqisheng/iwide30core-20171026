...
namespace Paymax\util;

use Exception;
use Paymax\config\PaymaxConfig;
use Paymax\config\SignConfig;
use Paymax\exception\InvalidResponseException;
use Paymax\sign\RSAUtil;
...
class HttpCurlUtil
{
    ...
    private function setHeader(){
        ...
        $defaultHeader = array(
            'lang:PHP',
            'publisher:Paymax',
            'sdk-version:'.PaymaxConfig::$SDK_VERSION,
            'uname:'.php_uname(),
        ...
        $header = array(
            'Content-Type:application/json; charset='.PaymaxConfig::$CHARSET,
            'nonce:'.$nonce,
            'timestamp:'.$timestamp,
    ...
    private function to_sign_data($method,$body_data=null){
        $_cur_uri = $_cur_uri_query_string = stristr($this->uri,'/v1/');
