<?php
class Yibo_webservice implements IPMS {
	protected $CI;
	...
	public function get_rooms_change($rooms, $idents = array(), $condit = array()) {
		$this->CI->load->model ( 'hotel/pms/Yibo_hotel_model', 'pms' );
	...
	public function add_web_bill($order,$params=array()){
		...
		return $this->CI->pms->add_web_bill ( $order['web_orderid'], $order, $trans_no);
	}
	
	public function cancel_order($inter_id,$params) {
		$this->CI->load->model ( 'hotel/pms/Yibo_hotel_model', 'pms' );
		return $this->CI->pms->cancel_order ( $inter_id,$params);
	}
	...
	public function registerMember($params)
	{
		...
		if(isset($result->Result) && $result->Result==1) {
			...
			return array('code'=>0,'errmsg'=>$result->Message);
		} else {
			return array('code'=>1,'errmsg'=>$result->Message);
		}
	}
    ...
    protected function getPmsMember($openid, $telephone, $password, $inter_id='')
	{
		...
		if(isset($result->Result) && $result->Result==1) {
			...
			$updateParams = array('openid'=>$openid,'bonus'=>$result->Data->Score,'is_login'=>1,'level'=>$this->yibomemberlevel($result->Data->MebType),'last_login_time'=>time());
			$this->getMemberModel()->updateMemberByOpenId($updateParams);
...
// 	public function createMember($params)
	...
	public function updatePassWordin($params) {
		...
		$data ['mobileNo'] = $params[1]['telephone'];
		$data ['oldPwd'] = '';
		$data ['newPwd'] = $params[1]['custom1'];
		$s = $this->curl_post ( $url, $data );
		if ($s->Result == 1) {
			return array('code'=>0,'errmsg'=>"修改成功！");
		} else if ($s->Result == 0) {
			return array('code'=>1,'errmsg'=>$s->Message);
		}else{
...
// 			$postsTr = $params[1];
// 			$url = 'http://h5.100inn.cc/MemberService.asmx/UpdatePwd';
// 			$data ['mobileNo'] = $postsTr[0];
// 			$data ['oldPwd'] = '';
// 			$data ['newPwd'] = $postsTr[2];
// 			$s = $this->curl_post ( $url, $data );
// 			if ($s->Result == 1) {
// 				$result ['s'] = 1;
// 				$result ['errmsg'] = '修改成功';
...
// 	}
	...
	//拦截跳转
	public function header_url_center(){
		return true;
	}
	public function order_checkin_type($params)
	{
		$this->CI->load->model ( 'hotel/pms/Yibo_hotel_model', 'pms' );
		$web_orderid=$params[1]['web_orderid'];
		return $this->CI->pms->order_checkin_type ( $web_orderid );
	}
}