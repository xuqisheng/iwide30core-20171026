...
class Qrcode_model extends MY_Model{
	...
	function get_qrcodes_list($inter_id,$nums = NULL,$offset = NULL,$key = ''){
		...
		$where = "inter_id='{$inter_id}' AND (id LIKE '%{$key}%' OR keyword LIKE '%{$key}%' OR intro LIKE '%{$key}%')";
// 		$db_read->where(array('inter_id'=>$inter_id));
		$db_read->where($where);
// 		$db_read->or_where(['id like'=>"%{$key}%",'keyword like'=>"%{$key}%",'intro like'=>"%{$key}%"]);
