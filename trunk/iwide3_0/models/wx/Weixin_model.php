...
// 				}
	...
	protected function get_redis_val($key){
	...
	protected function transCusService(){
		$msg_content = str_ireplace('<Content><![CDATA[人工客服]]></Content>', '<Content><![CDATA[kf]]></Content>', $this->content);
		$sql = 'INSERT IGNORE INTO iwide_wxapi_custom_msg VALUES (?,?,?,?,?)';
		$this->db->query($sql,[$this->data['MsgId'],$this->data['FromUserName'],date('Y-m-d H:i:s'),$msg_content,$this->data['ToUserName']]);
	...
	// 		$ch = curl_init('https://hotelservice.evergrande.com/WeiChat/TestDemo');
			...
			curl_setopt ( $ch, CURLOPT_POSTFIELDS, $msg_content );
			curl_setopt ( $ch, CURLOPT_NOBODY, true);
			curl_setopt ( $ch, CURLOPT_POST, true );
			curl_setopt ( $ch, CURLOPT_RETURNTRANSFER, true );
			curl_setopt ( $ch, CURLOPT_HTTPHEADER, array('Content-Type: application/xml', 'Content-Length: ' . strlen($msg_content)));
