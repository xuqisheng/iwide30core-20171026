...
class Iwidepay_capital_model extends MY_Model
{
    ...
    {
        $sql = "SELECT SUM(trans_amt) AS amount FROM ".self::TAB_IWIDEPAY_ORDER." WHERE transfer_status IN(1,2)";
    ...
    {
        $sql = "SELECT SUM(trans_amt) AS amount FROM ".self::TAB_IWIDEPAY_ORDER." WHERE transfer_status > 0";
        if (!empty($where_arr['inter_id']) && $where_arr['inter_id'] != 'ALL_PRIVILEGES')
        {
            $sql .= " AND  inter_id = '{$where_arr['inter_id']}'";
        }
