...
class Iwidepay_Transfer_Model extends MY_Model{
    ...
    /**
     * 获取结算信息 BY inter_id,hotel_id,type,handle_date
     * @param string $select
     * @param $where
     * @return
     */
    public function get_inter_settlement($select = '*',$where)
    {
        $res = $this->db_read()->select($select)->where($where)->get(self::TAB_IWIDEPAY_SETTLE)->row_array();
        return $res;
    }
     ...
     * @return mixed
     */
    public function update_settlement($data,$where)
    {
        if (!empty($data))
        {
            foreach ($data as $key => $value)
            {
                $this->db->set($key, $value, $key == 'amount' ? false : true);
            }

            $this->db->where($where);
            $this->db->update(self::TAB_IWIDEPAY_SETTLE);
    ...
    public function get_settlement_info($date='')
    {
        ...
        $sql = "select id,amount,bank,bank_card_no,inter_id,hotel_id,type,handle_date,orig_amount from " . self::TAB_IWIDEPAY_SETTLE . " where handle_date = '{$date}' and status = 0";
        $res = $this->db_read()->query($sql)->result_array();
        return $res;
    }
