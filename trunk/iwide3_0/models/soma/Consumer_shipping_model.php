    ...
    */
    ...
    public function get_list($filter= array(), $inter_id=NULL, $select=array(), $page = array(), $like = array(), $type)
    {
        ...
        $result = $result->limit($limit, $offset)
            ...
            ->get($table_name)
            ->result_array();
//       Log::debug('get_list_sql --'.$this->soma_db_conn_read->last_query());
        ...
        $order_select = array('order_id,saler_id,row_total,row_qty,real_grand_total');
        $order_filter = array();
        if (!empty($order_ids)) $order_filter = array('order_id' => $order_ids);
        $order_list = $this->order_model->getOrderList($inter_id, $order_select, $order_filter);
        $order_map = array();
//        Log::debug('sqljigjgijgi'.$this->soma_db_conn_read->last_query());
        foreach ($order_list as $v) {
            $order_map[$v['order_id']] = array(
                'per_price' => empty($v['row_qty'])?'0.0':number_format($v['row_total']/$v['row_qty'], 2),
        ...
        foreach ($result as &$val) {
            ...
            if (isset($val['order'])) {
                $val['per_price'] = isset($order_map[$val['order_id']]['per_price'])?$order_map[$val['order_id']]['per_price']:null;
...
//        Log::debug('sql00'.$this->soma_db_conn_read->last_query());
        //快递商换中文名
        $express_list = $this->get_express();
        $express_map = array_column($express_list, 'dist_label', 'dist_name');
        ...
        foreach ($order_list as $v) {
            $order_map[$v['order_id']] = array(
                'per_price' => number_format($v['row_total']/$v['row_qty'], 2),
                'real_pay' => $v['real_grand_total'],
                'saler_id' => $v['saler_id']
            );
        }
