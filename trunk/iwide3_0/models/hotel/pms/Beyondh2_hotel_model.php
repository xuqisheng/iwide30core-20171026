...
//			$hour=preg_replace('/\D/s', '', $extra_info['pms_code']);
			$hour=mb_substr($extra_info['pms_code'],4,null,'utf-8');
	...
	public function update_web_order($inter_id, $order, $pms_set){
		...
		if(!empty($web_order)){
			...
			$status = $this->pms_enum('status',$web_status);//$status_arr[$web_status];
			...
			if($web_status == 'InProgress' || $web_status == 'Finish'){
				...
				if(!empty($checkin_list[$web_orderid])){
					...
					foreach($checkin_list[$web_orderid] as $k => $v){
						...
						$istatus = $this->pms_enum('status',$wstatus);//$status_arr[$wstatus];
...
class Beyondh2Api{
	...
	private function beyondh_sign($arr, $timestamp){
		$s = '';
		foreach($arr as $v){
			$a = $this->sign_string($v);
			$s .= $a;
		}
		$s = strtolower($s);
	...
	private function sign_string($val){
		$s = '';
		if(is_null($val)){
			return '';
		}else{
			if($val === false){
				...
				if($val === true){
					...
					if(is_array($val) || is_object($val)){
						...
						if(strtotime($val) && strstr($val, 'T') !== false && $val != 'T'){
							$s .= date('Y-m-d H-i-s', strtotime($val));
						}else{
							if(strstr($val, 'E+')){
