...
class Zhongruan_hotel_model extends CI_Model {
    ...
    function get_web_roomtype($pms_set, $web_price_code, $startdate, $enddate, $params = array()) {
        ...
        if ($recache || $json === false || ! is_array ( $result ) || empty($result['pms_state'])) {
            $result = null;
    ...
    function get_web_roomtype_multi($pms_set, $web_price_code, $startdate, $enddate, $params = array()) {
        ...
        $result = $this->Get_HotelsByBGY ( $pms_set, $sh_hotels );
        if (! empty ( $result )) {
            foreach ( $result as $code => $rt ) {
                if (! empty ( $rt ['rooms'] )) {
                    foreach ( $rt ['rooms'] as $r ) {
                        $min_price = array ();
                        if (! empty ( $r->drt_amt )) {
                            $pms_state [$r->rm_cd] [$code] ['price_name'] = $rt ['code_name'];
                            $pms_state [$r->rm_cd] [$code] ['price_type'] = 'pms';
                            $pms_state [$r->rm_cd] [$code] ['extra_info'] = array (
                                    'type' => 'code',
                                    'pms_code' => $code 
