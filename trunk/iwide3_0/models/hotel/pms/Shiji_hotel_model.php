...
// 					$this->Order_model->handle_order($inter_id, $orderid, 1); // 若pms的订单是即时确认的，执行确认操作，否则省略这一步
	...
	public function add_web_bill($web_orderid, $order, $pms_set, $trans_no = ''){
		...
		if(!empty($pms_auth['pay_code'][$order['paytype']])){
			$pay_params['PaymentCode']=$pms_auth['pay_code'][$order['paytype']];
		}
		
		$res = $this->serv_api->addPaymentGateway($pay_params,['orderid'=>$order['orderid']]);
		if(!empty($res['OrderId'])){
		    if (empty($pms_auth['no_bill_reserv_type'])){
    ...
    //			$remark .= '该订单已使用微信支付，支付单号:' . $trans_no;
    			...
    			$new_resv=empty($pms_auth['reserv_type'][$order['paytype']])?(!empty($pms_auth['reserv_type']['paid']) ? $pms_auth['reserv_type']['paid'] : 'GPP'):$pms_auth['reserv_type'][$order['paytype']];
    			if($new_resv!=$web_order['Reservation_type']['code']){
...
//		$remark = empty($pms_auth['no_remark_rate'])?'微信渠道_金房卡 日房价/' . implode('/', $daily_remark) . ' 总房价 ' . $order['price']:'';
		...
		if(!empty($pms_auth['max_book_num'])&&$pms_auth['max_book_num']>1){
			$room_daily = [];
			$r = 0;
			foreach($order['order_details'] as $v){
				...
				$tmp_daily = [];
				for($i = 0; $i < $count_day; $i++){
					$tmp_daily[] = $real_price[$i] . ' ' . $house_date;
				}
				$room_daily[] = '【房间' . $r . '：' . implode('/', $tmp_daily) . '】';
			}
		}else{
			$room_daily=$daily_remark;
		}
		$remark = empty($pms_auth['no_remark_rate'])?'微信渠道_金房卡 日房价/' . implode('/', $room_daily) . ' 总房价 ' . $order['price']:'';
	    ...
	    $customer_title= empty($pms_auth['customer_title']) ? 'Mm' : $pms_auth['customer_title'];
