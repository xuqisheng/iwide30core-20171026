...
class Member_model extends CI_Model {
	...
	function check_openid_member($inter_id, $openid, $paras = array()) {
        ...
        if (! empty($member->mem_id)){
            $member_session_key=$inter_id . $openid . '_memberinfo';
            $this->session->set_userdata ( $member_session_key,json_encode($member));
        }
        return $member;
	}
	...
	function point_back($inter_id, $openid, $orderid,$order=array()) {
        $this->vid=$this->get_vid($inter_id);
        if(isset($this->vid) && $this->vid==1){
            $params = array();
            if ($inter_id == 'a421641095'){
                $config_data = array(
                        'POINT_REFUND_PARAMS' => 'hotel_web_id'
                );
            }
            if (!empty($config_data['POINT_REFUND_PARAMS'])){
                $point_refund_params = explode(',',$config_data['POINT_REFUND_PARAMS']);
                if (in_array('hotel_web_id', $point_refund_params) && !empty($order['hotel_id'])){
                    $this->load->model('common/Pms_model');
                    if ($hotel_pms_set = $this->Pms_model->get_hotel_pms_set($inter_id,$order['hotel_id'])){
                        $params['extra']['hotel_web_id']=$hotel_pms_set['hotel_web_id'];
                    }
                }
            }
            $this->load->model ( 'hotel/Member_new_model' );
            return $this->Member_new_model->refund($inter_id,$orderid,$params);
        }else{
            $this->load->model ( 'member/Imember' );
...
//		$params['is_pms_reduce']=$this->session->userdata($inter_id.'_point_is_pms_reduce');
		if (!isset($params['is_pms_reduce'])) {
			...
			if (!empty($config_data['POINT_NAME'])){
			    $params['point_name']=$config_data['POINT_NAME'];
			}
		}
		$point_name=self::POINT_NAME;
		if (isset($params['point_name'])){
