...
//            if(!empty($comment_counts))$comment_count = $comment_counts;
	...
	function get_hotel_comments($inter_id, $hotel_id = null, $status = null, $order_by = '', $nums = null, $offset = null,$params=array()) {
        ...
        if(!$new_comment){                     //旧的评论
            ...
            $comments = $this->get_hotel_comments_local($inter_id,$hotel_id,$status, $order_by, $nums, $offset,$params);
            $feedback_hash = array();
            $feedback = array();
            if(!empty($comments)){
                foreach($comments as $comment){
                    $feedback_ids[] = $comment['comment_id'];
                }
                $feedback = $this->get_hotel_feedback($inter_id,$feedback_ids);
            }
            if(!empty($feedback)){                             //酒店回复
...
//         $scores = $this->get_hotel_score_from_redis($inter_id,array($hotel_id),$openid);
    ...
    public function get_hotel_feedback($inter_id){     //酒店回复
        ...
        if(!empty($comment_ids))$db_read->where_in ( 'feedback_comment_id', $comment_ids );
        $db_read->where ( array (
            'inter_id' => $inter_id,
            'type'=>'hotel'));

        $feedback = $db_read->get(self::TAB_HCT)->result_array ();
