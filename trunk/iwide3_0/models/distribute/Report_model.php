...
// 		}
	...
	function get_send_logs($params,$limit=NULL,$offset=0){
		if($params['inter_id'] == 'a421641095'){
			return $this->get_send_logs_2($params,$limit=NULL,$offset=0);
		}
...
WHERE ge.distribute=? AND ga.inter_id=? AND ga.saler>0 AND ga.status != 99";//期限外的绩效不统计 situguanchen 20170411
	...
	function get_send_logs_count($params){
		if($params['inter_id'] == 'a421641095'){
			return $this->get_send_logs_count_2($params);
		}
...
(SELECT ga.inter_id,ga.saler,ga.grade_openid,ga.grade_table,ga.grade_id,ga.grade_id_name,ga.order_amount,ga.grade_total,
	...
	function get_send_logs_2($params,$limit=NULL,$offset=0){
		...
		$sql .= " GROUP BY oi.id) orders INNER JOIN
(SELECT ga.inter_id,ga.saler,ga.grade_id,ga.order_amount,ga.grade_total,ga.hotel_id saler_hotel,
				ga.grade_amount,ga.grade_time,ga.status,ga.grade_amount_rate,ga.grade_rate_type,ga.remark,ga.deliver_batch,
				ga.partner_trade_no,ga.send_time,ga.order_hotel,ga.order_status,ga.fans_hotel,hs.master_dept,hs.name as staff_name
				FROM iwide_distribute_grade_all ga
				LEFT JOIN iwide_hotel_staff hs ON hs.qrcode_id=ga.saler and hs.inter_id = ?

WHERE ga.grade_table='iwide_hotels_order' AND ga.inter_id=? AND ga.saler>0 AND ga.status != 99 ";//期限外的绩效不统计 situguanchen 20170411
		if($is_distributed == 1){
			$sql .= " AND hs.openid != '' ";
		}else{
			$sql .= " AND hs.openid = '' ";
	...
	function get_send_logs_count_2($params){
            $sql = "SELECT count(oiid) nums FROM (SELECT oi.id oiid,o.id,o.hotel_id,o.openid,o.inter_id,o.price,o.name,o.order_time,o.orderid,
              o.paid,o.paytype,oa.web_orderid,
              oi.id sid,oi.room_id,oi.iprice,oi.startdate,oi.enddate,oi.istatus,oi.allprice,oi.roomname,oi.webs_orderid
            FROM iwide_hotel_orders o
            INNER JOIN iwide_hotel_order_additions oa ON o.orderid=oa.orderid AND oa.inter_id=o.inter_id
            LEFT JOIN iwide_hotel_order_items oi ON o.orderid=oi.orderid
		...
		$sql .= " GROUP BY oi.id) orders INNER JOIN
(SELECT ga.inter_id,ga.saler,ga.grade_id,ga.grade_time,ga.status,ga.send_time,ga.order_hotel,ga.order_status,ga.fans_hotel,hs.name as staff_name
				 FROM iwide_distribute_grade_all ga
				LEFT JOIN iwide_hotel_staff hs ON hs.qrcode_id=ga.saler and hs.inter_id = ?
                WHERE ga.grade_table='iwide_hotels_order' AND ga.inter_id=? AND ga.saler>0 AND ga.status != 99";//期限外的绩效不统计 situguanchen 20170411
        if($is_distributed == 1){
...
// echo $this->_db('iwide_rw')->last_query();exit;
	...
	*/
	...
	public function get_hotel_new_add_fans($params,$limit=NULL,$offset=0){
		$inter_id = $params['inter_id'];
		$sql = "select  lg.id,im.mem_card_no,im.create_time as bind_time,lg.openid,lg.event_time,lg.event,lg.source as saler,ihs.hotel_name,ihs.name,ihs.hotel_id  
...
on lg.source=ihs.qrcode_id and lg.inter_id=ihs.inter_id where lg.inter_id='{$inter_id}'  and lg.source>0 group by lg.openid order by lg.event_time desc
) as tmp left join 
(
