<?php
class Roomservice_Shop_Model extends MY_Model{
     ...
     * @return  array 店铺信息
     */
    public function get_shop_info($shop_id,$select = '*')
    {
        $sql = "SELECT {$select} FROM iwide_roomservice_shop WHERE shop_id = ?";
        $data = $this->_db('iwide_r1')->query($sql, array($shop_id))->row_array();
        return $data;
    }
     ...
     * 根据条件获取店铺信息
     * */
    public function get($filter = array(),$count='one',$select = '*'){
        $db = $this->_db('iwide_r1');
        $db->select($select);
        $db->where($filter);
        $shop = $db->get('roomservice_shop');
        if($count == 'one'){
