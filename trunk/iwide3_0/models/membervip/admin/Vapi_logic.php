...
class Vapi_logic extends MY_Model_Member
{
    ...
    public function coupon_task_tag($inter_id = '', $task_id = 0)
    {
        ...
        $this->tag_data_group['input_tag'] = array(
            ...
            'temp_title'     => array(
                'name'  => '模版消息',
                'field' => !empty($temp_conf['temp_title_field']) ? $temp_conf['temp_title_field'] : 'interest_account',
                'value' => !empty($temp_conf['temp_title_value']) ? $temp_conf['temp_title_value'] : '会员权益到账通知',
            ),
            'temp_title_ext' => array(
                'interest_account'  => '会员权益到账通知',
                'coupon_expiration' => '优惠券到期提醒',
                'membership_review' => '会员资料审核提醒',
                'service_status'    => '服务状态提醒',
            ),
            'temp_id'        => array(
                'name'  => '模版ID',
    ...
    public function get_temp_conf($inter_id = '', $type = 0)
    {
        $where = array(
            'inter_id' => $inter_id,
            'type'     => $type,
        );
        $info = $this->_shard_db()->select('id')->where($where)->get('template_conf')->row_array();
        return $info;
    }

    public function save_temp_conf($inter_id = '', $temp_conf = array())
    {
        if (empty($temp_conf) or empty($inter_id) or empty($temp_conf['temp_title_field'])) {
            return false;
        }

        $type  = $temp_conf['temp_title_field'];
        $where = array(
            'inter_id' => $inter_id,
            'type'     => $type,
        );
        ...
        if (!empty($info)) {
            $save_data = array(
                'conf'   => @json_encode($temp_conf),
                'status' => 1,
            );
            $where['id'] = $info['id'];
            $result      = $this->_shard_db(true)->where($where)->set($save_data)->update('template_conf');
        } else {
            $save_data = array(
                'type'       => $type,
                'conf'       => @json_encode($temp_conf),
            ...
            $result = $this->_shard_db(true)->set($save_data)->insert('template_conf');
        }
        return $result;
    }
}
