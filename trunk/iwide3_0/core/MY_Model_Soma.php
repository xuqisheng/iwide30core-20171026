...
//        $this->langDir = $lang;
        ...
        if(!property_exists($CI, 'soma_db_conn') || empty($CI->soma_db_conn))
        {
            ...
            $this->db_conn = $this->soma_db_conn;
        } else {
            $this->db_conn = $this->soma_db_conn;
        }
        if(!property_exists($CI, 'soma_db_conn_read') || empty($CI->soma_db_conn_read))
        {
            ...
            $this->db_conn_read = $this->soma_db_conn_read;
        } else {
            $this->db_conn_read = $this->soma_db_conn_read;
        }
...
// var_dump( $over, $not_over );die;
    ...
    public function get($field, $value, $returnFields = '*', $options = array())
    {
        ...
        if(isset($options['groupBy'])){
            $db->group_by($options['groupBy']);
        }

        if ($debug) {
            $result = $db->select($returnFields)->order_by($orderBy)->limit($limit, $offset)->get_compiled_select($table_name);
        } else {
            $result = $db->select($returnFields)->order_by($orderBy)->limit($limit, $offset)->get($table_name)->result_array();
        }
