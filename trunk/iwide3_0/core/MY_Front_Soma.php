...
class MY_Front_Soma extends MY_Front
{
     ...
     * @var array
     */
    protected $idistributInterId = ['a450089706', 'a490782373'];
    ...
    protected function changeViewData(Array $arr)
    {
        if (!empty($arr)) {
            ...
            if (isset($arr['js_api_list'])  && is_array($arr['js_api_list'])) {
                $js_api_list = '';
                foreach ($arr['js_api_list'] as $v) {
                    $js_api_list .= "'{$v}',";
                }
                $js_api_list = substr($js_api_list, 0, -1);
            ...
            if (isset($arr['js_menu_show'])  && is_array($arr['js_menu_show'])) {
                $js_menu_show = '';
                foreach ($arr['js_menu_show'] as $v) {
                    $js_menu_show .= "'{$v}',";
                }
                $js_menu_show = substr($js_menu_show, 0, -1);
                $this->footerDatas['js_menu_show'] = $js_menu_show;
            }

            if (isset($arr['js_menu_hide'])  && is_array($arr['js_menu_hide'])) {
                $js_menu_hide = '';
                foreach ($arr['js_menu_hide'] as $v) {
                    $js_menu_hide .= "'{$v}',";
                }
                $js_menu_hide = substr($js_menu_hide, 0, -1);
                $this->footerDatas['js_menu_hide'] = implode(",", $js_menu_hide);
            }
        }
