...
<a href="<?php echo Hotel_base::inst()->get_url($fa['ad_link'],array(),TRUE);?>" class="pad3 bg_fff bd h24 martop color_main" style="display:block"><em class="iconfont"><?php if(!strstr($fa['ad_img'],"http")) echo $fa['ad_img'];?></em><?php echo $fa['des'];?></a>
<?php }}?>
        ...
        <div class="item_foot"  style="display:none;">
            ...
            <?php if(!empty($r['state_info'])){ foreach($r['state_info'] as $si){?>
            	<div class="webkitbox justify pad3">
                    ...
                    <div>
                    <?php if ($si['price_code']!=1){?>
                    <?php if (!empty($si['total_point'])){?>
                        <?php if($si['book_status']=='available'){?>
                        <?php if ($member->bonus>=$si['total_point']){?>
	                        <span style="float:right" onClick="pay(this,event)" room_id="<?php echo $r['room_info']['room_id'];?>" price_code="<?php echo $si['price_code'];?>" price_type="<?php echo $si['price_type'];?>"><?php 
							if (isset($si['condition']['pre_pay'])&&$si['condition']['pre_pay']==1){
							 	echo '<span pre class="color_minor"><div class="bg_minor">兑换</div><div class="h18">预付</div></span>';
							}
							 else{
							 	echo '<span now class="bg_minor">兑换</span>';
							 }
						?></span>
						<?php } else{?>
								<?php if ($member->is_login=='t'&&$member->member_mode==2){ $point_not_enough=1;?>
									<span style="float:right" now class="bg_999">兑换</span>
								<?php }else{if (isset($_SERVER['HTTPS'])&&strtolower($_SERVER['HTTPS'])=='on')$scheme='https://';else $scheme='http://';?>
		                        	<a href="<?php echo site_url('membervip/login').'?id='.$inter_id.'&redir='.urlencode($scheme.$_SERVER ['HTTP_HOST'] . $_SERVER ['REQUEST_URI']);?>"><span style="float:right" now class="bg_minor ">登录</span></a>
								<?php }?>
							<?php }?>                    
                        <?php } else{?>
                        	<span style="float:right" now class="bg_999">满房</span>
                        <?php }?>
                    <?php }else{?>
                    	<?php if($si['book_status']=='available'){?>
                        <span style="float:right" onClick="pay(this,event)" room_id="<?php echo $r['room_info']['room_id'];?>" price_code="<?php echo $si['price_code'];?>" price_type="<?php echo $si['price_type'];?>"><?php 
						if (isset($si['condition']['pre_pay'])&&$si['condition']['pre_pay']==1){
							echo '<span pre class="color_minor"><div class="bg_minor">订房</div><div class="h18">预付</div></span>';
						}
						 else{
						 	echo '<span now class="bg_minor">订房</span>';
						 }?></span>                    
                        <?php } else{?>
                        <span style="float:right" now class="bg_999">满房</span>
                        <?php }?>
                    <?php }}?>
                        ...
                        <div class="color_minor">
                        <?php if ($si['price_code']==1){?>
                        	<del>¥<?php echo floatval($si['allprice']);?></del>
                        	<?php }else{?>
							<?php if(!empty($si['total_point'])){ ?>
							<p><?php echo $si['total_point'];?>积分</p><?php if (!empty($point_not_enough)){$point_not_enough=0;?><p class='color_999 h20'>积分不足</p><?php }?>
							<?php }else{ ?>
                        	<p>¥<?php echo floatval($si['allprice']);?></p>
                        	<?php if(strpos($si['allprice'],',')!==false){?>
                        	<p class="h18">首晚价</p>
                        	<?php }?>
                        	<?php } ?>
                        	<?php }?>
...
<div class="h28 bg_fff martop" style="padding:3%;"><?php echo $foot_ads['title']?></div>
...
<div class="bg_fff" style='display:none'>
	...
	$.post('<?php echo Hotel_base::inst()->get_url("RETURN_MORE_ROOM");?>',{
		...
		protrol_code:$('#protrol_code').val()
			...
			$.each(data.rooms,function(i,n){
				...
				if(n.top_price!=undefined&&n.top_price!=''&&0){
				...
				if(n.state_info!=''){
					...
					<?php if ($member->is_login=='t'&&$member->member_mode==2){?>
						is_login=true;
					<?php }?>
					...
					$.each(n.state_info,function(sk,sn){
						...
						temp+='<p class="h24 color_999">'+sn.des+'</p></div><div>';
						if(sn.price_code!=1){
							if(sn.total_point!=undefined){
								if(sn.book_status=='available'){
									if(bonus>=sn.total_point){
										temp+='<span style="float:right" onclick="pay(this,event)" room_id="'+n.room_info.room_id+'"';
										temp+=' price_type="'+sn.price_type+'"' 
										temp+=' price_code="'+sn.price_code+'">';
										if(sn.condition&&sn.condition.pre_pay==1)
											temp+='<span pre class="color_minor"><div class="bg_minor">兑换</div><div class="h18">预付</div></span>';
										else 
											temp+='<span now class="bg_minor">兑换</span>';
										temp+='</span>';
									}else{
										if(is_login){
											point_not_enough=1;
											temp+='<span style="float:right" now class="bg_999">兑换</span>';
										}
										else
											temp+='<a href="<?php if (isset($_SERVER['HTTPS'])&&strtolower($_SERVER['HTTPS'])=='on')$scheme='https://';else $scheme='http://'; echo site_url('membervip/login').'?id='.$inter_id.'&redir='.urlencode($scheme.$_SERVER ['HTTP_HOST'] . $_SERVER ['REQUEST_URI']);?>"><span style="float:right" now class="bg_minor">登录</span></a>';
									}
								}
								else
									temp+='<span style="float:right" now class="bg_999">满房</span>';
							}
							else{
								if(sn.book_status=='available'){
									temp+='<span style="float:right" onclick="pay(this,event)" room_id="'+n.room_info.room_id+'"';
									...
									temp+=' price_code="'+sn.price_code+'">';
									if(sn.condition&&sn.condition.pre_pay==1)
										temp+='<span pre class="color_minor"><div class="bg_minor">订房</div><div class="h18">预付</div></span>';
									else 
										temp+='<span now class="bg_minor">订房</span>';
									temp+='</span>';
								}
								else
									temp+='<span style="float:right" now class="bg_999">满房</span>';
						...
						temp+='<div class="color_minor">';
						if(sn.price_code==1){
							temp+='<del>¥'+parseFloat(sn.allprice)+'</del>';
						}else{
							if(sn.total_point!=undefined){
								temp+='<p>'+parseFloat(sn.total_point)+'积分</p>';
								if(point_not_enough!=0){
									temp+="<p class='color_999 h20'>积分不足</p>";
									point_not_enough=0;
								}
							}else{
  								temp+='<p>¥'+parseFloat(sn.allprice)+'</p>';
  								if(sn.allprice.indexOf(',')>0){
  									temp+='<p class="h18">首晚价</p>';
  								}
							}
						}
						temp+='</div></div></div>';
