...
<!--        <form action="<?php /*echo site_url('roomservice/goods/add')*/?>" method="post" enctype="multipart/form-data" accept-charset="utf-8" class="form-horizontal">
-->
        <?php if(isset($ids)){?>
        <?php echo form_open( site_url('roomservice/goods/edit?ids='.$ids), array('class'=>'form-horizontal','enctype'=>'multipart/form-data' ), array() ); ?>
        <?php }else{?>
        <?php echo form_open( site_url('roomservice/goods/add'), array('class'=>'form-horizontal','enctype'=>'multipart/form-data' ), array() ); ?>
<?php }?>
...
--><script>
    ...
    $(function() {
        ...
        $('#shop_id').change(function(){
            ...
            var url = '<?php echo site_url('roomservice/goods_group/ajax_get_group_info')?>';
            $.post(url,{
                'shop_id':value,
	...
	if(SpecData.spec_name.length>0){
		...
		for(var i =0;i <SpecData.spec_type.length;i++){
			...
			for(var j = 0;j<SpecData.spec_name[i].length;j++){
				...
				if(SpecData.spec_name[i][j].length>=12)$('.addNewSpec',clone).hide();
			}
		}
	}
}())
...
function addNewSpec(){
	...
	if($(this).siblings('.diySpec').length>=11){
		$(this).hide();
	}
	$(this).before(add);
