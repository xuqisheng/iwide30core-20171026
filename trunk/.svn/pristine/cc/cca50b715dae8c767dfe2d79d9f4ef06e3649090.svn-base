...
<div class="form_list">
   ...
   <?php if(!empty($member->mem_id)&&!empty($point_consum_rate)&&empty($first_state['bonus_condition']['no_part_bonus'])){?>
	<div class="item">
    	...
    	placeholder="最多可用<?php echo $point_consum_set['max_use'];?>积分，最多可抵<?php echo $point_consum_set['max_use']*$point_consum_rate; ?>元" 
    	<?php }else{?>
        placeholder="共<?php echo $member->bonus;?>积分，最多可抵<?php echo $member->bonus*$point_consum_rate; ?>元"
        <?php }?> />
    </div>
   <?php } else{?>
	<div class="item" style="display:none">
    	<span>积分抵用</span>
        <input type="tel" id='bonus' name='bonus' placeholder="" />
    </div>
    <?php }?>
...
<div class="form_list">
	<div class="item pay_list">
        ...
        <ul><?php if(!empty($pay_ways)) foreach($pay_ways as $k=>$pw){?>
            <li pay_type='<?php echo $pw->pay_type;?>' <?php if (!empty($pw->disable))echo 'abled="0"';else echo 'abled="1"';?> class="pay_way <?php if($k==0){?>ischeck<?php }?> <?php if (!empty($pw->disable))echo 'gray'?>"><em class="ui_ico ui_ico13"></em><span><?php echo $pw->pay_name;?><?php if($pw->pay_type=='balance') { ?>(<?php echo $member->balance;?>元)<?php }
else if($pw->pay_type=='point') { $valid_point=1;?>(<?php if (!empty($pw->des))echo $pw->des;else echo $pw->point_need.'/'.$member->bonus;?>)<?php }?></span></li>
            <?php }?>
            ...
            <?php if(empty($valid_point)){?>
           		<li id="point_pay_way" style="display: none" pay_type="point" abled="0" class="pay_way"><em class="ui_ico ui_ico13"></em><span>积分支付</span></li>
            <?php }?>
        </ul>
    </div>
</div>
			...
			if(data.s==1&&data.consum_rate>0){
				if(part_bonus_set!=data.part_set){
					...
					if(part_bonus_set['max_use']!=undefined)
						$("#bonus").attr('placeholder',"最多可用"+part_bonus_set['max_use']+"积分，最多可抵"+part_bonus_set['max_use']*data.consum_rate+"元");
					else{
						var my_bonus=<?php echo empty($member->bonus)?0:$member->bonus;?>;
						var ex_money=my_bonus*data.consum_rate;
						$("#bonus").attr('placeholder',"共"+my_bonus+"积分，最多可抵"+ex_money+"元");
					}
				}
			}else{
				$("#bonus").parent().hide();
