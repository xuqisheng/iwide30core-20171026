...
<div class="wrapper">
            ...
            <div class="content-wrapper" style="min-width:1130px;" >
                ...
                <div class="contents">
                    <div class="hottel_name ">
                        ...
                        <div class="input_txt">
                            <form action="<?php echo site_url('eat-in/goods/index');?>" method="get">
                            ...
                            <input type="submit" class="bg_ff9900 color_fff" value="检索"/>&nbsp;&nbsp;&nbsp;
                             <a href="<?php echo site_url('eat-in/goods/add')?>">新增</a>
                            </form>
                        </div>
                    </div>
                            ...
                            <?php if(!empty($res)){?>
                                <?php foreach ($res as $r){?>
                                    <tr class=" form_con">
                                        ...
                                        <td><a class="color_72afd2" href="/index.php/eat-in/goods/edit?ids=<?php echo $r['goods_id'];?>">编辑</a>
    ...
    <script>
        function sale_status(shop_id,goods_id,status){
            ...
            if(shop_id != '' && goods_id != ''){
                $.post('<?php echo site_url('eat-in/goods/sale_status');?>',{
                    'shop_id':shop_id,
                    'goods_id':goods_id,
                    'status':status,
        ...
        function change_group(shop_id){
            if(shop_id != '' ){
                $.post('<?php echo site_url('eat-in/goods_group/ajax_get_group_info');?>',{
                    'shop_id':shop_id,
                    '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'
                },function(res){
                    if(res.errcode == 0){
