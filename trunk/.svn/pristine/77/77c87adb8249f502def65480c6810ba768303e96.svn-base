          ...
          <div class="row">
            <div class="col-xs-12">
              ...
              <!-- 
                ...
                <div class="row">
                	<div class="col-sm-12">
                	<?php echo form_open('hotel/hotel_report/rooms_sales',array('class'=>'form-inline','id'=>'para_form'));?>
                	<div class="form-group" >
                		<label>订单酒店</label><select id='hotel' name='hotel_id' class="form-control input-sm">
                		<option value=""<?php if(empty($hotel_id)):echo ' selected';endif;?>>-- 请选择酒店 --</option>
                	...
                	<div class="form-group">
                		<label>统计日期 </label>
                		<input class="form_datetime form-control input-sm" data-date-format="yyyymmdd" type="text" name="check_date_start" class="form-control input-sm" placeholder="" aria-controls="data-grid" value="<?php echo empty($posts['check_date_start']) ? '' : $posts['check_date_start']?>">
                		<label>至 </label>
                		<input class="form_datetime form-control input-sm" data-date-format="yyyymmdd" type="text" name="check_date_end" class="form-control input-sm" placeholder="" aria-controls="data-grid" value="<?php echo empty($posts['check_date_end']) ? '' : $posts['check_date_end']?>">
                	</div>
                	<div class="btn-group">
                	...
                	<div class="btn-group">
                		<a class="btn btn-sm bg-green" href="javascript:void(0);" onclick='out_port()'>导出</a>
                	</div>
                	</div>
                	</form>
                	</div>
...
function out_port(){
	location.href='<?php echo site_url("hotel/hotel_report/ext_rooms_sales").'?'?>'+$('#para_form').serialize();
}
...
// 	  modal.find('.modal-body input').val(recipient)
	...
	$.getJSON('<?php echo site_url("hotel/hotel_report/get_cofigs?ctyp=ROOMS_SALES")?>',function(data){
...
$('#set_btn_save').click(function(){
	$.post('<?php echo site_url("hotel/hotel_report/save_cofigs?ctyp=ROOMS_SALES")?>',$("#setting_form").serialize(),function(data){
		if(data == 'success'){
