    ...
    <div class="head_income_item is_cur">
        ...
        <div class="income_item_detail">
            <div class="webkitbox _1">
                ...
                <div class="co_aaa"><p style="text-align: left">已发￥<?php echo $send?></p><p style="text-align: left">待发￥<?php echo $unsend?></p></div>
            </div>
            <div class="webkitbox _2">
                <p>核定收益笔数</p><p><?php echo $grades_times?></p>
                <div class="h4 co_aaa"><p style="text-align: left">已发<?php echo $send_times?>笔</p><p style="text-align: left">待发<?php echo $unsend_times?>笔</p></div>
            </div>
        </div>
    </div>
...
<div class="_block bg_fff content_income">
	<div class="h3 fill1"><?php echo isset($grades_types[$log->grade_table]) ? $grades_types[$log->grade_table] : '其他'?>:<?php echo empty($hotels[$log->order_hotel]) ? '' : $hotels[$log->order_hotel]?>-<?php echo substr($log->grade_table ,0,10)=='iwide_soma'?substr_replace($log->product,'****',-7,4):$log->product?><?php if($log->grade_table == 'iwide_fans_sub_log'):echo '&nbsp;'.mb_substr($log->nickname, 0, 1). str_repeat('*', 2);endif;?></div>
    <div class="webkitbox">
    	<p>
        	<span class="co_aaa fill2"><?php echo empty($log->order_time) ? '--' : date('Y.m.d H:i:s',strtotime($log->order_time))?></span>
        	<span class="fill3" <?php if($log->grade_amount > 0 || $log->grade_table == 'iwide_distribute_group_member'||$log->grade_table == 'iwide_firstorder_reward'||$log->grade_table == 'iwide_tips_orders'):?>
        ...
        <p style="text-align:right;">
        	<?php if($log->grade_amount > 0 || $log->grade_table == 'iwide_distribute_group_member' || $log->grade_table == 'iwide_firstorder_reward'||$log->grade_table == 'iwide_tips_orders'):?><span class="h0 fill4" <?php if($log->grade_table == 'iwide_hotels_order' && $log->status == 3): 
    ...
    <div class="_block h4 co_aaa" style="display:none">
        <div class="fill5"><?php echo isset($grades_types[$log->grade_table]) ? $grades_types[$log->grade_table] : '其他'?>:<?php echo empty($hotels[$log->order_hotel]) ? '' : $hotels[$log->order_hotel]?>-<?php echo substr($log->grade_table ,0,10)=='iwide_soma'?substr_replace($log->product,'****',-7,4):$log->product?></div>
        ...
        <div><p>关注粉丝：<?php echo '&nbsp;'.mb_substr($log->nickname, 0, 1). str_repeat('*', 2);?></p></div>
            ...
            <div>
                ...
                <p>交易类型：<?php echo isset($grades_types[$log->grade_table]) ? $grades_types[$log->grade_table] : '其他'?></p>
            </div>
            <div class="marL3">
                <p>核定时间：<?php echo empty($log->grade_time)?'--':date('Y.m.d H:i:s',strtotime($log->grade_time))?></p>
            </div>
            </div>
        <?php elseif($log->grade_table == 'iwide_tips_orders'):?> 
        <div class="webkitbox mar3">
            <div>
                <p>订单编号：<span class="fill6"><?php echo $log->order_id?></span></p>
                <p>打赏用户：<span class="fill7"><?php echo ($log->grade_table == 'iwide_distribute_group_member' || $log->grade_table == 'iwide_firstorder_reward')?'--':(mb_substr($log->nickname, 0, 1). str_repeat('*', 2));?></span></p>
                <p>打赏状态：<?php echo empty($o_sts[$log->grade_table][$log->order_status]) ? '' : $o_sts[$log->grade_table][$log->order_status]?></p>
            </div>
            <div class="marL3">
                <p>核定时间：<span class="fill8"><?php echo empty($log->grade_time)?'--':date('Y.m.d H:i:s',strtotime($log->grade_time))?></span></p>
                <p>交易类型：<?php echo isset($grades_types[$log->grade_table]) ? $grades_types[$log->grade_table] : '其他'?></p>
                <p>打赏金额：￥<span class="fill9"><?php echo ($log->grade_table == 'iwide_distribute_group_member' ||$log->grade_table == 'iwide_firstorder_reward')?'--':$log->order_amount?></span></p>
            </div>
        </div>
    	<?php else:?>
    	<div class="webkitbox mar3">
            <div>
                <p>订单编号：<span class="fill6"><?php echo substr($log->grade_table ,0,10)=='iwide_soma'?substr_replace($log->order_id,'****',-7,4):$log->order_id?></span></p>
                <p>交易粉丝：<span class="fill7"><?php echo ($log->grade_table == 'iwide_distribute_group_member' || $log->grade_table == 'iwide_firstorder_reward')?'--':(mb_substr($log->nickname, 0, 1). str_repeat('*', 2));?></span></p>
                <p>交易状态：<?php echo empty($o_sts[$log->grade_table][$log->order_status]) ? '' : $o_sts[$log->grade_table][$log->order_status]?></p>
            </div>
            <div class="marL3">
                <p>核定时间：<span class="fill8"><?php echo empty($log->grade_time)?'--':date('Y.m.d H:i:s',strtotime($log->grade_time))?></span></p>
                <p>交易类型：<?php echo isset($grades_types[$log->grade_table]) ? $grades_types[$log->grade_table] : '其他'?></p>
                <p>交易金额：￥<span class="fill9"><?php echo ($log->grade_table == 'iwide_distribute_group_member' ||$log->grade_table == 'iwide_firstorder_reward')?'--':$log->grade_amount?></span></p>
            </div>
        </div>
        <?php endif;?>
    </div>
...
<div class="foot_fixed">
	<div class="_block btn_list webkitbox">
	<a href="<?php echo site_url('distribute/dis_v1/incomes')?>?id=<?php echo $inter_id?>&g_typ=ALL"<?php if($g_typ == 'ALL'):?> class="link_style"<?php endif;?>>收益详情</a>
	<a href="<?php echo site_url('distribute/dis_v1/incomes')?>?id=<?php echo $inter_id?>&g_typ=OLD"<?php if($g_typ == 'OLD'):?> class="link_style"<?php endif;?>>已发放收益</a>
	<a href="<?php echo site_url('distribute/dis_v1/incomes')?>?id=<?php echo $inter_id?>&g_typ=NEW"<?php if($g_typ == 'NEW'):?> class="link_style"<?php endif;?>>未发放收益</a>
	<a href="<?php echo site_url('distribute/dis_v1/incomes')?>?id=<?php echo $inter_id?>&g_typ=PRE"<?php if($g_typ == 'PRE'):?> class="link_style"<?php endif;?>>未核定收益</a>
    </div>
</div>
