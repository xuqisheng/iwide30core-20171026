...
class Consumer extends MY_Front_Soma_Wxapp {
...
// var_dump( $item );die;
	...
	public function package_usage()
	{
        ...
        $arr = @unserialize($item['compose']);
        $compose = array();
        foreach ($arr as $k => $v) {
        	if($v['content'] != "" && $v['num'] != 0) {
        		$compose[] = $v;
        	}
        }
...
// $inter_id = 'a450089706';
					$type = $MessageWxtempTemplateModel::TEMPLATE_CONSUMER_SUCCESS;// 消费成功
					$templateInfo = $MessageWxtempTemplateModel->get_template_detail_byType( $type, $inter_id );
					if( $templateInfo ){
						$template_id = $templateInfo['template_id'];
						$array = array();
						$array['name'] = isset( $result['data'][0]['name'] ) ? $result['data'][0]['name'] : NULL;
						$array['order_id'] = isset( $result['data'][0]['order_id'] ) ? $result['data'][0]['order_id'] : NULL;
						$array['product_id'] = isset( $result['data'][0]['product_id'] ) ? $result['data'][0]['product_id'] : NULL;
						$array['consumer_code'] = $code;
