...
class Qrcodelist extends MY_Admin_Roomservice {
	...
	public function index(){
        ...
        $base_url = site_url('roomservice/qrcodelist/index/');
        //获取公众号下的酒店
    ...
    public function output_qrcode(){
        ...
        foreach($res as $arr){
                ...
                $arr['type_id'] = iconv('utf-8', 'gb2312',$arr['type_id']);
...
//导入权限
        ...
        //查询interId下的店铺
        $this->load->model('roomservice/roomservice_shop_model');
        $shop_info = $this->roomservice_shop_model->get_list(array('inter_id'=>$this->inter_id,'hotel_id'=>$entity_id,'is_delete'=>0,'sale_type'=>1));
        $shop_list = array();
        if($shop_info){
