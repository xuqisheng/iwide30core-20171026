	...
	<div class="item">
        <div class="webkitbox justify pad3 bg_fff bd_bottom" rid="<?php echo $r['room_info']['room_id']; ?>">
            ...
            <div class="showprice color_main">
                <span class="y color_minor"><?php echo $r['lowest']; ?></span>
			<?php foreach ($r['state_info'] as $si){?>
                ...
                <?php } else{?>
                <span now class="bg_999"><p>满</p></span>
                <?php }?>
            <?php } ?>
            </div>
            <?php }else {?>
	...
	$.post('<?php echo Hotel_base::inst()->get_url("RETURN_MORE_ROOM");?>',{
		...
		protrol_code:$('#protrol_code').val()
			...
			$.each(data.rooms,function(i,n){
				temp+='<div class="item"><div class="webkitbox justify pad3 bg_fff bd_bottom" rid="'+n.room_info.room_id+'"><div class="img"  onclick="show_room_detail(this,event)">';
				temp+='<div class="squareimg"><img src="'+n.room_info.room_img+'"/></div></div>';
				...
				temp+='<p class="backvote">入住返券</p>';
				<?php }?>
				temp+='</div>';
				if(n.state_info!=''){
					temp+='<div class="showprice color_main">';
					temp+='<span class="y color_minor">'+n.lowest+'</span>';
					if(n.state_info!=''){
						$.each(n.state_info,function(sk,sn){
							if(sn.book_status=='available'){
								...
								temp+='<span now class="bg_999"><p>满</p></span>';
						});
					}
					temp+='</div>';
				}
