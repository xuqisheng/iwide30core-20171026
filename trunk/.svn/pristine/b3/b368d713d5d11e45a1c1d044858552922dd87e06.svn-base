...
// 
        ...
        // var_dump($this->datas['gift_list']);exit;
...
// var_dump( $items );die;
    ...
    public function sharing_received()
    {
        ...
        */
        ...
        // 青岛红树林众筹活动，活动过后关闭
        // 1.自己是分销员，覆盖分销员信息
        // 2.自己不是分销员，判断分享给他的那个人是不是分销员
        // 3.分享给他的那个人也不是，继承上一个分销员信息
        if (in_array($this->inter_id, $this->idistributInterId) && time() < strtotime('2017-08-20 23:59:59')) {
            if ($self_saler && $self_saler['typ'] == 'STAFF') {
                $redirect .= '&saler=' . $self_saler['info']['saler'];
                $giveDistributeValue = $self_saler['info']['saler'];
            } else {
                if ($share_saler && $share_saler['typ'] == 'STAFF') {
                    $redirect .= '&saler=' . $share_saler['info']['saler'];
