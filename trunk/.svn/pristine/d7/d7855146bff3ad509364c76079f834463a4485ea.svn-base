...
class Argyle_webservice implements IPMS{
	protected $CI;
	...
	public function get_rooms_change($rooms, $idents = array(), $condit = array()){
		...
		$this->CI->load->model('hotel/pms/Argyle_hotel_model', 'pms');
	...
	/*	public function search_hotel_front($params){
			$this->CI->load->model('hotel/pms/Argyle_hotel_model');
			return $this->CI->Argyle_hotel_ext_model->search_hotel_front($params[0],$params[1],$this->pms_set);
		}*/
