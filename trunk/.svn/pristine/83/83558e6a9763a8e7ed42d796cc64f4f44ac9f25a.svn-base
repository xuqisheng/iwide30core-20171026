...
<header class="order_intro">
    ...
    <div class="datetime"><?php echo date("m月d日",strtotime($startdate));?></div>
...
<div class="form_list">
	<div class="item pay_list">
        ...
        <ul><?php if(!empty($pay_ways)) foreach($pay_ways as $k=>$pw){?>
            <li pay_type='<?php echo $pw->pay_type;?>' abled='1' class="pay_way <?php if($k==0){?>ischeck<?php }?>"><em class="ui_ico ui_ico13"></em><span><?php echo $pw->pay_name;?><?php if($pw->pay_type=='balance') { ?>(<?php echo $member->balance;?>元)<?php }
