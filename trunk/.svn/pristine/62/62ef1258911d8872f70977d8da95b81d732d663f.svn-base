              ...
              <!-- 
                ...
                <div class="row">
                	...
                	<div class="form-group">
                		<label>粉丝所属酒店</label><select name='hotel_id' class="form-control input-sm">
                	...
                	<div class="form-group">
						...
						<div class="form-group">
							<label>所属部门</label> <select class="form-control input-sm" name="dept"><option value="">--全部--</option>
								<?php foreach ($depts as $k=>$v){if(!empty($v->master_dept)){?><option value="<?php echo $v->master_dept;?>"<?php if(isset($posts['dept'])&&$posts['dept'] == $v->master_dept){echo ' selected';}?>><?php echo $v->master_dept;?></option><?php  }}?>
							</select>
						</div>
...
?>                        	<th width="10%" class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="<?php echo $item['name'];?>: activate to sort column ascending"><?php echo $item['name'];?></th>
				...
				$fields[] = $key;
                  ...
                  <div class="row">
	                  <div class="col-sm-5">
	                  	<div class="dataTables_info" id="data-grid_info" role="status" aria-live="polite">共<?=$total?>条<a class="btn btn-sm bg-green" href="<?php echo site_url("distribute/distri_report/ext_fans_room_order2/".$posts['hotel_id'].'_'.$posts['begin_time'].'_'.$posts['end_time'].'_'.$posts['fans_id'].'_'.$posts['saler_name'].'_'.$posts['saler_no'].'_'.$posts['dept'])?>">导出</a></div>
	                  </div>
	                  <div class="col-sm-7">
