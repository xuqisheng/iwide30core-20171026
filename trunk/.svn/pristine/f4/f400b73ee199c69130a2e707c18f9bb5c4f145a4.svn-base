...
class Dis_v1 extends MY_Front {
	...
	function do_reg(){
		$this->load->model('distribute/staff_model');
		$dis_sess_code = $this->session->userdata ( 'dis_sess_code' );
		$post_code = $this->input->post('dis_sess_code',true);
		if(!empty($dis_sess_code) && !empty($post_code)){
			if($dis_sess_code != addslashes($post_code)){
				echo json_encode(array('errmsg'=>'redir','message'=>'请勿提交多次！'));
				die;
			}else{//删掉session
				$this->session->set_userdata(array(
                	'dis_sess_code' => '',
