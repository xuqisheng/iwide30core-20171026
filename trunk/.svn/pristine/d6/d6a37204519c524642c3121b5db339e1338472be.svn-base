...
<div style="color:#92a0ae;">
    <div class="over_x">
        <div class="content-wrapper" style="min-width:1130px;" >
            <div class="banner bg_fff p_0_20">打印机列表</div>
            <div class="contents">
                <div class="hottel_name ">
                        <?php echo $this->session->show_put_msg(); ?>
                    <div class="input_txt">
                        <form id="" action="<?php echo site_url('roomservice/printer/index');?>" class="form-inline" method="get">
                            店铺：<select id="shop"  name="shop_id" class="w_450">
                        ...
                        关键词：<input type="text" name="wd" class="form-control input-sm" placeholder="打印机名" aria-controls="data-grid" value="<?php echo empty($filter['wd']) ? '' : $filter['wd']?>">
                        <input type="submit" class="btn bg_ff9900 color_fff" value="检索"/>&nbsp;&nbsp;&nbsp;
                         <a href="<?php echo site_url('roomservice/printer/add')?>" class="btn btn-sm bg-green">新增打印机
                         </a>
                        </form>
                    </div>
                </div>
                        ...
                        <tr class="bg_f8f9fb form_title">
                           ...
                           <?php if(!empty($res)){
                               foreach($res as $k=>$v) {
                                   ...
                                   <td><?php echo $v['printer_no'] ?></td>
                                   <td><?php echo $v['name'] ?></td>
                                   <?php if ($v['source'] == 1) { ?>
                                       <td><?php echo isset($shops[$v['shop_id']]['shop_name']) ? $shops[$v['shop_id']]['shop_name'] : '' ?></td>
                                   <?php } else if ($v['source'] == 2) { ?>
                                       <td><?php echo isset($okpay_types_arr[$v['shop_id']]['name']) ? $okpay_types_arr[$v['shop_id']]['name'] : '' ?></td>
                                   <?php } else if ($v['source'] == 3) {?>
                                       <td><?php echo isset($shops_dining_room[$v['shop_id']]['shop_name']) ? $shops_dining_room[$v['shop_id']]['shop_name'] : '' ?></td>
                                       <?php
                               }
                                 ?>
                                 <td><?php echo isset($v['show_type'])?$v['show_type']:''?></td>
                                <td><?php echo isset($v['print_status'])?$v['print_status']:''?></td>
                                <td><a class="color_72afd2" href="<?php echo site_url('roomservice/printer/edit?ids='.$v['id'])?>">编辑</a>
                                    <?php if($v['status'] == 1){?>
                                    <a class="color_72afd2" onclick="change_staus('<?php echo $v['id']?>',2)" >停止</a>
                                    <?php }else{?>
                                       <a class="color_72afd2" onclick="change_staus('<?php echo $v['id']?>',1)">运行</a>
                                    <?php }?>
                                 </td>
                            </tr>
                            <?php }}?>
...
// 	  modal.find('.modal-body input').val(recipient)
    ...
    function change_staus(id,status){
        ...
        $.post('<?php echo site_url('roomservice/printer/change_status')?>',{
            'id':id,
            'status':status,
