...
class Ueditor extends MY_Admin_Api {
    ...
    public function __construct() {
		...
		if(empty($inter_id) && !is_array($hidden)){
			exit("非法上传！");
		}
	...
	public function uploadimg(){
        ...
        $info = $this->_do_upload($data, 'upfile','membervip', 'ueditor');
		...
		if ($info) {
			...
			if(strpos($file, "https")===0 || strpos($file, "http")===0){
				...
				$host=($this->is_ssl() ? 'https' : 'http')."://".$_SERVER['HTTP_HOST'];
				$file=$host.$file;
			}
		} else {
			$state = 'FAIL';
	...
	public function get_remote_image(){
		...
		foreach ( $imgUrls as $imgUrl ) {
			...
			$file = "/public/media/ueditor/".$file;
			if(strpos($file, "https")===0 || strpos($file, "http")===0){
	...
	private function _get_remote_image(){
		...
		foreach ( $source as $imgUrl ) {
			...
			//写入文件
			$tmpName = $savePath .$file ;
			$file = "/public/media/ueditor/$date/".$file;
			if(strpos($file, "https")===0 || strpos($file, "http")===0){
	...
	private function _ueditor_upload($config=array()){
		...
		if ($info) {
			...
			if(!empty($info['upfile'])){
				$url=$info['upfile'];
			}else{
				$url = "/public/media/ueditor/$date/".$title;
			}
