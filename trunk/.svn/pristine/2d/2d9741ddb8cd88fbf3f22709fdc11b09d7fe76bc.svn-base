...
<?php if(isset($id)){?>
<?php echo form_open('roomservice/printer/edit?ids='.$id,array('id'=>'tosave','class'=>'form-horizontal','enctype'=>'multipart/form-data','onsubmit'=>'return sub();' ))?>
<?php }else{?>
<?php echo form_open('roomservice/printer/add',array('id'=>'tosave','class'=>'form-horizontal','enctype'=>'multipart/form-data','onsubmit'=>'return sub();' ))?>
<?php }?>
...
<!--                                    2.微信支付订单确认接单后自动打印<br><br>-->
        ...
        <div class="bd_left list_layout">
            <div>
                <div>所属店铺</div>
                <div class="select_input flexgrow">
                    <div class="input"><input placeholder="搜索或下拉选择" value="<?php echo isset($posts['shop_id'])&&isset($shops_dining_room[$posts['shop_id']]['shop_name'])?$shops_dining_room[$posts['shop_id']]['shop_name']:''?>"></div>
                    <?php if(!empty($shops_dining_room)){ ?>
                        <div class="silde_layer bd" id="el_hotel">
                            <?php foreach($shops_dining_room as $hk=>$hv){?>
                                <div data="<?php echo $hv['dining_room_id']?>"><?php echo $hv['shop_name']?></div>
                            <?php }?>
                        </div>
                    <?php }?>
                    <input type="hidden" name="shop_id" value="<?php echo isset($posts['shop_id'])?$posts['shop_id']:''?>">
...
<script>
    function change_source(){
        ...
        location.href = '<?php echo site_url('roomservice/printer/add?s=')?>' + s;
    }
