 ...
 *         
 */
class Verify extends MY_Front_Member_Wxapp
{
    ...
    public function save_verify()
    {
        ...
        if (is_object($ext) && $ext->id > 0) {
            if ($ext->audit == '1') {
                $this->out_put_msg ( 2,'您已经通过审核，无需再次提交');
            }
            // 已经存在记录，继续提交则更新记录
  ...
  function doCurlPostRequest($url, $requestString, $extra = array(), $timeout = 20)
    {
    ...
    {
        $this->load->view('member/zhouji/member_witch');
    }
