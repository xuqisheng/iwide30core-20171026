...
<div class="center pad3 bg_fff">
    ...
    <?php if( isset($killsec) &&  !empty($killsec)): ?>
    	<p class="color_main h20 pad3">
            <em><?php if($show_y_flag): ?>¥<?php endif; ?></em>
            <span class="h30"><?php echo $killsec['killsec_price']; ?></span>
            <span class="btn_void btn_small" style="margin-left:8px">秒杀</span>
        </p>
    <?php endif; ?>
	<p class="h20">市场价：<?php if($show_y_flag): ?>¥<?php endif; ?><?php echo $package['price_package']; ?></p>
...
<div class="whiteblock support_list h24 <?php if($no_attr_flag): ?>Ldn<?php endif; ?>">
    <!--
...
<div class="foot_fixed">
    <div class="bg_fff webkitbox center">
    	...
    	<!-- <a class="h24 bg_main txtclip pad10" >¥99.00 <?php echo $show_name;  ?>购买</a> -->
    ...
    <a id="killsec_btn" class="h24 bg_main txtclip pad10">
        <span class="mask bdradius">活动加载中……</span>
        <span class="text">
    <?php if($show_y_flag): ?>¥<?php endif; ?><?php echo $killsec['killsec_price'];?> <?php 
    ...
    elseif( !empty($groupons) && !$is_expire ): ?>
        ...
        <a href="<?php echo Soma_const_url::inst ()->get_groupon_first_pay(array('act_id'=>$v['act_id'],'id'=>$inter_id));?>" class="h24 bg_main txtclip  pad10"><?php if($show_y_flag): ?>¥<?php endif; ?><?php echo $v['group_price'];?> | <?php echo $v['group_count'];?>人团</a>
	...
	<?php else: ?>
        <?php if( isset( $isTicket ) && !$isTicket ):?>
            <div class="h24 bg_minor txtclip  pad10 select_type" href="<?php echo Soma_const_url::inst ()->get_package_pay(array('pid'=>$_GET['pid'],'id'=>$inter_id));?>"><?php if($show_y_flag): ?>¥<?php endif; ?><?php echo $package['price_package'];?><?php 
                    if($package['type'] == $packageModel::PRODUCT_TYPE_BALANCE) {
                        echo $show_name,'购买';
                    } elseif($package['type'] == $packageModel::PRODUCT_TYPE_POINT) {
                        echo "积分购买";
                ...
                ?></div>
        <?php else: ?>
            <div class="h24 bg_minor txtclip  pad10 select_type" href="<?php echo Soma_const_url::inst()->get_url('*/*/ticket_select_time',array('id'=>$inter_id,'pid'=>$_GET['pid'],'tkid'=>$ticketId,'bType'=>$bType)); ?>"><?php if($show_y_flag): ?>¥<?php endif; ?><?php echo $package['price_package'];?><?php 
                    if($package['type'] == $packageModel::PRODUCT_TYPE_BALANCE) {
                        echo $show_name,'购买';
                    } elseif($package['type'] == $packageModel::PRODUCT_TYPE_POINT) {
                        echo "积分购买";
