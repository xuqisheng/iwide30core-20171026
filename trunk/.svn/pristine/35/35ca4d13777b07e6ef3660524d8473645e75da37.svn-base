            ...
            <div class="content-wrapper" style="min-width:1130px; ">
                ...
                <div class="contents_list bg_fff" style="font-size:13px;margin-top:20px">
                    <div class="classifications display_flex">
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==1)echo 'class="add_active"'?>><a href="<?php echo site_url('take-away/orders/index?order_status=1')?>">待付款</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==2)echo 'class="add_active"'?>><a href="<?php echo site_url('take-away/orders/index?order_status=2')?>">待确认</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==3)echo 'class="add_active"'?>><a href="<?php echo site_url('take-away/orders/index?order_status=3')?>">待配送</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==4)echo 'class="add_active"'?>><a href="<?php echo site_url('take-away/orders/index?order_status=4')?>">配送中</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==5)echo 'class="add_active"'?>><a href="<?php echo site_url('take-away/orders/index?order_status=5')?>">已完成</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==6)echo 'class="add_active"'?>><a href="<?php echo site_url('take-away/orders/index?order_status=6')?>">已取消</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==99)echo 'class="add_active"'?>><a href="<?php echo site_url('take-away/orders/index')?>">所有状态</a></div>
                    </div>
                </div>
                <div style="padding: 10px;">
                    <form class="form" method='get' id="this_form" action='<?php echo site_url('take-away/orders/index')?>'>
                        ...
                        <div class="border_1 m_t_10 bg_fff">
                            ...
                            <div>
                                ...
                                <?php } elseif ($list['order_status'] == $orderModel::OS_CONFIRMED && $list['pay_status'] == $orderModel::IS_PAYMENT_YES && $list['pay_way'] != 3 || $list['pay_way'] == 3 && $list['order_status'] == $orderModel::OS_CONFIRMED) { ?>
                                    <span>准备中</span><br>
                                    <?php
                                    //达达配送if ($list['shipping_type'] == 2 && $list['type'] == 3 && !in_array($list['dada_status'],array('0','5','7'),true))
                                    if ($list['shipping_type'] == 3 && $list['type'] == 3 && !in_array($list['ele_status'],array('0','4','5'),true))
                                    {
                                    ?>
                                        <span class="">配送</span> &nbsp;|&nbsp;
                                        <span class="">取消</span>
                                ...
                                <?php
                                //达达配送 if ($list['shipping_type'] == 2 && $list['type'] == 3 && !in_array($list['dada_status'],array('0','5','7'),true))
                                if ($list['shipping_type'] == 3 && $list['type'] == 3 && !in_array($list['ele_status'],array('0','4','5'),true))
                                {
                                    ?>
                           ...
                           <!--
                            ...
                            -->
                                <div>
                                    ...
                                    {
                                        ?>
                                        <a class="blue send_ele" data="<?php echo $list['order_id'];?>" href="javascript:;">推送蜂鸟</a>
                                        <?php
                                    }

                                    // 蜂鸟配送状态
                                    else if ($list['ele_status'] == 90) {
                                        ?>
                                        <span class="yellow">待接单</span> |
                                        <a class="blue cancel_ele" data="<?php echo $list['order_id'];?>" href="javascript:;">取消</a>
                                        <?php
                                    } else if ($list['ele_status'] == 0) {
                                        ...
                                        {
                                            ?>
                                            | <a class="blue resend_ele" data="<?php echo $list['order_id'];?>" href="javascript:;">重新发送</a>
                                            <?php
                                        }
                                        ?>
                                            <?php
                                    } else if ($list['ele_status'] == 6) {
                                        echo '<span>已申请取消</span>';
                                    }
                                    else if ($list['ele_status'] == 1) {
                                        ?>
                                        <span>已接单</span><br/><a class="blue cancel_ele" data="<?php echo $list['order_id'];?>" href="javascript:;">取消</a>
                                        <?php
                                    }
                                    else if ($list['ele_status'] == 20) {
                                        ?>
                                         <span>待到店</span><br/><a class="blue cancel_ele" data="<?php echo $list['order_id'];?>" href="javascript:;">取消</a>
                                        <?php
                                    } else if ($list['ele_status'] == 80) {
                                        ?>
                                        <span class="green">已到店</span><br/><a class="blue cancel_ele" data="<?php echo $list['order_id'];?>" href="javascript:;">取消</a>
                                        <?php
                                    } else if ($list['ele_status'] == 2) {
                                        echo '<span class="green">配送中</span>';
                                    } else if ($list['ele_status'] == 3) {
                                        echo '<span class="green">已送达</span>';
                                    } else if ($list['ele_status'] == 4) {
                                    ?>
                                        <span class="">已取消</span>
    ...
    <script>
        ...
        $('.cancel_dd').click(function()
        {
            ...
            $.ajax({
                dataType:"json",
                type:'post',
                url:"<?php echo site_url('/take-away/orders/dadaCancelOrder')?>",
                data:data,
        ...
        $('.cancel_ele').click(function()
        {
            ...
            $.ajax({
                dataType:"json",
                type:'post',
                url:"<?php echo site_url('/take-away/orders/eleCancelOrder')?>",
                data:data,
                success: function(res)
                {
                    alert(res.msg);
        ...
        $('.resend_dd').click(function()
        {
            ...
            $.ajax({
                dataType:"json",
                type:'post',
                url:"<?php echo site_url('/take-away/orders/reSendOrderToDada')?>",
                data:data,
                success: function(res)
                {
                    alert(res.msg);
        ...
        $('.send_dd').click(function()
        {
            ...
            $.ajax({
                dataType:"json",
                type:'post',
                url:"<?php echo site_url('/take-away/orders/send_dd')?>",
                data:data,
                success: function(res)
                {
                    alert(res.msg);
        ...
        //发送蜂鸟订单
        $('.send_ele,.resend_ele').click(function()
        {
            if(!window.confirm('确定要更改？'))
            {
                return false;
            ...
            $.ajax({
                dataType:"json",
                type:'post',
                url:"<?php echo site_url('/take-away/orders/send_ele')?>",
                data:data,
                success: function(res)
                {
                    alert(res.msg);
        ...
        $('.confirms_remark').click(function()
        {
            ...
            $.ajax({
                dataType:"json",
                type:'post',
                url:"<?php echo site_url('/take-away/orders/save_remark')?>",
                data:data,
                success: function(res)
                {
                    alert(res.msg);
        ...
        $('.confirms_order').click(function()
        {
            ...
            if(a)
            {
                ...
                $.ajax({
                    dataType:"json",
                    type:'post',
                    url:"<?php echo site_url('/take-away/orders/refund_goods')?>",
                    data:data,
                    beforeSend: function()
                    {
    ...
    <script>
        ...
        function change_status(order_id,status){
            if(order_id){
                ...
                $.post('<?php echo site_url('take-away/orders/update_order_status');?>',{
                    oid:order_id,
                    status:status,
        ...
        function print_order(order_id){
            if(order_id){
                ...
                $.post('<?php echo site_url('take-away/orders/print_order');?>',{
                    oid:order_id,
                    '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'
                },function(data){
                    if(data.errcode==0){
