...
<?php }?>
    ...
    <div class="webkitbox martop input_item">
        ...
        <div>
            ...
            <?php if (!empty($states['item_state'][$od['sub_id']]['can_continue'])){$self_continue=1;?>
            	(房间号:<?php if (!empty($od['room_no'])){?> <?php echo empty($od['room_no'])?'':$od['room_no'];?><?php };echo ','.date('m-d',strtotime($od['enddate'])).'离店';?>)
            <a href="javascript:;" onclick='check_self_continue("<?php echo $od["sub_id"]?>")' class="color_link"> 续住</a>
            <?php }?>
        </div>
    </div>
    <?php }}?>
