...
class Shop extends MY_Admin_Ticket {
	...
	public function index(){
       ...
       // $filter['hotel_id'] = empty($this->hotel_id)?'':$this->hotel_id;
        ...
        $base_url = site_url('/ticket/shop/index/');
        $first_url = site_url('/ticket/shop/index/').$search_url;
        $suffix = $search_url;
	...
	public function add()
	{
       ...
       // $submit = addslashes($this->input->post('submit'));
        if($post){//add数据
            ...
            $filter['status'] = !empty($post['status']) ? $post['status'] : 1;
            ...
            //增加门票字段数据
            $filter['spots_level'] = addslashes(trim($post['spots_level']));
            $filter['outdate_order'] = addslashes(intval($post['outdate_order']));
            $filter['instant_confirm'] = addslashes(intval($post['instant_confirm']));
            $filter['shop_image'] = addslashes(trim($post['shop_image']));
            $filter['shop_profiles'] = addslashes(trim($post['shop_profiles']));
            $filter['advance_book_num'] = addslashes(intval($post['advance_book_num']));
            $filter['time_range'] = trim($post['time_range']);

            //分享内容
            $filter['share_img'] = addslashes(trim($post['share_img']));
            $filter['share_title'] = addslashes(trim($post['share_title']));
            $filter['share_spec'] = addslashes(trim($post['share_spec']));
            ...
            $filter['wait_time'] = !empty($post['wait_time']) ? $post['wait_time'] : 0;
            //优惠信息
