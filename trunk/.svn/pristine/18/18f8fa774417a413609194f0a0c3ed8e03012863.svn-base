			...
			<?php echo form_open( site_url('hotel/orders/item_edit_post'), array('class'=>'form-horizontal','enctype'=>'multipart/form-data' ), array('orderid'=>$list['orderid'],'item_id'=>$list['id']) ); ?>
                <div class="box-body">
                    ...
                    <div class="form-group  has-feedback">
						...
						<div class="col-sm-8">
						<?php if(!empty($day_range)){?>
							<select name='enddate' id='enddate' >
							<?php foreach($day_range as $dr){?>
							<option value='<?php echo $dr;?>'><?php echo date('Y-m-d',strtotime($dr));?></option>
							<?php }?>
							</select>
							<?php } else {?>
                    ...
                    <div class="box-footer ">
						...
						<?php if(!empty($status)){?>
                           ...
                           <?php }?>
                            <button type="button" class="btn btn-info pull-right" onclick="history.go(-1);">返回</button>
                        </div>
                    </div>
                    <!-- /.box-footer -->
                </div>
