...
$buttions.= '<div class="input-daterange input-group" id="datepicker">';
$buttions.= '<span class="input-group-addon">日期从</span><input type="text" class="input-sm form-control" name="start" /><span class="input-group-addon">到</span><input type="text" class="input-sm form-control" name="end" />';
$buttions.= '<span class="input-group-btn"><button type="button" class="btn btn-default btn-sm bg-green pull-right" id="grid-btn-export"><i class="fa fa-sign-out"></i>&nbsp;导出</button></span>';
$buttions.= '</div>';
...
/*有更多的按钮，URL在此定义，id依次编号 id="grid-btn-extra-0-1-2-...*/
...
//
//
$('#grid-btn-export').bind('click', selected, function(){
	var u= '<?php echo EA_const_url::inst()->get_url("*/*/export"); ?>';
	var start= $('input[name=start]').val();
	var end= $('.input-daterange input[name=end]').val();
	if(start.length>0 && end.length>0){
		if(confirm('您确定要导出 "'+ start + '" 到 "'+ start+ '" 数据吗？')){
			var p= '?s='+start+'&e='+end;
			window.open(u+p, 'export');;
		}
	} else{
		alert('请输入导出的开始、结束时间！');
	}
...
//
$('.input-daterange input').datepicker({format:"yyyy-mm-dd", language: "zh-CN",});
