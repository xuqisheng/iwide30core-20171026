  ...
  <div class="content-wrapper">
	<section class="content">
      <div class="row">
        ...
        <div class="col-xs-4">
          <div class="box box-solid">
            <div class="box-header with-border"><i class="fa fa-file-text-o"></i><h3 class="box-title">发放内容</h3></div>
            <div class="box-body">
                <h5>
                  ...
                  <div class="box-body"><h5><?php if (!empty($list['rule_dates']['d']['r']['week'])){?>
                          ...
                          <?php if (!empty($list['rule_dates']['d']['d'])){
                              foreach ($list['rule_dates']['d']['d'] as $d){
                                  if(!empty($d)){
                                    $tmp=explode('-', $d); echo date('Y.m.d',strtotime($tmp[0]));if (!empty($tmp[1])) echo '-'.date('Y.m.d',strtotime($tmp[1]));echo ' '; }}}?>
                          <?php if ((empty($list['rule_dates']['d']['r']['week'])||(isset($list['rule_dates']['d']['r']['week'][0])&&$list['rule_dates']['d']['r']['week'][0]==7))&&(empty($list['rule_dates']['d']['d'])||(isset($list['rule_dates']['d']['d'][0])&&$list['rule_dates']['d']['d'][0]==''))){echo '无';}?></h5></div>
              </div>
          </div>
        <div class="col-xs-4">
          <div class="box box-solid">
            <div class="box-header with-border"><i class="fa fa-credit-card"></i><h3 class="box-title">按订单状态</h3></div>
              <div class="box-body"><h5><?php echo $rule_type[$list['rule_type']]?></h5></div>
          </div>
        </div>
          <div class="col-xs-4">
              <div class="box box-solid">
                  <div class="box-header with-border"><i class="fa fa-credit-card"></i><h3 class="box-title">是否激活</h3></div>
          ...
          <div class="col-xs-6">
          <div class="box box-solid">
            <div class="box-header with-border"><i class="fa fa-clone"></i><h3 class="box-title">其他条件</h3></div>
            <div class="box-body">
                    <h5>
                        <?php
                            if(!empty($list['extra_rule']['min_amount'])){
                                echo "消费满".$list['extra_rule']['min_amount']."元&nbsp&nbsp";
                            }
                            if(!empty($list['extra_rule']['order_nums'])){
                                echo "订单满".$list['extra_rule']['order_nums']."单&nbsp&nbsp";
                            }
                            if(!empty($list['trigger_times'])){
                                echo "执行".$list['trigger_times']."次&nbsp&nbsp";
                            }

                            if(!empty($send_condition->num->order)){
                                echo "每个订单送".$send_condition->num->order."张&nbsp&nbsp";
                            }

                            if(!empty($send_condition->num->night)){
                                echo "每个间夜送".$send_condition->num->night."张&nbsp&nbsp";
                            }
                    ...
                    <h5>
                        <?php
                            if(empty($list['extra_rule']['is_random']) || $list['extra_rule']['is_random']==1){
                                echo "不随机发放&nbsp&nbsp";
                            }else{
                                echo "随机发放&nbsp&nbsp";
                            }

                            if(!empty($list['extra_rule']['random_percent'])){
                                echo $list['extra_rule']['random_percent']."%概率&nbsp&nbsp";
                            }

                            if(!empty($list['extra_rule']['random_amounts'])){
                                echo $list['extra_rule']['random_amounts']."库存&nbsp&nbsp";
                            }
                        ?>
                    </h5>
            </div>
        ...
        <div class="col-xs-12">
          <div class="box box-solid">
            ...
            <div class="box-body">
               <ol type="1">
                   <?php foreach($logs as $arr){  ?>
                       <li class="text-red"><?php echo $arr['log_des'];?></li>
                 <?php  }  ?>
              </ol>
            </div>
            <!-- /.box-body -->
