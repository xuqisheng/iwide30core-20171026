...
class Printer extends MY_Admin_Roomservice {
	...
	public function index(){
        ...
        $base_url = site_url('/roomservice/printer/index/');
        $first_url = site_url('/roomservice/printer/index/').$search_url;
        $suffix = $search_url;
        ...
        $shops_dining_room = $this->appointment_dining_room_model->get_list($filterH);
        if($shops_dining_room){
            foreach($shops_dining_room as $ok=>$ov){
                $shops_dining_room[$ov['dining_room_id']] = $ov;
            }
        }
        ...
        $view_params = array (
            ...
            'shops_dining_room'=>$shops_dining_room,
        );
	...
	public function add()
	{
       ...
       $post =  $this->input->post ();
        ...
        {//add数据
            ...
            $filter['hotel_id'] = !empty($shop) && $post['source'] == 1 ?$shop['hotel_id']:0;
     ...
     //   $this->load->model ( 'hotel/hotel_model' );
        ...
        $this->load->model('appointment/appointment_dining_room_model');
        $shops_arr = $this->appointment_dining_room_model->get_list($filterH);
        $shops_dining_room = array();
        if($shops_arr){
            foreach($shops_arr as $v){
                $shops_dining_room[$v['dining_room_id']] = $v;
            }
            unset($shops_arr);
        }
