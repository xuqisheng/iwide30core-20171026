...
class Auto_send_script extends CI_Controller
{
    ...
    public function index($inteid = 'a455510007 ')
    {
        ...
        ;
        $startime = date("Y-m-d H:i:s", strtotime("-1 day")); // 目前是写死是前一天的时间
        ...
        foreach ($list as $arr) {
           $member=$this->member->getMemberInfoById($arr['open_id']);
            ...
            if ($res['s'] == 1) {
                ...
                MYLOG::w('OPEN_ID为 '.$arr['open_id'].' 用户,第'.($arr['send_count'] + 1).'次发送，发送成功 | 结果：'.json_encode($res));
            } else {
                $update_data = [
                    'is_send' => 1,
                    'send_count' => $arr['send_count'] + 1,
                ...
                MYLOG::w('OPEN_ID为 '.$arr['open_id'].' 用户,第'.($arr['send_count'] + 1).'次发送，发送失败 | 结果：'.json_encode($res));
            }
        }
        else{
            //检索时已经是会员，踢出队列
