...
class Qrcodes_model extends MY_Model {
     ...
     * @param number $offset 起始位置
     */
    public function get_salers_lite($inter_id,$distributed = 1,$saler_name = NULL,$saler_no = NULL,$cellphone = NULL,$hotel_id = NULL,$department = NULL,$status = NULL,$limit = NULL,$offset = 0){
    	$sql = "SELECT s.id,s.inter_id,s.status_time,s.audit_time,s.master_dept,s.qrcode_id,s.`name`,s.cellphone,s.hotel_name,s.hotel_id,s.`status`,s.`is_club`,s.`distribute_hidden` FROM iwide_hotel_staff s WHERE s.inter_id=? AND s.is_distributed=?";
    	$params[] = $inter_id;
    	$params[] = $distributed;
    	if(!empty($saler_name)){
    		$sql .= " AND s.`name` LIKE ?";
    	...
    	$res = $this->_db('iwide_r1')->query($sql,$params);
    	$salers = array_column($res->result_array(), 'qrcode_id');
    	$ext = [];
    	if(!empty($salers)){
	    	$sql = 'SELECT saler,SUM(a.grade_total) grade_total,SUM(IF(a.`status`=1,a.grade_total,0)) undeliver FROM iwide_distribute_grade_all a WHERE `status` IN (1,2,9) AND inter_id=? AND saler IN ? GROUP BY saler';
	    	$query = $this->_db('iwide_r1')->query($sql,[$inter_id,$salers])->result();
	    	foreach ($query as $q){
	    		$ext[$q->saler] = ['grade_total'=>$q->grade_total,'undeliver'=>$q->undeliver];
	    	}
    	}
    	return [$res->result(),$ext];
    }
    /**
     * 根据指定条件取分销员数量
...
//     	return $this->_db('iwide_rw')->query($sql,$params)->row()->counts;
			...
			switch ($curr_saler->status) {
				case 0 :
					if ($status == 2 || $status == 3) {
						...
						$res = $this->_db('iwide_rw')->update('hotel_staff',$params);
						if($res && !empty($curr_saler->openid)){
							$arr = array('inter_id'=>$inter_id,'openid'=>$curr_saler->openid,'nickname'=>$curr_saler->name,'audit_time'=>isset($curr_saler->audit_time) ? $curr_saler->audit_time : date('Y-m-d H:i:s'));
							$this->Template_msg_model->send_staff_status_msg($arr,$status == 2 ? 'staff_status_pass' : 'staff_status_unpass');
						}
						return $res;
					}
					break;
    ...
    private function _get_qr_code($inter_id, $intro, $keyword, $name, $id = NULL) {
		...
		if (isset ( $jsoninfo ['url'] )) {
			$res = $this->db->insert ( 'qrcode', array (
					'id'          => $id,
					'intro'       => $intro,
					'keyword'     => $keyword,
					'name'        => $name,
...
LEFT JOIN iwide_fans f ON f.openid=a.grade_openid AND f.inter_id=a.inter_id WHERE a.saler=? AND a.inter_id=? AND a.status != 99";
	...
	public function get_saler_grades_count($inter_id, $saler,$typ = '',$grade_table='') {
		$sql = "SELECT COUNT(a.id) nums FROM iwide_distribute_grade_all a WHERE a.saler=? AND a.inter_id=? AND a.status != 99 ";
		$params = array ( $saler, $inter_id );
		if(!empty($typ)){
			if(is_array($typ)){
				$sql .= " AND `status` IN ?";
