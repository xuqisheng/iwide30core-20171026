...
class Lvyun_hotel_model extends CI_Model {
	...
	public function get_web_order_items($inter_id, $order, $pms_set){
		...
		$res = $this->get_to($url, $data, $inter_id, $func_data);
...
// 					}
	...
	function order_to_web_normal($inter_id, $orderid, $paras = array(), $pms_set = array(),$order){
		...
		$res = $this->post_to ( $url, $data, $inter_id,array('orderid'=>$orderid) );
		/*
		 * 返回的信息如下：
...
// 		$taCode = empty ( $pms_auth ['taCode'] ) ? '' : $pms_auth ['taCode'];
		...
		$res = $this->post_to ( $url, $data, $order ['inter_id'],array('orderid'=>$order ['orderid']) );
	...
	function get_to($url, $data = array(), $inter_id = '', $func_data = array()) {
		$this->load->helper ( 'common' );
		$r_url = $url;
		$url_param = '';
		if (! empty ( $data )) {
	...
	function post_to($url, $data = array(), $inter_id = '', $func_data = array()) {
		$this->load->helper ( 'common' );
