...
class CardcenterService extends MemberBaseService
{
    ...
    //卡券详细页面
    public function receive($inter_id = '',$openid = '',$ec_code = '', $url_group = array(), $extra = array()){
        $this->getBase();
        $this->assign_data = $url_group;
    ...
    //卡券详细页面
    public function cardinfo($show_inter_id = '',$show_openid = '', $inter_id = '',$member_card_id = 0,$url_group = array(),$extra = array()){
        $this->getBase();
        ...
        if(!empty($extra['share_domain'])){
            ...
            $assign_data['js_share_config']['link'] = \EA_const_url::inst()->get_url('*/*/receive',array('id'=>$show_inter_id,'sf'=>$encrypt));
        }
