...
class MY_Front_Soma extends MY_Front
{
     ...
     * @var array
     */
    protected $idistributInterId = ['a490782373'];
...
EOF;
        ...
        if(ENVIRONMENT != 'dev' && !$this->input->get('rel_res') && !$this->input->is_ajax_request() && (!in_array($this->inter_id, $this->idistributInterId) || $this->input->get('fans_act'))) {
    ...
    private function saveQueryParams()
    {
        ...
        //直播地址
        $zburl = $this->input->get('zburl');
        if($zburl) {
            $this->session->set_tempdata('zburl', $zburl, $ttl);
        }

        $rel_res = $this->input->get('rel_res');
        if($rel_res) {
...
(function() {
  ...
  s.parentNode.insertBefore(hm, s);
    ...
    protected function _view($file, $datas = array())
    {
         ...
         * 悦榕庄皮肤
         */
        if(in_array($this->inter_id, ['a493015889', 'a497864944'])) {
    ...
    public function get_page_block($uri)
    {
        ...
        $pids = isset($pids['pids']) ? $pids['pids'] : [];
    ...
    protected function changeViewData(Array $arr)
    {
        if (!empty($arr)) {
            ...
            if (isset($arr['js_share_config'])  && is_array($arr['js_share_config'])) {
                $this->headerDatas['js_share_config'] = $arr['js_share_config'];
            }

            if (isset($arr['js_api_list'])  && is_array($arr['js_api_list'])) {
                $this->headerDatas['js_api_list'] = implode(",",$arr['js_api_list']);
            }

            if (isset($arr['js_menu_show'])  && is_array($arr['js_menu_show'])) {
                $this->headerDatas['js_menu_show'] = implode(",",$arr['js_menu_show']);
            }

            if (isset($arr['js_menu_hide'])  && is_array($arr['js_menu_hide'])) {
                $this->headerDatas['js_menu_hide'] = implode(",",$arr['js_menu_hide']);
            }
        }
