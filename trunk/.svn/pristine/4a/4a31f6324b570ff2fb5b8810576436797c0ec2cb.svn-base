...
<div class="webkitbox justify bg_fff">
    ...
    <?php if (isset($member->logined)&&$member->logined==0){if (isset($_SERVER['HTTPS'])&&strtolower($_SERVER['HTTPS'])=='on')$scheme='https://';else $scheme='http://';?>
        <a href="<?php echo site_url('membervip/login/index').'?id='.$inter_id.'&redir='.urlencode($scheme.$_SERVER ['HTTP_HOST'] . $_SERVER ['REQUEST_URI']);?>"><em class="iconfont">&#X4B;</em> <?php if (empty($login_action_name))echo '登录';else echo $login_action_name;?></a>
...
<?php if (!empty($index_tel)){?>
<div class="bohao">
    <a href="tel:<?php echo $index_tel?>">订房专线 <?php echo $index_tel?></a>
</div>
<?php }?>
<?php }else{?>
<div class="pad3">
