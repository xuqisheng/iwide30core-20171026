...
$buttions.= '<button type="button" class="btn btn-default disabled single-btn" id="grid-btn-extra-0" target="qr"><i class="fa fa-qrcode"></i>&nbsp;生成微信卡券</button>';
$buttions.= '<button type="button" class="btn btn-default disabled single-btn" id="grid-btn-extra-1" target="qr"><i class="fa fa-qrcode"></i>&nbsp;查看微信卡券审核状态</button>';
...
?>
	...
	$("#grid-btn-extra-0").click(function(){
		$.get("<?php echo EA_const_url::inst()->get_url("*/*/createWxCard");?>", {ci_id:selected},
		    function(data){
		        if(data.error) {
		            alert(data.errmsg);
				} else {
	...
	$("#grid-btn-extra-1").click(function(){
		$.get("<?php echo EA_const_url::inst()->get_url("*/*/checkWxCardStatus");?>", {ci_id:selected},
		    function(data){
		        alert(data);
	    },"text");
	});
