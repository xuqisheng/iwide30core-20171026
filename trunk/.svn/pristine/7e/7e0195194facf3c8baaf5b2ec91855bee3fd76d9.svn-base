<?php
class Kezhan_webservice implements IPMS{
	protected $CI;
...
//		$this->_pwd = $pms_param['pwd'];
	...
	public function get_rooms_change($rooms, $idents = array(), $condit = array()){
		...
		$this->CI->load->model ( 'hotel/pms/Kezhan_hotel_model', 'pms' );
...
/*	public function search_hotel_front($params){
		$this->CI->load->model('hotel/pms/Kezhan_hotel_model');
		return $this->CI->Kezhan_hotel_ext_model->search_hotel_front($params[0],$params[1],$this->pms_set);
	}*/
