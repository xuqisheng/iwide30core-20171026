...
<div class="form_list">
   ...
   <?php if(!empty($member->mem_id)&&!empty($point_consum_rate)){?>
	<div class="item">
    	<span>悦银抵用</span>
        <?php if($member->bonus<100){?>
            <input max="<?php echo $member->bonus;?>" disabled type="tel" id='bonus' name='bonus' placeholder="可用悦银至少需要100" />
        <?php }else{?>
            <input max="<?php echo $member->bonus;?>" type="tel" id='bonus' name='bonus' placeholder="共<?php echo $member->bonus;?>悦银，最多可抵<?php echo $member->bonus*$point_consum_rate; ?>元" />
        <?php } ?>
    </div>
    <?php } ?>
...
<div class="form_list">
	<div class="item pay_list">
        ...
        <ul><?php if(!empty($pay_ways)) foreach($pay_ways as $k=>$pw){?>
            <li pay_type='<?php echo $pw->pay_type;?>' abled='1' class="pay_way <?php if($k==0){?>ischeck<?php }?>"><em class="ui_ico ui_ico13"></em><span><?php echo $pw->pay_name;?><?php if($pw->pay_type=='balance') { ?>(<?php echo $member->balance;?>元)<?php }?></span></li>
            <?php }?>
            <?php if(!empty($point_exchange)&&isset($point_exchange['can_exchange'])&&!empty($member)){?>
            <li id='bonus_pay_way' pay_type='bonus' <?php if($point_exchange['can_exchange']==0){?>style='color:gray;' abled='0'<?php }else{?>abled='1'<?php }?> class="pay_way"><em class="ui_ico ui_ico13"></em><span>悦银兑换(<?php echo $point_exchange['point_need'];?>/<?php echo $member->bonus;?>)</span></li>
            <?php }?>
        </ul>
    </div>
</div>
<div class="form_list" id='consume_code'>
	<div class="item">
    	<span>消费密码</span>
...
$(function(){
	...
	$('.usevote').click(function(){
		...
		$.post('/index.php/hotel/hotel/return_usable_coupon?id=<?php echo $inter_id;?>',{
			...
			temp='';
			if(data.cards!=''){
				max_room_night_use=data.count.num;
				max_order_use=data.count.num;
				$.each(data.cards,function(i,n){
					...
					if(n.coupon_type==undefined||n.coupon_type==''||n.coupon_type=='voucher'){
						temp+=' coupon_type="voucher" code='+n.code+' amount="'+n.reduce_cost+'" max_use_num="'+n.hotel_max_use_num+'" card_type="'+n.ci_id+'" use_num_type="'+n.hotel_use_num_type+'"><div class="checkbox"><em class="ui_ico ui_ico13"></em></div><div class="ui_vote"><p class="bordertop_img"></p><div class="vote_con">';
						temp+='<p class="ui_price ui_color">'+n.reduce_cost+'元</p>';
					}else if(n.coupon_type=='discount'){
						if(n.room_discount_type!=undefined){
							temp+=' room_discount_type="'+n.room_discount_type+'" ';
						}
						temp+=' coupon_type="discount" code='+n.code+' amount="'+n.reduce_cost+'" max_use_num="1" card_type="'+n.ci_id+'" use_num_type="order"><div class="checkbox"><em class="ui_ico ui_ico13"></em></div><div class="ui_vote"><p class="bordertop_img"></p><div class="vote_con">';
						temp+='<p class="ui_price ui_color">'+n.reduce_cost*10+'折</p>';
					}
					temp+='<p><b>'+n.title+'</b></p>';
					temp+='<p class="ui_gray">'+n.brand_name;
					if(n.is_wxcard==1)temp+='--已添加到卡包';
					temp+='</p></div><div class="val_date">';
					//temp+='<p class="ui_red">还有'+calcDate(n.valid_date)+'天过期</p>';
					temp+='<p class="ui_gray">有效期至'+n.valid_date+'</p></div></div></li>';
				
				});
			}
			else{
               temp+='<li class="ischeck" onclick="choose_coupon(this)"><div class="ui_vote" style="width:90%"><p class="bordertop_img"></p><div class="vote_con"><p class="votename" style="text-align: center;">暂无可用优惠券哦</p></div></div></li>';
			}
			$('#votelist').html(temp);
			$('.page_loading').remove();
		},'json');
	...
	$('.room_count .num').change(function(){
		...
		$('#total_price').html((real_price-total_favour).toFixed(2));
		 ...
		 <?php if(!empty($point_exchange)&&isset($point_exchange['can_exchange'])&&!empty($member)){?>
		 	...
		 	$('#bonus_pay_way').find('span').html("悦银兑换("+need_point+"/<?php echo $member->bonus;?>)");
...
function choose_coupon(obj){
	...
	if($(obj).attr('coupon_type')=='discount'){
		if($(obj).attr('room_discount_type')&&$(obj).attr('room_discount_type')=='first_night'&&tmp_price){
			first_night_price=tmp_price[0]*$('#roomnum').val();
			coupon_val=first_night_price-coupon_val*first_night_price;
		}else{
