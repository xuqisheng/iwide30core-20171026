...
<div class="webkitbox justify bg_fff">
    ...
    <?php if (isset($member->logined)&&$member->logined==0){?>
        <a href="<?php echo site_url('membervip/login/index').'?id='.$inter_id;?>"><em class="iconfont">&#X4B;</em> 登录</a>
    	<a href="<?php echo site_url('membervip/reg/index').'?id='.$inter_id;?>"><em class="iconfont">&#X4C;</em> 注册</a>
    	<?php }else{ 
    		echo '<div class="txtclip">欢迎您，';
    		echo empty($member->name)?$member->nickname:$member->name;
    		echo '</div>';
		}?>
    </div>
</div>
...
<form action="sresult?id=<?php echo $inter_id?>" onsubmit="to_search($(this))" method="post" id="index_search">
   ...
   <div class="list_style index_list bd_bottom">
	<div class="webkitbox">
        ...
        <a href="<?php echo site_url('hotel/check/nearby').'?id='.$inter_id;?>" class="center">
	...
	<div class="webkitbox arrow">
        ...
        <div class="keyword" style="padding:12px 0" url="<?php echo site_url('hotel/check/ajax_city_filter').'?id='.$inter_id;?>">关键字/位置/名称</div>
    </div>
</div>
...
<div class="webkitbox others center">
	<?php foreach($homepage_set['menu'] as $k=>$v){?>
		<div class="<?php echo $v['code'];?>" <?php if($v['code'] == 'athour') echo 'href="'.site_url('hotel/hotel/search').'?id='.$inter_id.'&type=athour'.'"';?> <?php if($v['code'] == 'order') echo 'href="'.site_url('hotel/hotel/myorder').'"';?> <?php if($v['code'] == 'ticket') echo 'href="'.site_url('hotel/hotel/search').'?id='.$inter_id.'&type=ticket'.'"';?>>
	    	<div class="img">
            	<!--em class="iconfont <?php echo $v['code'];?>"></em-->
            	<p class="squareimg"><img src="<?php echo base_url('public/hotel/public/images/spring').'/'.$v['code'].'.png';?>"></p>
            </div>
...
<div class="pad3">
    <div class="often_like webkitbox pad3 <?php if ($inter_id!='a476756979'){?>bg_F8F8F8<?php }?> bdradius">
        <div class="often always">
            <em class="iconfont color_main h34 ">&#x28;</em> <span>常住酒店</span>
        </div>
        <div class="like bd_left collect" style="padding-left:8px">
...
<div class="ui_pull often_pull" style="display:none;" onClick="toclose()">
    <div class="bg_fff pull_box center">
       ...
       <?php foreach($last_orders as $lo){?>
            <li onClick="event.stopPropagation()"><a href="<?php echo site_url('hotel/hotel/index').'?id='.$inter_id.'&h='.$lo['hotel_id']?>"><?php echo $lo['hname']; ?></a></li>
            <?php }?>
        <?php } else {?>
         <li>无</li>
        <?php }?>
...
<div class="ui_pull" id="collects" style="display:none;" onClick="toclose()">
    <div class="bg_fff pull_box center">
        ...
        <?php if(!empty($hotel_collection)) {?>
            ...
            <li onClick="event.stopPropagation()"><a href="<?php echo $hc['mark_link']?>"><?php echo $hc['mark_title'];?></a></li>
            <?php }?>
        <?php }else{?>
            <li>无</li>
        <?php }?>
