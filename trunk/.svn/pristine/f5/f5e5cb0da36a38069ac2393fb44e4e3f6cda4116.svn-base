...
$.fn.uploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><span class="up_filename">${fileName}</span> <span class="uploadify-progress"><span class="uploadify-progress-bar"></span></span> <span class="uploadbtn">上传</span> <span class="delfilebtn"><img style="width:12px; height:12px;cursor:pointer" src="${delimg}"></span></div>';
	var defaults = {
		...
		removeTimeout: 500,//上传完成后进度条的消失时间
	...
	this.each(function(){
		...
		if(option.buttonImage!=''){
			inputStr += '<img src="'+option.buttonImage+'" style="width:"'+option.width+'px; height:'+option.height+'px;cursor:pointer"  />'
		}else{
	  ...
	  var fileObj = {
		  ...
		  onSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					var file = files[i];
					//处理模板中使用的变量
					var $html = $(option.itemTemplate.replace(/\${fileID}/g,'fileupload_'+instanceNumber+'_'+file.index).replace(/\${fileName}/g,file.name).replace(/\${fileSize}/g,formatFileSize(file.size)).replace(/\${instanceID}/g,_this.attr('id')).replace(/\${delimg}/g,option.delimg));
					//如果是自动上传，去掉上传按钮
					if(option.auto){
