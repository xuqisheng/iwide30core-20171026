		...
		if(!empty ($order)){
			...
			if(!isset($result['errmsg'])){
                ...
                if($order['point_favour'] > 0){
                    $res = $this->addpayment($pms_set,$pms_set['hotel_web_id'], $web_orderid, -$order['point_favour'],$order ['orderid'],1089,'coupon_favour');
                }
...
//                if($web_order[0]->ChgtoID==0){}
                ...
                foreach($order['order_details'] as $key=>$local_order){
                    ...
                    if(!empty($local_order['webs_orderid'])){

                        foreach($web_orders as $wkey=>$web_order){

                            if( $local_order['istatus']!=4 && $web_order->FolioState!=1 && $status_arr[$web_order->FolioState]!=$local_order['istatus']){

                                if($local_order['webs_orderid']==$web_order->FolioID){
