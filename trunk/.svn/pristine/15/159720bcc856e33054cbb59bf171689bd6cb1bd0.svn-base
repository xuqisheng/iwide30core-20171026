...
<style>
/* 要求修改部分  */
.viplist,.backvote{display:none !important;}

.room_list .item_left,.vote_spread  .item_left{padding:2.5% 3%;width:70%;}

.room_list .room_img{ width:80px;height:50px; overflow:hidden}
.room_list_many .item_right{ top:1.5em; padding-top:1.5%;}
.room_list_many .showprice .ui_price{padding-right:0; display:block;}
.showprice p{ background:#f99e12; color:#fff; padding:0.2em 2%; display:inline-block; margin-top:1.5%; border-radius:0.3em;}
.showprice p span{ font-size:2em; vertical-align:middle}
.showprice p .iconfont{ font-size:5px; vertical-align:middle}
/* end */
</style>
	...
	<div class="item" onClick="silde_down(this)">
       ...
       <!--- <div class="isfull item_right showprice">  <!-- 满房需要加isfull 变灰色字体-->
        <div class=" <?php if(!empty($r['all_full'])) {?>isfull<?php }?> item_right showprice ">
        	<div class="ui_price"><b><?php echo $r['highest']; ?></b></div>
            <p><span>查看价格</span><em class="iconfont">&#x34;</em></p>
        </div>
        <?php }else {?>
...
function show_room_detail(obj,event){ 
	...
	$.post("<?php echo site_url('hotel/hotel/return_room_detail').'?id='.$inter_id; ?>",{
		...
		else  tmphtml+='<span class="slideson ui_img_auto_cut" title="1"><img src="'+data.room_img+'" /></span>';
		tmphtml+='</div>';
	...
	$.post('/index.php/hotel/hotel/return_more_room?id=<?php echo $inter_id;?>',{
		...
		protrol_code:$('#protrol_code').val()
			...
			$.each(data.rooms,function(i,n){
				...
				temp+='<div class="sever" onclick="show_room_detail(this,event)">'+n.room_info.sub_des+'</div>';
				...
				if(n.state_info!=''){
					...
					temp+='<div class="ui_price"><b>'+n.highest+'</b></div><p><span>查看价格</span><em class="iconfont">&#x34;</em></p></div>';
				}
				else{
