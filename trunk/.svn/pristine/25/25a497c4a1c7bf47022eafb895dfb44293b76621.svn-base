    ...
    <div class="head_income_item is_cur">
        ...
        <div class="income_item_detail">
            <div class="webkitbox _1">
                <p>核定收益总额</p><p class="ui_price"><?php echo empty($grades->total) ? 0 : $grades->total?></p>
                <div class="co_aaa"><p style="text-align: left">已发￥<?php echo empty($send->total) ? 0 : $send->total?></p><p style="text-align: left">待发￥<?php echo empty($unsend->total) ? 0 : $unsend->total?></p></div>
            </div>
            <div class="webkitbox _2">
                <p>核定收益笔数</p><p><?php echo empty($grades->counts) ? 0 : $grades->counts?></p>
                <div class="h4 co_aaa"><p style="text-align: left">已发<?php echo empty($send->counts) ? 0 : $send->counts?>笔</p><p style="text-align: left">待发<?php echo empty($unsend->counts) ? 0 : $unsend->counts?>笔</p></div>
            </div>
        </div>
    </div>
...
<div class="_block bg_fff content_income">
	<div class="h3 fill1"><?php echo isset($grades_types[$log->grade_table]) ? $grades_types[$log->grade_table] : '其他'?>:<?php echo empty($hotels[$log->order_hotel]) ? '' : $hotels[$log->order_hotel]?>-<?php echo $log->product?><?php if($log->grade_table == 'iwide_fans_sub_log'):echo '&nbsp;'.mb_substr($log->nickname, 0, 1). str_repeat('*', 2);endif;?></div>
    <div class="webkitbox">
    	<p>
    ...
    <div class="_block h4 co_aaa" style="display:none">
        <div class="fill5"><?php echo isset($grades_types[$log->grade_table]) ? $grades_types[$log->grade_table] : '其他'?>:<?php echo empty($hotels[$log->order_hotel]) ? '' : $hotels[$log->order_hotel]?>-<?php echo $log->product?></div>
        ...
        <div><p>关注粉丝：<?php echo '&nbsp;'.mb_substr($log->nickname, 0, 1). str_repeat('*', 2);?></p></div>
            ...
            <div>
                <p>粉丝编号：<?php echo $log->grade_id?></p>
        ...
        <div class="webkitbox mar3">
            <div>
                <p>订单编号：<span class="fill6"><?php echo substr($log->grade_table ,0,10)=='iwide_soma'?substr_replace($log->grade_id,'****',-7,4):$log->grade_id?></span></p>
...
<div class="foot_fixed">
	...
	<a href="<?php echo site_url('distribute/dis_ext/incomes')?>?id=<?php echo $inter_id?>&g_typ=ALL"<?php if($g_typ == 'ALL'):?> class="link_style"<?php endif;?>>收益详情</a>
	<a href="<?php echo site_url('distribute/dis_ext/incomes')?>?id=<?php echo $inter_id?>&g_typ=OLD"<?php if($g_typ == 'OLD'):?> class="link_style"<?php endif;?>>已发放收益</a>
	<a href="<?php echo site_url('distribute/dis_ext/incomes')?>?id=<?php echo $inter_id?>&g_typ=NEW"<?php if($g_typ == 'NEW'):?> class="link_style"<?php endif;?>>未发放收益</a>
	<a href="<?php echo site_url('distribute/dis_ext/incomes')?>?id=<?php echo $inter_id?>&g_typ=PRE"<?php if($g_typ == 'PRE'):?> class="link_style"<?php endif;?>>未核定收益</a>
    </div>
</div>
