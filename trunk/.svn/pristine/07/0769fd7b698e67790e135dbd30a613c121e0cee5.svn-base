...
<div class="tabbable "> <!-- Only required for left/right tabs -->
    <ul class="nav nav-tabs">
        ...
        <li id="tab_header2" <?php $scope = $model->m_get('scope'); if( !$scope || $scope == Soma_base::STATUS_TRUE ) echo 'style="display:none;"'; ?>><a href="#tab2" data-toggle="tab"><i class="fa fa-link"></i> 选择商品</a></li> 
    </ul>
...
$(document).ready(function() {
    ...
    var val = $("#el_type").val();
    if( val != <?php echo Soma_base::STATUS_TRUE;?> ){
        $('#el_send_scope').parents('.form-group').hide();
    }
});

$("#el_scope").change(function(){
    var val = $(this).val();
    if( val == <?php echo Soma_base::STATUS_TRUE;?> ){
        $('#tab_header2').hide()
    }else{
...
$("#el_type").change(function(){
    var val = $(this).val();
    if( val == <?php echo Soma_base::STATUS_TRUE;?> ){
        $('#el_send_scope').parents('.form-group').show();
    }else{
        $('#el_send_scope').parents('.form-group').hide();
    }
});
