 ...
 * Time: 11:32
 */
class Xiruan_webservice implements IPMS{
	protected $CI;
	...
	public function get_rooms_change($rooms, $idents = array(), $condit = array()){
		...
		$this->CI->load->model('hotel/pms/Xiruan_hotel_model', 'pms');
	...
	/*	public function search_hotel_front($params){
			$this->CI->load->model('hotel/pms/Xiruan_hotel_model');
	...
	public function get_order_state($params){
		...
		return $this->CI->Xiruan_hotel_model->get_order_state($params[0],$this->pms_set,$status_des);
	}
