<?php
class Youcheng_webservice implements IPMS{
	protected $CI;
...
//		$this->_url = $pms_param['url'];
	...
	public function get_rooms_change($rooms, $idents = array(), $condit = array()){
		...
		$this->CI->load->model ( 'hotel/pms/Youcheng_hotel_model', 'pms' );
	...
	public function add_web_bill($order,$params=array()){
		$this->CI->load->model ( 'hotel/pms/Youcheng_hotel_model', 'pms' );
		return $this->CI->pms->add_web_bill ( $order['web_orderid'], $order, $this->pms_set);
	}
	...
	function check_order_canpay($order) {
        // TODO: Implement check_order_canpay() method.
        echo '';
	}
...
/*	public function search_hotel_front($params){
		$this->CI->load->model('hotel/pms/Youcheng_hotel_model');
		return $this->CI->Yasite_hotel_ext_model->search_hotel_front($params[0],$params[1],$this->pms_set);
	}*/
