!function(a){
	...
	if(c){
		var k=(a.navigator.appVersion.match(/android/gi),
		a.navigator.appVersion.match(/iphone/gi)),
		c=a.devicePixelRatio;
		c=k?c>=3?3:c>=2?2:1:1,d=1/c;
	}
	if(g.setAttribute("data-dpr",c),!h){
		if(h=f.createElement("meta"),h.setAttribute("name","viewport"),
		h.setAttribute("content","target-densitydpi=device-dpi,initial-scale="+d+",maximum-scale="+d+",minimum-scale="+d+",user-scalable=no"),g.firstElementChild)
		...
		h.setAttribute("content","target-densitydpi=device-dpi,initial-scale="+d+",maximum-scale="+d+",minimum-scale="+d+",user-scalable=no")
		g.firstElementChild.appendChild(h);
	}
	...
	},!1),"complete"===f.readyState?f.body.style.fontSize=10+"px":f.addEventListener("DOMContentLoaded",function(){
			f.body.style.fontSize=10+"px"
	},!1),b()
}(window);