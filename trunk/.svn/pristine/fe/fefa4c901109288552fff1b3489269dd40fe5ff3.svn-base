	...
	$('.submit_btn').click(function(){
		...
		if(bonus){
			...
			if(bonus_condition['poc']!=undefined&&bonus_condition['poc']==1){
		        ...
		        if(Length>0){
					$.MsgBox.Alert('此价格不能同时使用积分与优惠券！请重新选择');
					$('#bonus').val('');
					return;
		        }
			}
		...
		$.MsgBox.Alert('您的订单已经提交, 请稍候...');
		$('#mb_btnbox').remove();
		$.post('saveorder',data,function(data){
			if(data.s==1){
				$('#prevend').val(1);
