				...
				$fields[] = $key;
                    ...
                    <tr>
                    	...
                    	if (in_array ( 'saler_hotel_name', $fields )) {
                    		?>
                    	<td><?php if(isset( $hotels[$item ['saler_hotel']] )):echo $hotels[$item ['saler_hotel']];else:echo '-';endif;?></td>
                    	<?php
                    	}
                    	...
                    	if (in_array ( 'partner_trade_no', $fields )) :
                    		?>
                    	<td><?php echo isset( $item ['partner_trade_no'] ) ? $item ['partner_trade_no'] :  '-';?></td>
                    </tr>
                    	<?php endif;endforeach;?>
                    </tbody>
                  </table>
                  ...
                  <div class="row">
	                  <div class="col-sm-5"><?php if($this->uri->segment(3) == 'exnew'):?>
	                  	<div class="dataTables_info" id="data-grid_info" role="status" aria-live="polite">共<?=$total?>条<a class="btn btn-sm bg-green" href="<?php echo site_url("distribute/distri_report/ext_all/".$posts['hotel_id'].'_'.$posts['cout_date_begin'].'_'.$posts['cout_date_end'].'_'.$posts['order_id'].'_'.$posts['saler_name'].'_'.$posts['saler_no'].'_'.$posts['grade_date_begin'].'_'.$posts['grade_date_end'].'_'.$posts['send_date_begin'].'_'.$posts['send_date_end'].'_'.$posts['department'])?>">导出</a></div>
	                  <?php else:?>
	                  	<div class="dataTables_info" id="data-grid_info" role="status" aria-live="polite">共<?=$total?>条<a class="btn btn-sm bg-green" href="<?php echo site_url("distribute/distri_report/ext_qrs/".$posts['hotel_id'].'_'.$posts['cout_date_begin'].'_'.$posts['cout_date_end'].'_'.$posts['order_id'].'_'.$posts['saler_name'].'_'.$posts['saler_no'].'_'.$posts['grade_date_begin'].'_'.$posts['grade_date_end'].'_'.$posts['send_date_begin'].'_'.$posts['send_date_end'].'_'.$posts['department'])?>">导出</a></div>
	                  <?php endif;?>
	                  </div>
	                  <div class="col-sm-7">
...
// 	  modal.find('.modal-body input').val(recipient)
	...
	$.getJSON('<?php echo site_url("distribute/distri_report/get_cofigs?ctyp=dist_rooms_order")?>',function(data){
...
$('#set_btn_save').click(function(){
	$.post('<?php echo site_url("distribute/distri_report/save_cofigs?ctyp=dist_rooms_order")?>',$("#setting_form").serialize(),function(data){
		if(data == 'success'){
