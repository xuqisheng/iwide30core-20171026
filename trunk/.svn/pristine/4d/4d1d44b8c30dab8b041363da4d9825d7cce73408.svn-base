...
function pageloading (_str,alpha){ //页面loading动画
	if(!alpha){alpha=0.5;}
	if(!_str) _str='正在加载';
	var tmp = '<div class="pageloading" style="background:rgba(0,0,0,'+alpha+')">';
...
var toshow=function(_this){
	var _curtop = $(document).scrollTop();
	$(_this).show();
...
function showtips(option,fn){
	...
	+ '<div class="pulltitle h30">'+option.title+'</div>'
	+ '<div style="text-align:justify">'+option.tips+'</div>'
	+ '<div class="webkitbox center">'
	+ '<p><'+tmp+' id="left_btn" onclick="'+option.leftClick+'" href="'+option.leftUrl+'" class="btn_main">'+option.leftLink+'</'+tmp+'></p>'
	+ '<p><'+tmp+' id="right_btn" onclick="'+option.rightClick+'" href="'+option.rightUrl+'" class="btn_main">'+option.rightLink+'</'+tmp+'></p>'
	+ '</div></div></div>';
	$('body').append(html);
...
$(function(){
	...
	$('.down_num').on('touchstart',function(e){
		...
		if(input.attr('min')==undefined)$min=1;
		var val = parseInt(input.val())
		if(isNaN(val)){
			$.MsgBox.Alert('数量必须为数字');
			return false;
		};
		if( val>$min&&val>1){
			input.val(val-1);
	...
	$('.up_num').on('touchstart',function(e){
		...
		if(input.attr('max')==undefined)$max=9999;
		var val = parseInt(input.val());
		if(isNaN(val)){
			$.MsgBox.Alert('数量必须为数字');
			return false;
