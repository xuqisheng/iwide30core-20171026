...
class Order extends MY_Front_Soma_Iapi
{
    ...
    public function get_select_hotel_time()
    {
        ...
        if (empty($orderId) || empty($hotelId) || empty($roomId) || empty($priceCode)) {
    ...
    public function post_booking()
    {
        ...
        if(!$params->has('post_order_id')){
            show_error('post_order_id', 400);
        }
        if(!$params->has('post_hotel_id')){
            show_error('post_hotel_id', 400);
        }
        if(!$params->has('post_room_id')){
            show_error('post_room_id', 400);
        }
        if(!$params->has('post_price_code')){
            show_error('post_price_code', 400);
        }
        if(!$params->has('post_code')){
            show_error('post_code', 400);
        }
        if(!$params->has('post_name')){
            show_error('post_name', 400);
        }
        if(!$params->has('post_phone')){
            show_error('post_phone', 400);
        }
        if(!$params->has('post_start')){
            show_error('post_start', 400);
        }
        if(!$params->has('post_end')){
            show_error('post_end', 400);
        }
        if(!$params->has('post_room_name')){
            show_error('post_room_name', 400);
        }
        if(!$params->has('post_code_name')){
            show_error('post_code_name', 400);
        }
        if(!$params->has('aiid')){
            show_error('aiid', 400);
        }
        if(!$params->has('post_num')){
            show_error('post_num', 400);
        }

        $params = [
            'orderId' => $params['post_order_id'],
            'hotelId' => $params['post_hotel_id'],
            'roomId' => $params['post_room_id'],
            'priceCode' => $params['post_price_code'],
...
//        }
        ...
        $result = OrderService::getInstance()->getBooking($params);
        $data = $result->getData();
        if ($result->getStatus() !== Result::STATUS_OK) {
            $this->json(FrontConst::OPER_STATUS_FAIL_TOAST, $result->getMessage(), $data);
        } else {
