$.post('/index.php/report/getpublic',{},function(d){	 
	...
	for(var i=0;i<d.length;i++){
		...
		if($('#gethotel').attr('showpublic') == d[i].inter_id){
			...
			$.post('/index.php/report/gethotel?interid='+$('#gethotel').attr('showpublic'),{},function(d){
	...
	$('#showpublic').change(function(d){
  		$.post('/index.php/report/gethotel?interid='+$('#gethotel select').val(),{},function(d){
        	$('#gethotel1').html('<select name="showhotel" id="showhotel" style="width:150px"><option value="">全部</option></select>');
			for(var i=0;i<d.length;i++){
