...
ScratchCard.prototype ={
	...
	_initHTML:function(){
		...
		var html='<div class="info">'
					+'<div class="prompt" name="scratchCardPrompt">'+this.param.prompt+'</div>'
					+'<span class="btn h28" name="scratchCardBtn">'+this.param.btnCotent+'</span>'
				+'</div>'
	...
	_initCanvas:function(){ 
		...
		var _color = this.param.tipsColor;
		var _tips = this.param.tips;
		var into = document.createElement("canvas");
		// var img=document.getElementById("s8_ggl");
		var ctx2 = into.getContext('2d');
		into.width = canvas.clientWidth;
		into.height = canvas.clientHeight;

		var img = new Image();
		img.src = '/public/tips/default/images/s8_ggl.png';
		img.onload = function() {
		     ctx2.drawImage(img,0,0,canvas.clientWidth,canvas.clientHeight);
				var pat=ctx.createPattern(into,'repeat');
				ctx.globalCompositeOperation = "source-over";
				ctx.rect(0,0,canvas.clientWidth,canvas.clientHeight);
				ctx.fillStyle=pat;
				ctx.fill();

		ctx.font = "19px pingfang sc regular";
		ctx.textAlign = "center";
		ctx.fillStyle = _color;
		ctx.fillText(_tips,canvas.width/2,72);
