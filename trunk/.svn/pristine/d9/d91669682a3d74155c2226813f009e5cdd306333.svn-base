...
$('.fixed_header .webkitbox div').click(function(){
	...
	if($(this).attr('rel') == 'qa'){
		var str = '';
		$.getJSON("<?php echo site_url('distribute/dis_ext/msgs_asy');?>?id=<?php echo $inter_id?>&mstyp=qa",function(data){
			$.each(data,function(k,v){
				str += '<a href="<?php echo site_url('distribute/dis_ext/msg_det');?>?id=<?php echo $inter_id?>&mid='+v.eid+'" class="item">';
...
function asy_msgs(){
	var str = '';
	$.getJSON("<?php echo site_url('distribute/dis_ext/msgs_asy');?>?id=<?php echo $inter_id?>&mstyp=",function(data){
		$.each(data,function(k,v){
			str += '<a href="<?php echo site_url('distribute/dis_ext/msg_det');?>?id=<?php echo $inter_id?>&mid='+v.eid+'" class="item">';
			if(v.msg_typ == 0){
