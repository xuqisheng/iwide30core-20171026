...
class Wftpayreturn extends MY_Controller {
    ...
    public function okpay_payreturn(){
        ...
        $news = 1;//是否是新号
        if(in_array($order['inter_id'],$inter_ids)){
            $news = 0;//不是新号
        }
        if($news && isset($pay_paras['sub_ok_key_h_'.$order['hotel_id']]) && !empty($pay_paras['sub_ok_key_h_'.$order['hotel_id']]))
            $this->ClientResponseHandler->setKey($pay_paras['sub_ok_key_h_'.$order['hotel_id']]);
        elseif(isset($pay_paras['sub_key_h_'.$order['hotel_id']]) && !empty($pay_paras['sub_key_h_'.$order['hotel_id']])){
            $this->ClientResponseHandler->setKey($pay_paras['sub_key_h_'.$order['hotel_id']]);
        }else{
            $this->ClientResponseHandler->setKey($pay_paras['key']);
        }
