...
//var_dump( $_POST );die;
    ...
    public function package_send()
    {
        ...
        $giftTheme = array(
            ...
            array('theme_id'=>1,'theme'=>'theme0.jpg','theme_name'=>'精选'),
...
// var_dump( $items );die;
    ...
    public function _received()
    {
        ...
        if( $giftReceived == Soma_base::STATUS_TRUE ){
            ...
            $gift->nickname = isset( $fans_received['nickname'] ) ? $fans_received['nickname'] : '您的好友'; // 领取人昵称
    ...
    public function sharing_received()
    {
        ...
        */
        ...
        // 清空分享链接中的酒店分销员和泛分销员信息
        $redirect .= '&saler=&fans_saler=';
        ...
        if($self_saler) {
            ...
            $param_key = 'saler';
            if($self_saler['typ'] == 'FANS') { $param_key = 'fans_saler'; }
            $redirect .= '&' . $param_key . '=' . $self_saler['info']['saler'];
        } else {
            ...
            if($share_saler) {
                ...
                if($saler) {
                    $redirect.= "&saler={$saler}";
                } else {
                    $fans_saler = $this->input->get('fans_saler');
                    if($fans_saler) { $redirect.= "&fans_saler={$fans_saler}"; }
                }
            }
        }
...
//        parent::_view('gift'. DS. $file, $datas);
    ...
    public function check_follow_ajax()
    {
        ...
        $statis_url= $public['follow_page'];//http://www.baidu.com';
        
        if( isset($userinfo['subscribe']) && $userinfo['subscribe']==0 ){
            //微信返回的信息显示没有关注
            $result['status']= 2;
            $result['message']= "您还没关注公众号，可能会影响礼品使用，请先关注{$public['name']}";
            $result['data']= $statis_url;
    
        } else {
            $result['message']= "您可以在{$public['name']}公众号使用您的礼品";
