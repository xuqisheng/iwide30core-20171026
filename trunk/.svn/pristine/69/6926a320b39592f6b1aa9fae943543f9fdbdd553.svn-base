<?php
class Uploadico extends MY_Front {
	...
	function index() {
		...
		$this->display('chat/upload_ico_form', array (
	...
	function do_upload() {
		...
		if (! $this->upload->do_upload ()) {
			...
			$this->display('chat/upload_ico_form', $error );
			...
			$w = $this->input->get('w')?intval($this->input->get('w')):'';
			$h = $this->input->get('h')?intval($this->input->get('h')):'';
			...
			if ($image_width1>$image_height1) {
				if ($image_width1>$towidth) {
					$recof['width']  = $towidth;
				}
			}
			else {
				if ($image_height1>$toheidth) {
					$recof['height']  = $toheidth;
				}
			}
			...
			if (empty($isdir)) {
				...
				foreach ($arrpath as $v) {
					if ($v!='') {
						$newpath = $newpath.$v.'/';
						$isdirchild = $this->ftp->list_files($newpath);if (empty($isdirchild)) {
			...
			$this->display('chat/upload_ico_success', $data );
