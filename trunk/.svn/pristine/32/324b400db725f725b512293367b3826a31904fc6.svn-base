...
// 		$data ['order_params'] = array('{NAME}'=>'会员名');
		...
		if (! empty ( $data ['tid'] )) {
			...
			if (! empty ( $list )) {
				...
				redirect ( site_url ( 'distribute/tmmsg/index' ) );
			}
		} else {
			$msg_list = $model->get_temp_msg_list ( $this->inter_id, self::ENUM_ORDER_TMG_TYPE );
			$types = $model->get_temp_msg_types ( 'fans_dist' );
