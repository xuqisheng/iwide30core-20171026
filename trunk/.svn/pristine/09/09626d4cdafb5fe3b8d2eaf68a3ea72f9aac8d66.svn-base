...
<section class="order_message">
    ...
    <ul>
        ...
        <li><span>订单总价</span><span class="ui_price ui_color"><?php echo ($order['price'] + $order['point_favour'] + $order['coupon_favour']);?></span></li>
        <?php if($order['point_used_amount']!=0){?>
            <li><span>使用悦银数</span><span><?php echo round($order['point_used_amount']);?></span></li>
        <?php }?>
        <?php if($order['point_favour']!=0){?>
            <li><span>悦银抵用</span><span><?php echo round($order['point_favour']);?></span></li>
        <?php }?>
        <?php if($order['coupon_favour']!=0){?>
            <li><span>使用优惠券</span><span><?php echo round($order['coupon_favour']);?></span></li>
        <?php }?>
        <li><span>实付金额</span><span><?php echo $order['price'];?></span></li>
