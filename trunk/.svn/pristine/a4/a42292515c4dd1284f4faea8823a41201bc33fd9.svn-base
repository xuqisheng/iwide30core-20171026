...
class Ticket extends MY_Front {
    ...
    /**
     * 增加景区列表 （存在多个景区则显示否则跳转到商品列表）
     */
    public function scenic()
    {
        $param = $this->input->get();
        $data['inter_id'] = $this->inter_id;
        $per_page = isset($param['page_show']) && !empty($param['page_show']) ? $param['page_show'] : 30;
        $cur_page = isset($param['page_num']) && !empty($param['page_num']) ? $param['page_num'] : 1;
        //获取店铺数量
        ...
        if ($total_count == 1)
        {
            $query = array(
                ...
                'hotel_id'  => $list[0]['hotel_id'],
            );
            redirect(site_url('ticket/ticket/index?'.http_build_query($query)));
        }
        ...
        $this->display('ticket/scenic',$data);
    }
    ...
    public function scenic_list()
    {
        ...
        $data['inter_id'] = $this->inter_id;
        $per_page = $param['page_show'] ? $param['page_show'] : 20;
	...
	public function index(){
        ...
        $this->display('ticket/index',$data);
	}
...
//var_dump($group_arr);die;
    ...
    public function checkout(){
        ...
        $data['shop']['setting'] = json_encode($json_setting);
        ...
        $this->display('ticket/checkout',$data);
    }
    //获取优惠信息：优惠，券，积分
    ...
    //计算商品提前预约优惠
    public function count_ticket_discount()
    {
        $this->load->helper('appointment');
        $param = $this->input->post();
        $dissipate = trim($param['dissipate']);
        $dissipate = $dissipate/1000;
        $date = date('Y-m-d',$dissipate);
        $dissipate = date('Y-m-d H:i:s',$dissipate);//消费时间

        if (!empty($param['goods_info']) && is_array($param['goods_info']))
        {
            $goodsids = array();
            foreach($param['goods_info'] as $v)
            {
                $goodsids[] = $v['goods_id'];
            }
            ...
            $this->load->model('roomservice/roomservice_goods_model');
            $order = array(
                'inter_id'  => addslashes($param['inter_id']),
                'shop_id'   => addslashes($param['shop_id']),
                'hotel_id'  => addslashes($param['hotel_id']),
                'date'  => $date,
            );


            $goods_res = $this->roomservice_goods_model->ticket_order_goods_info($order,$param['goods_info'],$goodsids);//var_dump($goods_res);die;
            if(!empty($goods_res))
            {
                $goods_res = $this->count_ticket($goods_res,$dissipate);
                $new_arr = array();
                if (!empty($goods_res['goods_info']))
                {
                    foreach ($goods_res['goods_info'] as $key => $val)
                    {
                        $new_arr[$val['goods_id']][$val['spec_id']] = $val;
                    }
                }

                $goods_res['goods_info'] = $new_arr;
                ajax_return(1,'成功',$goods_res);
            }
        }
        ajax_return(0,'暂无数据');
    }
     ...
     * @param $dissipate 预约时间
     * @return array
     */
    protected function count_ticket($goods_res,$dissipate)
    {
        $discount_fee = 0;
        if (!empty($goods_res))
        {
                ...
                if ($value['ticket_credits'] == 2)
                {
                    if ($value['ticket_day'] > 0 && $value['ticket_limit'] > 0)
                    {
                        ...
                        if ($date <= $dissipate)
                        {
                            ...
                            {
                                $discount = $value['num'] * $value['shop_price'] * (1 - $value['ticket_limit']/10);
                            }
                            //立减
                            else
                            {
                                $discount = $value['num'] * $value['ticket_limit'];
                            }
                        }
    ...
    public function address_list(){
        ...
        $address = $this->db->where(
                        ...
                        ->result_array();
        $data['user_address'] = $address;
        $this->display('ticket/address_list',$data);
    }
    //新增 编辑收货地址页
    public function address(){
        ...
        $this->display('ticket/address',$data);
    }
    ...
    public function saveAddress(){
        ...
        $arr = array();
        if(empty($post['contact']) || empty($post['phone'])/* || empty($post['longitude']) || empty($post['latitude'])*/){
            $return['msg'] = '不能有空数据';
            echo json_encode($return);
            die;
        }
        ...
        */
        ...
        $arr['status'] = 1;
        $url = site_url('ticket/ticket/checkout?id='.$inter_id.'&hotel_id='.$hotel_id.'&shop_id='.$shop_id);
        if($addr_id){//更新
    ...
    public function saveOrder()
    {
        ...
        $order['dissipate'] = date('Y-m-d H:i:s',$dissipate);//消费时间
       ...
       // $goods_info = array(0=>array('goods_id'=>1,'num'=>2,'spec_id'=>'1'),1=>array('goods_id'=>12,'num'=>1,'spec_id'=>'234'));
        ...
        if($shop_info['sale_type'] == 4){//门票
            ...
            else if($shop_info['identify_type'] == 2 && empty($post['room_name']))
            {
                $return['msg'] = '请填写房间号信息';
                echo json_encode($return);
                die;
       ...
       // $order['type_id'] = $type_id;
        ...
        if ($out_date < $order['dissipate'])
        {
            if ($shop_info['advance_book_num'] > 0)
            {
                $return['msg'] = '可提前'.$shop_info['advance_book_num'].'天预订';
            }
            else
            {
                $return['msg'] = '只能提前24小时预订';
            }
            echo json_encode($return);
            die;
        }
        ...
        $goods_res = $this->roomservice_goods_model->ticket_order_goods_info($where_filter,$goods_info,$goodsids);
        if(empty($goods_res)){
        ...
        {
            $count_ticket = $this->count_ticket($goods_res,$order['dissipate']);
        }

        $order['goods'] = $count_ticket['goods_info'];
        $order['openid'] = $this->openid;
        $order['shipping_type'] = 0;
        $order['shipping_cost'] = 0;
        $order['ticket_discount_fee'] = $count_ticket['discount_fee'];

        //print_r($order);exit;
        //插入订单表
        $this->load->model('roomservice/roomservice_orders_model');
        $result = $this->roomservice_orders_model->checkout_ticket($order,$shop_info);
        if($result['errcode'] == 1){
        ...
        if($result['data']['pay_type'] == 1){//微信支付
            $result['data']['pay_url'] = site_url('wxpay/ticket_pay') . '?id=' .$inter_id .'&order_id=' .$result['data']['order_id'];//默认使用微信原生配置;
        }elseif($result['data']['pay_type'] == 2){//储值支付
            $res = $this->roomservice_orders_model->pay_order_in_banlance($inter_id,$result['data']['order_id']);
            if($res)
            {
                //判断是否即时确认  自动接单
                if ($shop_info['instant_confirm'] == 1)
                {
                    $order['order_id'] = $result['data']['order_id'];
                    $order['openid'] = $openid;
                    $this->roomservice_orders_model->auto_update_status($inter_id,$order);
                }

                $result['msg'] = '支付成功';
                $result['data']['pay_url'] = site_url('ticket/ticket/order_detail?id='.$inter_id.'&hotel_id='.$order['hotel_id'].'&oid='.$result['data']['order_id']);
            }else{
                $result['errcode'] = 1;
                $result['msg'] = '支付失败';
            }
            ...
            $result['data']['pay_url'] = site_url('wftpay/ticket_pay') . '?id=' .$inter_id .'&order_id=' .$result['data']['order_id'];//默认使用威富通配置;
        }
        echo json_encode($result);
        die;
    ...
    function order_detail(){
        ...
        $this->load->model('roomservice/roomservice_orders_model');
        $data['order'] = $this->roomservice_orders_model->get_order_simple(array('inter_id'=>$inter_id,'openid'=>$openid,'order_id'=>$order_id,'type'=>4));
        if(!empty($data['order'])){
            //读取店铺信息
            ...
            if($data['order']['pay_way'] == 1){//wx
                $data['order']['pay_url'] = site_url('wxpay/ticket_pay') . '?id=' .$inter_id .'&order_id=' .$data['order']['order_id'];//默认使用微信原生配置;
            }elseif($data['order']['pay_way'] == 4){//威富通
                $data['order']['pay_url'] = site_url('wftpay/ticket_pay') . '?id=' .$inter_id .'&order_id=' .$data['order']['order_id'];//默认使用微信原生配置;
            }

            //再来一单地址
            $query = array(
                'id'        => $data['order']['inter_id'],
                'hotel_id'  => $data['order']['hotel_id'],
                'shop_id'   => $data['order']['shop_id'],
            );
            $data['order']['again_url'] = site_url('/ticket/ticket/index?'.http_build_query($query));
            ...
            $data['signPackage'] = $this->common_data;
            $this->display('ticket/order_detail',$data);
        }else{
            echo 'data error';
            die;
        }
    ...
    public function orderfollow(){
        ...
        if($order){
            ...
            $data['action'] = $action;
            $this->display('ticket/orderfollow',$data);
        }else{
            echo 'error data';
            die;
        }
    ...
    public function cancelOrder(){
        ...
        if(!empty($order)) {
            ...
            $orderModel = $this->roomservice_orders_model;
            if($order['order_status']==$orderModel::OS_PER_CANCEL ||$order['order_status']==$orderModel::OS_HOL_CANCEL||$order['order_status']==$orderModel::OS_SYS_CANCEL || $order['order_status'] == $orderModel::OS_FINISH){//取消或者完成
                $return['msg'] = '订单已经核销或者取消';
                echo json_encode($return);
                die;
            }
            if($order['order_status'] == $orderModel::OS_CONFIRMED){
                $return['msg'] = '订单已经接单，请电话联系取消';
                echo json_encode($return);
                die;
            }
            //记录订单操作日志
                ...
                if($id){
                    ...
                    if(1 == $order['pay_way']){//微信支付
                        ...
                        /*if(is_array($refund_result) && "WAITING_FAIL" == $refund_result['return_code'] ) {
                            ...
                            {
                                //发送模板消息
                                $orderModel->handle_order($this->inter_id,$order_id,'',26);
                                $array = array(
                                    'inter_id'=>$this->inter_id,
                                    'openid' =>'',
                                    'order_id' => $order_id,
                                ...
                                $this->db->insert('roomservice_action',$array);
                                $order_log['action_note']= " | 分账发起微信退款";
                                $this->db->insert('roomservice_orders_log',$order_log);//记录订单操作记录
                                $return['errcode'] = 0;
    ...
    public function orderlist(){
        $openid		= $this->session->userdata($this->inter_id."openid");
        $inter_id	= $this->inter_id;
        $type= $this->input->get('type')?intval($this->input->get('type')):4;//默认客房
        $data = array();
        $data['inter_id'] = $this->inter_id;
        $data['type'] = $type;
        //查询公众号信息
       ...
       // $shops = $this->db->where(array('inter_id'=>$inter_id,'is_delete'=>0))->get('roomservice_shop')->result_array();
        ...
        $order = $this->roomservice_orders_model->get_list(array('inter_id'=>$inter_id,'openid'=>$openid,'type'=>4));
        $order_detail  =  array();
        if($order){
            ...
            foreach($order as $k=>$v){
               ...
               // $detail = $this->roomservice_orders_model->get_order_item_detail(array('inter_id'=>$inter_id,'openid'=>$openid,'order_id'=>$v['order_id']));
                ...
                if($v['pay_way'] == 1){//wxpay
                    $order[$k]['pay_url'] = site_url('wxpay/ticket_pay') . '?id=' .$inter_id .'&order_id=' .$v['order_id'];//默认使用微信原生配置;;
                }elseif($v['pay_way'] == 4){//wft
                    $order[$k]['pay_url'] = site_url('wftpay/ticket_pay') . '?id=' .$inter_id .'&order_id=' .$v['order_id'];//默认使用威富通配置;;
                }
                ...
                $order[$k]['again_url'] = site_url('/ticket/ticket/index?'.http_build_query($query));
            }
        }

        $data['orderlist'] = $order;
        $this->display('ticket/orderlist',$data);
    }
