...
<div class="whitetable">
    ...
    <div class="bd_left list_layout">
        ...
        <div id="shipping_cost" <?php if(isset($posts['sale_type']) && $posts['sale_type'] != 3){?>style="display: none;"<?php }?>>
            <div>配送费</div>
        ...
        <div>
            ...
            <div class="flex flexcenter" style="max-width:100%">
                <?php $day = array(1=>'联系人',2=>'手机号')?>
                <?php foreach($day as $k=>$v){?>
                    <label class="check"><input name="verify_info[]" type="checkbox" value="<?php echo $k?>" <?php echo !empty($posts['verify_info'])&&in_array($k,$posts['verify_info'])?' checked ':''?> /><span class="diyradio"><tt></tt></span><?php echo $v;?></label>
                <?php }?>
            </div>
        </div>
    </div>
