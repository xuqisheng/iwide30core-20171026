...
class Finance_model extends MY_Model {
	...
	public function filter( $params=array(), $select= array(), $format='array' )
	{
		...
		*/
		...
		$status_des = $this->Enum_model->get_enum_des("PAY_WAY");
		//print_r($status_des);
		
		foreach ($resulthotels as $key => $value) {
			$hotel_id = $value['hotel_id'];
			$hotelsinfo = $this->_db()->query("SELECT * FROM ".$this->db->dbprefix."hotels WHERE hotel_id='".$hotel_id."'")->result_array();
			$hotelname = $hotelsinfo[0]['name'];
			$value['hotelname'] = $hotelname;
			$value['paytype'] = isset($status_des[$value['paytype']])?$status_des[$value['paytype']]:'其它方式';
