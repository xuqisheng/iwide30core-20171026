...
class Center extends MY_Front_Wxapp
{
...
//        $cards = $this->igetcard->getCardsByOpenid($openid);
        ...
        $this->out_put_msg ( 1, '', $data ,'member/index/center');
...
// 		$data['info'] =$this->member_model->get_fans_info($openid);
		...
		if($memberinfo) {
			...
			$this->out_put_msg ( 1, '', $data ,'member/userinfo');
			//$this->display('member/userinfo', $data);
		} else {
			$this->out_put_msg ( 2, '服务超时', "" ,'member/userinfo');
	...
	{		
		...
		$num = $this->session->userdata('sms');
		$this->set_user_session("sms", $num);
		$this->out_put_msg ( 1, '已发短信至手机' );
		//echo $result;
	}
...
//            $this->db->inset( 'iwide_weixin_text',array('content'=>json_encode()));
            $x = json_encode(array('sessionCode'=>$this->user_session('code'),'userCode'=>$picCode));
