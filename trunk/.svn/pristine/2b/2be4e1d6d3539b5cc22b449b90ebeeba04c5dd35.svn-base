          ...
          <div class="row">
            <div class="col-xs-12">
              ...
              <!-- 
                ...
                <div class="row">
                	<div class="col-sm-12">
                	<?php echo form_open('distribute/distri_report/exsale_get_fans/','class="form-inline"')?>
...
?>                        	<th width="10%" class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="<?php echo $item['name'];?>: activate to sort column ascending"><?php echo $item['name'];?></th>
				...
				$fields[] = $key;
                    ...
                    <tr>
                    	 ...
                    	 <td>
                    	 <?php 
                    	 $level = $levels[intval($item['level'])];
                    	 echo $level; 
                    	 ?>
                    	 </td>
                    	 <td>
                  ...
                  <div class="row">
	                  <div class="col-sm-5">
	                  	<div class="dataTables_info" id="data-grid_info" role="status" aria-live="polite">共<?=$total?>条<a class="btn btn-sm bg-green" href="<?php echo site_url("distribute/distri_report/ext_exsale_get_fans/".$posts['hotel_id'].'_'.$posts['begin_time'].'_'.$posts['end_time'].'_'.$posts['source'])?>">导出</a></div>
	                  </div>
	                  <div class="col-sm-7">
...
// 	  modal.find('.modal-body input').val(recipient)
	...
	$.getJSON('<?php echo site_url("distribute/distri_report/get_cofigs?ctyp=dist_sale_get_fans")?>',function(data){
		if(data != null){
...
$('#set_btn_save').click(function(){
	$.post('<?php echo site_url("distribute/distri_report/save_cofigs?ctyp=dist_sale_get_fans")?>',$("#setting_form").serialize(),function(data){
		if(data == 'success'){
