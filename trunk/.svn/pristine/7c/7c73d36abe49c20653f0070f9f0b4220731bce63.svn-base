	...
	<a onclick="go_hotel('<?php echo site_url('hotel/hotel/index?id=').$inter_id.'&h='.$r->hotel_id;?>')" tmp="<?php echo $r->hotel_id?>" href="javascript:void(0);" class="item">
        ...
        <div class="allin_box">
            ...
            <?php if(!empty($r->lowest)){?>
            	...
            	<?php }else{?>
            	<div id="lowest_p_<?php echo $r->hotel_id;?>" class="h6 color_main_gray">暂无价格</div>
            	<?php }?>
                <div class="h6 color_main_gray"><?php if(isset($r->distance)){?>距离<span><?php echo $r->distance;?></span>Km<?php }?></div>
            </div>
...
<?php if(isset($r->comment_data['comment_count'])){?><span class="color_main_gray"><?php echo $r->comment_data['comment_count'];?>条评论</span><?php }?></div>
            <?php }?>
