	...
	<div class="item" onClick="silde_down(this)">
        <div class="item_left" rid="<?php echo $r['room_info']['room_id']; ?>">
           ...
           <!-- <?php if(!empty($r['room_info']['imgs']['hotel_room_service'])){foreach($r['room_info']['imgs']['hotel_room_service'] as $rs){echo $rs['info'].' '; }}?> --> 
            ...
            <div class="backvote" style="display:none">入住返券</div>
        </div>
       ...
       <!--- <div class="isfull item_right showprice">  <!-- 满房需要加isfull 变灰色字体-->
        <div class="item_right showprice">
            ...
            <em class="iconfont ui_color">&#x34;</em>
        </div>
        <?php }else {?>
...
function show_room_detail(obj,event){ 
	...
	$('.detail_pull .room_img img').attr('src','');
	$('#detail_service').html('');
	$('.detail_pull .content').html('');
	$.post("<?php echo site_url('hotel/hotel/return_room_detail').'?id='.$inter_id; ?>",{
		...
		else  tmphtml+='<span class="slideson ui_img_auto_cut"><img src="'+data.room_img+'" /></span>';
		tmphtml+='</div>';
...
// 					});
// 				}
				temp+=n.room_info.sub_des+'</div><div class="backvote" style="display:none;">入住返券</div></div>';
				if(n.state_info!=''){
					temp+='<div class="item_right showprice">';
					temp+='<div class="ui_price"><b>'+n.lowest+'</b><tt class="ui_color_gray">起</tt></div><em class="iconfont ui_color">&#x34;</em></div>';
				}
				else{
