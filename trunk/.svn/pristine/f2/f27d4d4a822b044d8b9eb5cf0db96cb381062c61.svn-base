          ...
          <div class="row">
            <div class="col-xs-12">
              ...
              <!-- 
                ...
                <div class="row">
                	<div class="col-sm-12">
                	<?php echo form_open('distribute/distribute/incomes')?>
                	...
                	<label>开始日期 <input type="text" name="begin_time" class="form-control input-sm" placeholder="" aria-controls="data-grid"></label>
                	<label>结束日期 <input type="text" name="end_time" class="form-control input-sm" placeholder="" aria-controls="data-grid"></label>
                		<button type="submit" class="btn btn-sm bg-green" id="grid-btn-search"><i class="fa fa-search"></i>&nbsp;检索</button>
                	<label><input type="checkbox" name="dis_cate" value="1" id="valid_only"<?php if($this->session->userdata('income_sort_type') == 'VALID'):echo ' checked';endif;?> />只显示金额大于0的记录</label>
                	</div>
                	<div class="btn-group">
                	</div>
                	</form>
                  ...
                  <table id="data-grid" class="table table-bordered table-striped table-condensed">
                    <thead>
                    	<tr role="row">
                    		...
                    		<th width="10%" class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="订单号: activate to sort column ascending">订单号</th>
                    		<th width="10%" class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="产品: activate to sort column ascending">产品</th>
                    		...
                    		<th width="10%" class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="产生时间: activate to sort column ascending">产生时间</th>
                    		<th width="5%" class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="状态: activate to sort column ascending">状态</th></tr></thead>
                    ...
                    <tbody><?php foreach ($res as $income):?>
                    	<tr>
                    		...
                    		<td><?=$income->order_amount?></td>
                    		<td><?=$income->grade_amount?></td>
                    		<td><?=$income->grade_total?></td>
                    		<td><?=$income->grade_time?></td>
                    		<td><a href="javascript:;" onclick="send(<?=$income->id?>);">发放</a></td>
                    	</tr><?php endforeach;?>
                    </tbody>
                  </table>
                  <div class="row">
...
$buttions.= '<button type="button" class="btn btn-sm bg-green" id="grid-btn-send-all"><i class="fa fa-plus"></i>&nbsp;批量发放绩效</button>';
...
$(document).ready(function() {
	$('#valid_only').change(function(){
		window.location.href="<?php echo site_url('distribute/distribute/incomes')?>?sort="+($(this).prop('checked') ? 'VALID' : 'ALL');
	});
