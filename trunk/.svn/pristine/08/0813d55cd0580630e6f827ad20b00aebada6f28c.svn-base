...
//     	$this->load->model('wx/Weixin_model');
    	if($data){
    		...
    		if($sign == sha1($raw.$access_token)){
    			echo $this->sendCustomMsg($this->input->get('id',TRUE),$raw) ? 'SUCCESS' : 'FAILD';
    		}else {
    			log_message('error','签名错误 | '.$sign.' | '.$raw.$access_token.' | '.sha1($raw.$access_token));
    			echo '签名错误';
    		}
    	}else{
    		if(!empty($this->getCusMsgStatus($content)))
    			echo $this->setCusMsgClose($content) ? 'SUCCESS' : 'FAILD';
    		else 
    			echo 'FAILD | '.$this->getCusMsgStatus($content);
    	}
    }
	...
	public function rec(){
		...
		$sign = sha1('{"touser":"o9Vbtw9PgJbfM8ia-GJuzq0TTn2k","msgtype":"text","text":{"content":"测试客服消息"}}'.$access_token);
		$res = '{"sign":"'.$sign.'","touser":"o9Vbtw9PgJbfM8ia-GJuzq0TTn2k","msgtype":"text","text":{"content":"测试客服消息"}}';
		echo file_get_contents ( 'php://input' );
		log_message('error','------------- | '.file_get_contents ( 'php://input' ).' | '.$res);
		die;
		echo $res;
	}
