...
<div class="wrapper">
    ...
    .j_head >div:nth-of-type(4){width:255px;}
            ...
            <div class="content-wrapper" style="min-width:1130px; ">
                ...
                <div class="contents_list bg_fff" style="font-size:13px;margin-top:20px">
                    <div class="classifications display_flex">
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==1)echo 'class="add_active"'?>><a href="<?php echo site_url('ticket/orders/index?order_status=1')?>">待付款</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==2)echo 'class="add_active"'?>><a href="<?php echo site_url('ticket/orders/index?order_status=2')?>">待接单</a></div>
                        <!--
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==3)echo 'class="add_active"'?>><a href="<?php echo site_url('ticket/orders/index?order_status=3')?>">待配送</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==4)echo 'class="add_active"'?>><a href="<?php echo site_url('ticket/orders/index?order_status=4')?>">配送中</a></div>
                       -->
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==3)echo 'class="add_active"'?>><a href="<?php echo site_url('ticket/orders/index?order_status=3')?>">待消费</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==5)echo 'class="add_active"'?>><a href="<?php echo site_url('ticket/orders/index?order_status=5')?>">已消费</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==6)echo 'class="add_active"'?>><a href="<?php echo site_url('ticket/orders/index?order_status=6')?>">已取消</a></div>
                        <div <?php if(isset($filter['order_status'])&&$filter['order_status']==99)echo 'class="add_active"'?>><a href="<?php echo site_url('ticket/orders/index')?>">所有状态</a></div>
                    </div>
                </div>
                <div style="padding: 10px;">
                    <form class="form" method='get' id="this_form" action='<?php echo site_url('ticket/orders/index')?>'>
                        <input type="hidden" name="order_status" value="<?php echo isset($filter['order_status'])?$filter['order_status']:''?>"/>
                            ...
                            <span>
                                ...
                                <div>
                                    ...
                                    <span class="t_time"><input name="book_start_time" type="text" id="datepicker"
                                                                class="datepicker moba form_datetime" value="<?php echo empty($filter['book_start_time']) ? '' : $filter['book_start_time']?>"></span>
                                    <font>至</font>
                                    <span class="t_time"><input name="book_end_time" type="text" id="datepicker2"
                                                                class="datepicker moba form_datetime" value="<?php echo empty($filter['book_end_time']) ? '' : $filter['book_end_time']?>"></span>
                                </div>
                                <div>
                                    <span>关键字</span>
                        ...
                        <div class="border_1 m_t_10 bg_fff">
                            <div class="bg_f8f9fb fomr_term p_0_30_0_10 b_b_1">
                                <div>
                                    ...
                                    <span style="float:right">消费时间：<?php echo $list['dissipate'] ? $list['dissipate'] : '--';?></span>
                            ...
                            <div>
                                ...
                                <?php } elseif ($list['order_status'] == $orderModel::OS_CONFIRMED && $list['pay_status'] == $orderModel::IS_PAYMENT_YES && $list['pay_way'] != 3 || $list['pay_way'] == 3 && $list['order_status'] == $orderModel::OS_CONFIRMED) { ?>
                                    <span>待核销</span><br>
                                    <span class="blue"
                                          onclick="change_status('<?php echo $list['order_id'] ?>' , '<?php echo $orderModel::OS_FINISH ?>')">核销</span> &nbsp;|&nbsp;
                                    <span class="red"
                                          onclick="change_status('<?php echo $list['order_id'] ?>' , '<?php echo $orderModel::OS_HOL_CANCEL ?>')">取消</span>
    ...
    <script>
        ...
        $('.cancel_dd').click(function()
        {
            ...
            $.ajax({
                dataType:"json",
                type:'post',
                url:"<?php echo site_url('/ticket/orders/dadaCancelOrder')?>",
                data:data,
        ...
        $('.send_dd').click(function()
        {
            ...
            $.ajax({
                dataType:"json",
                type:'post',
                url:"<?php echo site_url('/ticket/orders/reSendOrderToDada')?>",
                data:data,
                success: function(res)
                {
                    alert(res.msg);
        ...
        $('.confirms_remark').click(function()
        {
            ...
            $.ajax({
                dataType:"json",
                type:'post',
                url:"<?php echo site_url('/ticket/orders/save_remark')?>",
                data:data,
                success: function(res)
                {
                    alert(res.msg);
        ...
        $('.confirms_order').click(function()
        {
            ...
            if(a)
            {
                ...
                $.ajax({
                    dataType:"json",
                    type:'post',
                    url:"<?php echo site_url('/ticket/orders/refund_goods')?>",
                    data:data,
                    beforeSend: function()
                    {
    ...
    <script>
        ...
        function print_order(order_id){
            if(order_id){
                ...
                $.post('<?php echo site_url('ticket/orders/print_order');?>',{
                    oid:order_id,
                    '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'
                },function(data){
                    if(data.errcode==0){
