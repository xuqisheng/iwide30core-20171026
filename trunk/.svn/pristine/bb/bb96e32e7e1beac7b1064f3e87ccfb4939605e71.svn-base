...
<section class="order_message">
    ...
    <ul>
        ...
        <li><span>订单总价</span><span class="ui_price ui_orgin"><?php echo $order['price'];?></span></li>
        <?php if($order['coupon_favour']!=0){?>
        <li><span>优惠金额</span><span class="ui_price"><?php echo $order['coupon_favour'];?></span></li>
        <?php } ?>
    ...
    <?php if($order['paytype']=='weixin' && isset($order['refund'])){?>
        <li><span>退款状态</span><span><?php if($order['refund']==0){?>未退款<?php }elseif($order['refund']==1){?>退款成功<?php }else{?>退款失败<?php }?></span></li>
    <?php }?>
    </ul>
</section>
