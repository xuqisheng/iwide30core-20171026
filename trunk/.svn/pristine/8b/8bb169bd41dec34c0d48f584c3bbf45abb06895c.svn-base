...
// 						continue;
					...
					if(!empty ($external [$rm ['room_id']] [$p ['RateCode']])){
						foreach($external [$rm ['room_id']] [$p ['RateCode']] as $local){
							...
							if (isset($level_reflect['discount_price_code'][$local ['price_code']])&&!empty($no_discount)){
							    continue;
							}
							if (isset($level_reflect['local_stock_price_code'][$local ['price_code']])){
							    $local ['least_num'] > $least_num and $local ['least_num'] = $least_num;
							    $local ['least_num'] > 1 and $local ['least_num'] = 1;
							    $local ['book_status'] = $local ['least_num'] > 0 ? 'available' : 'full';
							}else{
		...
		if(!empty ($order)){
			...
			if($room_codes ['code'] ['price_type'] == 'pms' || !empty($room_codes ['code'] ['extra_info'] ['pms_code'])){
				if($room_codes ['code'] ['extra_info'] ['type'] == 'discounts'){
					...
					$remark .= 'pms活动价：' . $order ['first_detail'] ['price_code_name'].'。';
				} else{
				    $remark .= $room_codes ['code'] ['price_type'] == 'pms'?'pms价格：' . $order ['first_detail'] ['price_code_name'].'。':'微信自定义价格：' . $order ['first_detail'] ['price_code_name'].'。';
