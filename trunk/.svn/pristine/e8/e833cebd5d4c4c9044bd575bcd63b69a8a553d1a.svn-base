...
if(isset($admin)){
    $buttions.= '<button type="button" class="btn btn-sm disabled" id="grid-btn-edit-publics"><i class="fa fa-edit"></i>&nbsp;编辑公众号</button>';
}
...
var dataSet= <?php echo json_encode($list); ?>;
...
var url_edit_publics= '<?php echo EA_const_url::inst()->get_url("*/*/edit_publics"); ?>';	//跟button对应
...
if( count($result['data'])<$num){
    ...
    $gridjs= VIEWPATH. $tpl .DS . $m. DS. 'adminuser_operate/gridjs.php';
    ...
    $gridjs= VIEWPATH. $tpl .DS . $m. DS. 'adminuser/gridjs_ajax.php';
    if( file_exists($gridjs) )
        require_once $gridjs;
    else
