          ...
          <div class="row">
            <div class="col-xs-12">
              ...
              <!-- 
                ...
                <div class="row">
                	<div class="col-sm-12">
                	<?php echo form_open('distribute/distri_report/exnew_fas_hotel/','class="form-inline"')?>
                	<div class="form-group">
                		<label>开始日期 </label>
                		<input class="form_datetime form-control input-sm" data-date-format="yyyymmdd" type="text" name="date_begin" class="form-control input-sm" placeholder="" aria-controls="data-grid" value="<?php echo empty($posts['begin_time']) ? '' : $posts['begin_time']; ?>">
                		<label>至 </label>
                		<input class="form_datetime form-control input-sm" data-date-format="yyyymmdd" type="text" name="date_end" class="form-control input-sm" placeholder="" aria-controls="data-grid" value="<?php echo empty($posts['end_time']) ? '' : $posts['end_time']; ?>">
                	</div>
                	<div class="btn-group">
...
?>                        	<th width="10%" class="sorting" tabindex="0" aria-controls="data-grid" rowspan="1" colspan="1" aria-label="<?php echo $item['name'];?>: activate to sort column ascending"><?php echo $item['name'];?></th>
				...
				$fields[] = $key;
                    ...
                    <tr>
                    	 <td><?=$item['name']?></td>
                    	 <td><?=empty($item['cnt'])?0:$item['cnt']?></td>
                    	 <td><?=$item['gz']?></td>
                    	 <td><?=$item['pm']?></td>
                    	 <td><?=empty($item['zjx'])?0:$item['zjx']?></td>
                    </tr>
                    <?php endforeach;?>
                    </tbody>
                  </table>
                  ...
                  <div class="row">
	                  <div class="col-sm-5">
	                  	<div class="dataTables_info" id="data-grid_info" role="status" aria-live="polite">共<?=$total?>条<a class="btn btn-sm bg-green" href="<?php echo site_url("distribute/distri_report/ext_fans_hotel/".$posts['begin_time'].'_'.$posts['end_time'])?>">导出</a></div>
	                  </div>
	                  <div class="col-sm-7">
...
// 	  modal.find('.modal-body input').val(recipient)
	...
	$.getJSON('<?php echo site_url("distribute/distri_report/get_cofigs?ctyp=dist_fans_hotel")?>',function(data){
		if(data != null){
...
$('#set_btn_save').click(function(){
	$.post('<?php echo site_url("distribute/distri_report/save_cofigs?ctyp=dist_fans_hotel")?>',$("#setting_form").serialize(),function(data){
		if(data == 'success'){
