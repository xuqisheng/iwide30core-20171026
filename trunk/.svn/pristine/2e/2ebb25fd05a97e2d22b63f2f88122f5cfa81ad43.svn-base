        ...
        if($inter_id=='a472731996'){  //测试环境是 a469428180，正式是 a472731996
            ...
            if(isset($userInfo['data']) && !empty($userInfo['data'])  ){ //暂时写死
                ...
                if(in_array($lvl_pms_code,array(2,3))){  //暂时写死了
                    ...
                    if(!$fansInfo){
                        $deliver_infos = $this->getCI()->publics_model->get_public_by_id ( $openid_rel_model->get_redis_key_status('__DISTRIBUTION_DELIER_ACCOUNT') );
                        $url =  'http://credit.iwide.cn' . $_SERVER ['REQUEST_URI'] ;
                        $site_url = prep_url($deliver_infos['domain']).'/distribute/dis_ext/auto_back/'.'?id='.$openid_rel_model->get_redis_key_status('__DISTRIBUTION_DELIER_ACCOUNT').'&f='.base64_encode($inter_id.'***'.$openid.'***'.$url);
...
//                        }
    ...
    public function buycard($inter_id,$openid){
        ...
        $_card_info = $card_info['data'];

        if(isset($_card_info['is_distribution']) && $_card_info['is_distribution']=='t' && $inter_id == 'a472731996'){
...
//        $this->_ajaxReturn('ok!',base_url("index.php/membervip/depositcard/edituser?cardId={$cardId}&pay={$pay_type}"),1);
//        print_r($_POST);exit;
       // $this->_ajaxReturn('ok!',base_url("index.php/membervip/depositcard/pay?cardId={$cardId}&pay={$pay_type}"),1);
    }
    ...
    public function pay($inter_id,$openid,$_token){
        ...
        //创建支付订单
        $userData = $this->getCI()->session->get_userdata();
        if(isset($userData['salesId']) && !empty($userData['salesId'])) $distribution_num = $userData['salesId'];
        ...
        if(isset($create_info['err'])){
            ...
            if($pay_type == 'balance'){
                ...
                $data['redirect'] = base_url("index.php/wxpay/vip_pay?orderId=".$create_info['data']);
            }
            return $data;
        }
    }
     ...
     *
     */
    public function buydeposit($inter_id,$openid,$_token,&$_template,$_template_filed_names){
        $userInfo = $this->check_user_login($inter_id,$openid);
        if(!empty($userInfo['redirect']) || (!empty($userInfo['err']) && $userInfo['err']==40003)){
            return $userInfo;
        }
