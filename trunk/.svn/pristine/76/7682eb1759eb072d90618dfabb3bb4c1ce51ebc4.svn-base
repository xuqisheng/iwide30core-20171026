...
<div class="wrapper">
    ...
    <style>
        ...
        .l_button_yellow{background-color: #ff9900;display: inline-block;padding: 7px 20px;border-radius: 5px;cursor: pointer;border:1px solid #ff9900;}
        .l_button_yellow a{color: white;}
        ...
        .serch_v{width: 190px;}
        .l_button_white{background-color: white;color: #7e8e9f;display: inline-block;padding: 7px 20px;border-radius: 5px;cursor: pointer;border:1px solid #b5b5b5;margin-left: 5px;}
        .l_content_list td{padding: 10px 0;}
    </style>
            ...
            <div class="content-wrapper" style="min-width:1130px; ">
                ...
                <div  class="l_content clearfix">
                    <div class="l_content_topper">
                        <div class="l_content_rows" style="margin:0px;">
                            <span class="l_button_yellow"><a href="<?php echo site_url('tips/qrcodelist/create_qrcode')?>" target="_blank">批量生成</a></span>
                            <span class="l_button_white" id="Exportreport">批量导出</span>
                        </div>
                        <div class="floatr">
                            ...
                            <div class="l_content_rows">
                                <select class="form-control l_select" id="j_dept">
                                    <option value="">所有部门</option>
                                    <?php if(!empty($depts)){
                                        foreach($depts as $k=>$v){
                                            if(empty($v->master_dept))continue;
                                            ?>
                                            <option value="<?php echo $v->master_dept?>"><?php echo $v->master_dept?></option>
                                            <?php
                                        }}
                                    ?>
                                </select>
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for(var item in _json){
                ...
                _html += "<tr>";
                //***内容***
                _html += "<td>" + _json[item]["saler_name"] + "</td>"//打赏用户
                _html += "<td>" + _json[item]["saler"] + "</td>"//打赏时间
                _html += "<td>" + _json[item]["hotel_name"] + "</td>"//酒店名称
                _html += "<td>" + _json[item]["master_dept"] + "</td>"//酒店名称
                _html += "<td><img width='80px' src=" + _json[item]["url"] + " ></td>"//分销号
...
$("#Exportreport").on("click",function(){
    //导出报表
     window.location.href="<?php echo site_url('tips/qrcodelist/output_qrcode')?>?all=1";
})
</script>
</body>
</html>
