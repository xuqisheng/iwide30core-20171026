                    ...
                    <div class="showchild">
                      ...
                      <p>会员等级</p>
                        <ul>
                          <li>
                          <label><input type="checkbox" class="checkall"> 全选</label>
                          <label><input type="checkbox" name="lv_all" <?php if(isset($row['id']) && $row['beyond']==1 && $row['lv_all']==1) echo 'checked';?>> 不限制等级</label>
                          </li>
                          <?php foreach ($levels as $lk =>$lv) {
                            echo "<li><label><input type='checkbox' name='fans_lv' value='$lk'";
                            if(isset($row['id']) && $row['beyond']==1 && in_array($lk,$row['mem_levels'])){
                              echo 'checked';
                            }
                            echo ">$lv</label></li>";
                          }
...
function sub(){
  ...
  var mem_levels = new Array();//适用会员等级
  ...
  if(beyond == '1'){//粉丝归属
    ...
    $("input[name='fans_lv']:checked").each(function(index){
        mem_levels[index] =$(this).val();
    });
    if($("input[name='lv_all']").is(':checked')){
      var lv_all = 1;
    }else{
      var lv_all = 2;
    }
  ...
  if(beyond == 1 && mem_levels.length == 0 && lv_all == 2){
    $('#tips').html('请选择至少一个会员等级');
    return false;
  }
