...
class Publics_admin extends MY_Admin {

	protected $label_module= '运营公众号';
	protected $label_controller= '运营公众号';
...
// 				'gallery'=> $gallery,
    ...
    //				$message= ($result)?
                    if($result==0){
                        $inter_id= $this->session->get_admin_inter_id();
                        $this->load->model('wx/Public_admin_model');
                        $res=$this->Public_admin_model->update_publics($adminid,$inter_id,$post['inter_id']);
                        if($res){
