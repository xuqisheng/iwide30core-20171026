...
$.fn.extend({
	timePicker: function (setting) {
		...
		var fillDate = function (tmpday) {
			...
			if (option.showMore) {
				...
				moreDay.datePicker({
					...
					disabledTimes: option.disabledTimes,
					callback: function (date) {
		...
		var fillTime = function (CurDate) {
			...
			var isfirst = true;
			var curDateStr = $.getnumber(CurDate.getFullYear()) + '-' + $.getnumber(CurDate.getMonth() + 1) + '-' + $.getnumber(CurDate.getDate());
			var disableTimeRange = option.disabledTimes[curDateStr];
			for (var i = 0; i < option.range.length; i++) {
				...
				while (start.getTime() <= end.getTime()) {
					...
					if (disableTimeRange && (disableTimeRange.full || tTime >= disableTimeRange.start && tTime <= disableTimeRange.end) || tTime < CurDate.getTime() || tTime < new Date().getTime()) { 
						TimeBtn.addClass('forbidden'); 
					}
					else {
						if (tTime == CurDate.getTime() || isfirst) {
	...
	datePicker: function (setting) {
		...
		var FillMonth = function (curDate) {
			...
			for (var i = 1; i <= monthDays; i++) {
				...
				var tempDateStr = $.getnumber(tempDate.getFullYear()) + '-' + $.getnumber(tempDate.getMonth() + 1) + '-' + $.getnumber(tempDate.getDate());
				var shouldDisable = option.disabledTimes[tempDateStr] && option.disabledTimes[tempDateStr].full
				if (tempDate.getTime() >= today.getTime() && tempDate.getTime() < MaxDay.getTime() && !shouldDisable) {
