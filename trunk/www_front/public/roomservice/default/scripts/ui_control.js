...
$.extend({
	getsession:function(index){
		if(window.sessionStorage){
			if(window.sessionStorage[index]!=undefined)
				return window.sessionStorage[index];
		}
		return '';
	},
	...
	setsession:function(index,val){
		if(window.sessionStorage){
			window.sessionStorage[index]=val;
			return true;
		}
		return false;
	},
	setlocal:function(index,val){
		if(window.localStorage){
			window.localStorage[index]=val;
			return true;
		}
		return false;
	}
});
function down_num(e,_this){
	e.preventDefault();
	var input =  $(_this).siblings('.result_num').find('input');
	var $min = input.attr('min');
	if(input.attr('min')==undefined||input.attr('min')=='')$min=0;
	...
	if( val>$min&&val>0){
		...
		if(input.val()==0){
			$(_this).hide();
			$(_this).siblings('.result_num').hide();
		}
		return true;
	}
	return false;
}
function result_num(_this){
...
function up_num(e,_this){
	...
	if( val<$max){
		input.val(val+1);
		$(_this).siblings('.down_num').show();
		$(_this).siblings('.result_num').show();
		return true;
	}	
	return false;
}
...
$(function(){
	...
	if($('.num_control').length>0){
		$('.num_control').each(function(){
			var input = $(this).find('input');
			var val = parseInt(input.val());
			if( val==0){
				$('.down_num',this).hide();
				$('.result_num',this).hide();
			}else{
				$('.down_num',this).show();
				$('.result_num',this).show();
			}
		})
	}
});