...
$.fn.imgscroll = function(options) {
	...
	var creat_css =function(){
		var tmp ='<style>.'+option.partent_div+'{width:100%;height:'+oWidth/option.imgrate+'px;overflow: '+option.overflow+';position: relative;}';
			...
			tmp+='.'+option.partent_div+' .slideson{position:relative;font-size:1rem; height:100%;display:inline-block; width:100%;float:left;padding:'+option.imgpadding+'}';
	...
	creat_css();
	function swiPelft(){
		if(count+1>oLength*2 - 1&&!option.isround){
			option.callback({"index":count-oLength});return;
		}
		count++;
	...
	function swiPergt(){
		if(count-1<oLength&&!option.isround){
			option.callback({"index":count-oLength});
			return;
		}
		count--;
