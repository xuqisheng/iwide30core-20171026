...
function pageloading (_str){ //页面loading动画
	if(!_str) _str='正在加载';
	var tmp = '<div class="page_loading"><p class="isload">'+_str+'</p></div>';
	$('body').prepend(tmp);
}
var tmptime;
tmptime=window.setTimeout(function(){
	...
	tmptime=window.setTimeout(function(){
		$('.page_loading p').removeClass('isload').addClass('error').html('网络似乎有点问题，请稍后刷新重试~');
	},30000);
},10000);
