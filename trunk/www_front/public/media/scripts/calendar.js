...
$.fn.cusCalendar = function(options) {
	...
	//$('table').html();
	function generate(_selectedDateBegin,_selectedDateEnd,selectBeginDate,opts){
		$('body').css('overflow','hidden');
		var currentDate      = new Date();
		if(opts!=undefined&&opts.preSpDate!=undefined&&opts.preSpDate<0&&currentDate.getDate()==1){
			currentDate      = new Date(new Date()-86400000);
		}
		var str = "<thead><tr><td>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td>六</td></tr></thead>";
		str += "<tbody>";
		currentDate.setDate(1);
		monthCount=parseInt(opts.maxDays/30)+1;
	...
	function tdClick(selectBeginDate){
		...
		$("td",$('#ncalendar')).click(function(){
			...
			if(selectBeginDate&&tds.eq(tds.index($(this))+opts.minSelect).hasClass('unable')) return;
			$('span',$(this)).remove();
			var self = $(this).html();
			var _this = $(this);
			...
			if( tds.index($(this))<=tds.index($('#ncalendar .begin'))+opts.minSelect-1)  selectBeginDate=true;
			if(selectBeginDate){
				var end_index  = 0;
				if(selectedDate.getTime() >= selectedDateEnd.getTime()-opts.minSelect*MICRO_SECOND_DATE){
					selectedDateBegin = new Date(selectedDate.getFullYear() , selectedDate.getMonth() , selectedDate.getDate());
					selectedDateEnd=new Date(selectedDate.getFullYear() , selectedDate.getMonth() , selectedDate.getDate() + opts.minSelect);
				}else{
				...
				if ( opts._parent !=null && !opts.select_single&&opts.minSelect>=2){
				...
				if( opts._parent ==null || opts.select_single ){
					...
					window.setTimeout(function(){history.back(-1);},500);
	...
	if ( opts._parent !=null){
		$('#' +  opts._parent).click(function(){
			...
			$('body').append('<div id="ncalendar"><div><table>' + generate(selectedDateBegin,selectedDateEnd,true,opts) + '</table></div></div>');
			tdClick(true);
		});
	}
	else{
		...
		$('#' + opts.endTimeElement).click(function(){
			history.pushState({ path: this.path }, '', this.href);
			$('body').append('<div id="ncalendar"><div><table>' + generate(selectedDateBegin,selectedDateEnd,false,opts) + '</table></div></div>');
			tdClick(false);
		});
	}
